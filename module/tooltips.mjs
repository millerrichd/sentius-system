...
export default class Tooltips5e {
  ...
  async _onTooltipActivate() {
    ...
    if ( loading?.dataset.uuid ) {
      ...
      if ( doc instanceof sentius.documents.Actor5e ) return this._onHoverActor(doc);
      return this._onHoverContentLink(doc);
    }
  ...
  async _onHoverPassive(skill, ability, dc) {
    const skillConfig = CONFIG.SENTIUS.skills[skill];
    const abilityConfig = CONFIG.SENTIUS.abilities[ability ?? skillConfig.ability];

    let label;
    if ( skillConfig ) {
      label = game.i18n.format("SENTIUS.SkillPassiveSpecificHint", { skill: skillConfig.label, ability: abilityConfig.label });
    } else {
      ...
      label = game.i18n.format("SENTIUS.SkillPassiveHint", { skill: abilityConfig.label });
    }

    const party = game.settings.get("sentius", "primaryParty")?.actor;
    if ( !party ) {
    ...
    this.tooltip.classList.add("sentius-tooltip", "passive-tooltip");
    this.tooltip.innerHTML = await renderTemplate("systems/sentius/templates/journal/passive-tooltip.hbs", context);
