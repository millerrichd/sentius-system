...
/**
 * A data model and API layer which handles the schema and functionality of "group" type Actors in the sentius system.
 ...
 * const g = new sentius.documents.Actor5e({
 *  type: "group",
 *  name: "Test Group",
 *  system: {
...
export default class GroupActor extends ActorDataModel.mixin(CurrencyTemplate) {
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      type: new SchemaField({
        value: new StringField({ initial: "party", label: "SENTIUS.Group.Type" })
      }),
      description: new SchemaField({
        full: new HTMLField({ label: "SENTIUS.Description" }),
        summary: new HTMLField({ label: "SENTIUS.DescriptionSummary" })
      }),
      members: new ArrayField(new SchemaField({
        actor: new ForeignDocumentField(foundry.documents.BaseActor),
        quantity: new SchemaField({
          value: new NumberField({ initial: 1, integer: true, min: 0, label: "SENTIUS.Quantity" }),
          formula: new FormulaField({ label: "SENTIUS.QuantityFormula" })
        })
      }), { label: "SENTIUS.GroupMembers" }),
      attributes: new SchemaField({
        movement: new SchemaField({
          land: new NumberField({ nullable: false, min: 0, step: 0.1, initial: 0, label: "SENTIUS.MovementLand" }),
          water: new NumberField({ nullable: false, min: 0, step: 0.1, initial: 0, label: "SENTIUS.MovementWater" }),
          air: new NumberField({ nullable: false, min: 0, step: 0.1, initial: 0, label: "SENTIUS.MovementAir" })
        })
      }, { label: "SENTIUS.Attributes" }),
      details: new SchemaField({
        xp: new SchemaField({
          value: new NumberField({ integer: true, min: 0, label: "SENTIUS.ExperiencePointsCurrent" })
  ...
  async rest(config, result) {
     ...
     * @function sentius.groupRestCompleted
     * @memberof hookEvents
     ...
     */
    Hooks.callAll("sentius.groupRestCompleted", this.parent, results);
  ...
  _onUpdate(changed, options, userId) {
    if ( !foundry.utils.hasProperty(changed, "system.type.value") || (game.user !== game.users.activeGM)
      || (game.settings.get("sentius", "primaryParty")?.actor !== this.parent)
