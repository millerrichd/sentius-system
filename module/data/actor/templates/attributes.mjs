...
export default class AttributesFields {
  ...
  static get common() {
    return {
      init: new RollConfigField({
        ability: "",
        bonus: new FormulaField({ required: true, label: "SENTIUS.InitiativeBonus" })
      }, { label: "SENTIUS.Initiative" }),
  ...
  static get creature() {
    return {
      attunement: new SchemaField({
        max: new NumberField({
          required: true, nullable: false, integer: true, min: 0, initial: 3, label: "SENTIUS.AttunementMax"
        })
      }, { label: "SENTIUS.Attunement" }),
      senses: new SensesField(),
      spellcasting: new StringField({ required: true, blank: true, label: "SENTIUS.SpellAbility" }),
      exhaustion: new NumberField({
        required: true, nullable: false, integer: true, min: 0, initial: 0, label: "SENTIUS.Exhaustion"
      }),
      concentration: new RollConfigField({
        ability: "",
        bonuses: new SchemaField({
          save: new FormulaField({ required: true, label: "SENTIUS.ConcentrationBonus" })
        }),
        limit: new NumberField({ integer: true, min: 0, initial: 1, label: "SENTIUS.ConcentrationLimit" })
      }, { label: "SENTIUS.Concentration" })
    };
  }
  ...
  static prepareConcentration(rollData) {
    ...
    const abilityId = concentration.ability || CONFIG.SENTIUS.defaultAbilities.concentration;
  ...
  static prepareEncumbrance(rollData, { validateItem }={}) {
    const config = CONFIG.SENTIUS.encumbrance;
    const encumbrance = this.attributes.encumbrance ??= {};
    const baseUnits = CONFIG.SENTIUS.encumbrance.baseUnits[this.parent.type]
      ?? CONFIG.SENTIUS.encumbrance.baseUnits.default;
    const unitSystem = game.settings.get("sentius", "metricWeightUnits") ? "metric" : "imperial";
    ...
    if ( game.settings.get("sentius", "currencyWeight") && currency ) {
    ...
    const keys = Object.keys(CONFIG.SENTIUS.actorSizes);
    const index = keys.findIndex(k => k === this.traits.size);
    const sizeConfig = CONFIG.SENTIUS.actorSizes[
      keys[this.parent.flags.sentius?.powerfulBuild ? Math.min(index + 1, keys.length - 1) : index]
    ];
  ...
  static prepareExhaustionLevel() {
    ...
    const level = exhaustion?.getFlag("sentius", "exhaustionLevel");
  ...
  static prepareMovement() {
    ...
    const reduction = game.settings.get("sentius", "rulesVersion") === "modern"
      ? this.attributes.exhaustion * (CONFIG.SENTIUS.conditionTypes.exhaustion?.reduction?.speed ?? 0) : 0;
    for ( const type in CONFIG.SENTIUS.movementTypes ) {
      ...
      if ( noMovement || (crawl && (type !== "walk")) ) speed = 0;
        ...
        if ( heavilyEncumbered ) {
          speed = Math.max(0, speed - (CONFIG.SENTIUS.encumbrance.speedReduction.heavilyEncumbered[units] ?? 0));
        } else if ( encumbered ) {
          speed = Math.max(0, speed - (CONFIG.SENTIUS.encumbrance.speedReduction.encumbered[units] ?? 0));
        }
        if ( exceedingCarryingCapacity ) {
          speed = Math.min(speed, CONFIG.SENTIUS.encumbrance.speedReduction.exceedingCarryingCapacity[units] ?? 0);
        }
      }
