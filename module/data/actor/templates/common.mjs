...
export default class CommonTemplate extends ActorDataModel.mixin(CurrencyTemplate) {
    ...
    return this.mergeSchema(super.defineSchema(), {
      abilities: new MappingField(new SchemaField({
        value: new NumberField({
          required: true, nullable: false, integer: true, min: 0, initial: 10, label: "SENTIUS.AbilityScore"
        }),
        proficient: new NumberField({
          required: true, integer: true, min: 0, max: 1, initial: 0, label: "SENTIUS.ProficiencyLevel"
        }),
        max: new NumberField({
          required: true, integer: true, nullable: true, min: 0, initial: null, label: "SENTIUS.AbilityScoreMax"
        }),
        bonuses: new SchemaField({
          check: new FormulaField({ required: true, label: "SENTIUS.AbilityCheckBonus" }),
          save: new FormulaField({ required: true, label: "SENTIUS.SaveBonus" })
        }, { label: "SENTIUS.AbilityBonuses" })
      }), {
        initialKeys: CONFIG.SENTIUS.abilities, initialValue: this._initialAbilityValue.bind(this),
        initialKeysOnly: true, label: "SENTIUS.Abilities"
      })
    });
  }
  ...
  static _initialAbilityValue(key, initial, existing) {
    const config = CONFIG.SENTIUS.abilities[key];
    if ( config ) {
  ...
  prepareAbilities({ rollData={}, originalSaves }={}) {
    const flags = this.parent.flags.sentius ?? {};
    ...
    for ( const [id, abl] of Object.entries(this.abilities) ) {
      ...
      if ( !Number.isFinite(abl.max) ) abl.max = CONFIG.SENTIUS.maxAbilityScore;
  ...
  calculateAbilityCheckProficiency(multiplier, ability) {
    ...
    if ( multiplier < 1 ) {
      ...
      else if ( this.parent.flags.sentius?.jackOfAllTrades ) multiplier = .5;
    }
