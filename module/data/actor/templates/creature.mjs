...
export default class CreatureTemplate extends CommonTemplate {
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      bonuses: new SchemaField({
        mwak: makeAttackBonuses({ label: "SENTIUS.BonusMWAttack" }),
        rwak: makeAttackBonuses({ label: "SENTIUS.BonusRWAttack" }),
        msak: makeAttackBonuses({ label: "SENTIUS.BonusMSAttack" }),
        rsak: makeAttackBonuses({ label: "SENTIUS.BonusRSAttack" }),
        abilities: new SchemaField({
          check: new FormulaField({ required: true, label: "SENTIUS.BonusAbilityCheck" }),
          save: new FormulaField({ required: true, label: "SENTIUS.BonusAbilitySave" }),
          skill: new FormulaField({ required: true, label: "SENTIUS.BonusAbilitySkill" })
        }, { label: "SENTIUS.BonusAbility" }),
        spell: new SchemaField({
          dc: new FormulaField({ required: true, deterministic: true, label: "SENTIUS.BonusSpellDC" })
        }, { label: "SENTIUS.BonusSpell" })
      }, { label: "SENTIUS.Bonuses" }),
      skills: new MappingField(new RollConfigField({
        value: new NumberField({
          required: true, nullable: false, min: 0, max: 2, step: 0.5, initial: 0, label: "SENTIUS.ProficiencyLevel"
        }),
        ability: "dex",
        bonuses: new SchemaField({
          check: new FormulaField({ required: true, label: "SENTIUS.SkillBonusCheck" }),
          passive: new FormulaField({ required: true, label: "SENTIUS.SkillBonusPassive" })
        }, { label: "SENTIUS.SkillBonuses" })
      }), {
        initialKeys: CONFIG.SENTIUS.skills, initialValue: this._initialSkillValue,
        initialKeysOnly: true, label: "SENTIUS.Skills"
      }),
      tools: new MappingField(new RollConfigField({
        value: new NumberField({
          required: true, nullable: false, min: 0, max: 2, step: 0.5, initial: 1, label: "SENTIUS.ProficiencyLevel"
        }),
        ability: "int",
        bonuses: new SchemaField({
          check: new FormulaField({ required: true, label: "SENTIUS.CheckBonus" })
        }, { label: "SENTIUS.ToolBonuses" })
      })),
      spells: new MappingField(new SchemaField({
        value: new NumberField({
          nullable: false, integer: true, min: 0, initial: 0, label: "SENTIUS.SpellProgAvailable"
        }),
        override: new NumberField({
          integer: true, min: 0, label: "SENTIUS.SpellProgOverride"
        })
      }), { initialKeys: this._spellLevels, label: "SENTIUS.SpellLevels" })
    });
  }
  ...
  static _initialSkillValue(key, initial) {
    if ( CONFIG.SENTIUS.skills[key]?.ability ) initial.ability = CONFIG.SENTIUS.skills[key].ability;
    return initial;
  }
  ...
  static get _spellLevels() {
    const levels = Object.keys(CONFIG.SENTIUS.spellLevels).filter(a => a !== "0").map(l => `spell${l}`);
    return [...levels, "pact"];
  }
  ...
  static #migrateSensesData(source) {
    ...
    for ( let s of original.split(",") ) {
      ...
      if ( (type in CONFIG.SENTIUS.senses) && !(type in source.attributes.senses) ) {
  ...
  static #migrateToolData(source) {
    ...
    for ( const prof of original.value ) {
      const validProf = (prof in CONFIG.SENTIUS.toolProficiencies) || (prof in CONFIG.SENTIUS.tools);
...
function makeAttackBonuses(schemaOptions={}) {
  return new SchemaField({
    attack: new FormulaField({required: true, label: "SENTIUS.BonusAttack"}),
    damage: new FormulaField({required: true, label: "SENTIUS.BonusDamage"})
  }, schemaOptions);
}
