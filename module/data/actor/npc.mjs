...
export default class NPCData extends CreatureTemplate {
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      attributes: new SchemaField({
        ...
        }, {label: "SENTIUS.HitDice"}),
        hp: new SchemaField({
          value: new NumberField({
            nullable: false, integer: true, min: 0, initial: 10, label: "SENTIUS.HitPointsCurrent"
          }),
          max: new NumberField({
            nullable: false, integer: true, min: 0, initial: 10, label: "SENTIUS.HitPointsMax"
          }),
          ...
          formula: new FormulaField({required: true, label: "SENTIUS.HPFormula"})
      ...
      details: new SchemaField({
        ...
        environment: new StringField({required: true, label: "SENTIUS.Environment"}),
        cr: new NumberField({
          required: true, nullable: true, min: 0, initial: 1, label: "SENTIUS.ChallengeRating"
        }),
        spellLevel: new NumberField({
          required: true, nullable: false, integer: true, min: 0, initial: 0, label: "SENTIUS.SpellcasterLevel"
        })
      }, {label: "SENTIUS.Details"}),
      resources: new SchemaField({
        legact: new SchemaField({
          value: new NumberField({
            required: true, nullable: false, integer: true, min: 0, initial: 0, label: "SENTIUS.LegActRemaining"
          }),
          max: new NumberField({
            required: true, nullable: false, integer: true, min: 0, initial: 0, label: "SENTIUS.LegActMax"
          })
        }, {label: "SENTIUS.LegAct"}),
        legres: new SchemaField({
          value: new NumberField({
            required: true, nullable: false, integer: true, min: 0, initial: 0, label: "SENTIUS.LegResRemaining"
          }),
          max: new NumberField({
            required: true, nullable: false, integer: true, min: 0, initial: 0, label: "SENTIUS.LegResMax"
          })
        }, {label: "SENTIUS.LegRes"}),
        lair: new SchemaField({
          value: new BooleanField({required: true, label: "SENTIUS.LairAct"}),
          initiative: new NumberField({
            required: true, integer: true, label: "SENTIUS.LairActionInitiative"
          })
  ...
  static get compendiumBrowserFilters() {
    return new Map([
      ["size", {
        ...
        type: "set",
        config: {
          choices: CONFIG.SENTIUS.actorSizes,
      ...
      ["type", {
        ...
        type: "set",
        config: {
          choices: CONFIG.SENTIUS.creatureTypes,
          keyPath: "system.details.type.value"
        }
      }],
      ["cr", {
        label: "SENTIUS.ChallengeRating",
        type: "range",
        config: {
      ...
      ["movement", {
        label: "SENTIUS.Movement",
        type: "set",
        config: {
          choices: CONFIG.SENTIUS.movementTypes
        },
  ...
  static #migrateTypeData(source) {
    ...
    if ( match ) {
      ...
      const typeMatch = Object.entries(CONFIG.SENTIUS.creatureTypes).find(([k, v]) => {
        return (typeLc === k)
      ...
      if ( match.groups.size ) {
        ...
        const sizeMatch = Object.entries(CONFIG.SENTIUS.actorSizes).find(([k, v]) => {
  ...
  prepareBaseData() {
    ...
    this.attributes.hd.denomination = Number(denomination ?? CONFIG.SENTIUS.actorSizes[this.traits.size]?.hitDie ?? 4);
  ...
  prepareDerivedData() {
    ...
    const hpOptions = {
      ...
      mod: this.abilities[CONFIG.SENTIUS.defaultAbilities.hitPoints ?? "con"]?.mod ?? 0
    };
  ...
  async toEmbed(config, options={}) {
    ...
    template.innerHTML = await renderTemplate("systems/sentius/templates/actors/embeds/npc-embed.hbs", context);

    /**
     * A hook event that fires after an embedded NPC stat block rendered.
     * @function sentius.renderNPCStatBlock
     * @memberof hookEvents
     ...
     */
    Hooks.call("sentius.renderNPCStatBlock", this.parent, template, config, options);
  ...
  async _prepareEmbedContext() {
    ...
    const context = {
      ...
      actionSections: {
        trait: {
          label: game.i18n.localize("SENTIUS.NPC.SECTIONS.Traits"),
          actions: []
        },
        action: {
          label: game.i18n.localize("SENTIUS.NPC.SECTIONS.Actions"),
          actions: []
        },
        bonus: {
          label: game.i18n.localize("SENTIUS.NPC.SECTIONS.BonusActions"),
          actions: []
        },
        reaction: {
          label: game.i18n.localize("SENTIUS.NPC.SECTIONS.Reactions"),
          actions: []
        },
        legendary: {
          label: game.i18n.localize("SENTIUS.NPC.SECTIONS.LegendaryActions"),
      ...
      summary: {
        ...
        cr: `${formatCR(this.details.cr, { narrow: false })} (${game.i18n.localize("SENTIUS.ExperiencePointsAbbr")} ${
          formatNumber(this.details.xp.value)}; ${game.i18n.localize("SENTIUS.ProficiencyBonusAbbr")} ${
        ...
        senses: [
          formatter.format([
            ...Object.entries(CONFIG.SENTIUS.senses)
          ...
          `${game.i18n.localize("SENTIUS.PassivePerception")} ${formatNumber(this.skills.prc.passive)}`
        ].filterJoin("; "),

        // Skills (e.g. `Perception +17, Stealth +7`)
        skills: formatter.format(
          Object.entries(CONFIG.SENTIUS.skills)
        ...
        speed: formatter.format([
          ...
          ...Object.entries(CONFIG.SENTIUS.movementTypes)
            ...
            .map(([k, label]) => {
              ...
              if ( (k === "fly") && this.attributes.movement.hover ) {
                prepared = `${prepared} (${game.i18n.localize("SENTIUS.MovementHover").toLowerCase()})`;
              }
              return prepared;
            })
        ]),
        ...
        tag: game.i18n.format("SENTIUS.CreatureTag", {
          size: CONFIG.SENTIUS.actorSizes[this.traits.size]?.label ?? "",
    ...
    for ( const [index, [key, { abbreviation }]] of Object.entries(CONFIG.SENTIUS.abilities).entries() ) {
    ...
    for ( const type of ["vulnerabilities", "resistances", "immunities"] ) {
      ...
      for ( const category of ["damage", "condition"] ) {
        ...
        const { value, physical } = data.value.reduce((acc, t) => {
          if ( data.bypasses?.size && CONFIG.SENTIUS.damageTypes[t]?.isPhysical ) acc.physical.push(t);
          else acc.value.push(t);
          return acc;
        }, { value: [], physical: [] });
        const list = prepareTrait({ value, custom: data.custom }, trait);
        if ( list ) entries.push(list);
        if ( physical.length ) entries.push(game.i18n.format("SENTIUS.DamagePhysicalBypasses", {
          damageTypes: game.i18n.getListFormatter({ style: "long", type: "conjunction" }).format(
            physical.map(t => CONFIG.SENTIUS.damageTypes[t].label)
          ),
          bypassTypes: game.i18n.getListFormatter({ style: "long", type: "disjunction" }).format(
            Array.from(data.bypasses).map(t => CONFIG.SENTIUS.itemProperties[t]?.label).filter(_ => _)
          )
        }));
      }
