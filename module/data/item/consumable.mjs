...
export default class ConsumableData extends ItemDataModel.mixin(
  ...
  /** @override */
  static LOCALIZATION_PREFIXES = ["SENTIUS.CONSUMABLE", "SENTIUS.SOURCE"];
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      type: new ItemTypeField({ baseItem: false }, { label: "SENTIUS.ItemConsumableType" }),
  ...
  static get compendiumBrowserFilters() {
    return new Map([
      ["type", {
        label: "SENTIUS.ItemConsumableType",
        type: "set",
        config: {
          choices: CONFIG.SENTIUS.consumableTypes,
  ...
  prepareDerivedData() {
    ...
    const config = CONFIG.SENTIUS.consumableTypes[this.type.value];
    if ( config ) {
  ...
  async getSheetData(context) {
    ...
    context.parts = ["sentius.details-consumable", "sentius.field-uses"];
  }
  ...
  get chatProperties() {
    return [
      this.type.label,
      this.hasLimitedUses ? `${this.uses.value}/${this.uses.max} ${game.i18n.localize("SENTIUS.Charges")}` : null,
      this.priceLabel
    ];
  }
  ...
  static get itemCategories() {
    return CONFIG.SENTIUS.consumableTypes;
  }
  ...
  get proficiencyMultiplier() {
    const isProficient = this.parent?.actor?.getFlag("sentius", "tavernBrawlerFeat");
  ...
  get validProperties() {
    ...
    if ( this.type.value === "ammo" ) Object.entries(CONFIG.SENTIUS.itemProperties).forEach(([k, v]) => {
      if ( v.isPhysical ) valid.add(k);
      valid.add("ret");
    });
    else if ( this.type.value === "scroll" ) CONFIG.SENTIUS.validProperties.spell
  ...
  async getCraftCost(options={}) {
    ...
    const { consumable, magic } = CONFIG.SENTIUS.crafting;
    const { rarity } = this;
  ...
  getRollData(...options) {
    ...
    const spellLevel = this.parent.getFlag("sentius", "spellLevel");
