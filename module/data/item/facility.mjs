...
export default class FacilityData extends ItemDataModel.mixin(ActivitiesTemplate, ItemDescriptionTemplate) {
  ...
  /** @override */
  static LOCALIZATION_PREFIXES = ["SENTIUS.FACILITY", "SENTIUS.SOURCE"];
  ...
  static get compendiumBrowserFilters() {
    ...
    return new Map([
      ["level", {
        label: "SENTIUS.FACILITY.FIELDS.level.label",
        type: "range",
        config: {
          keyPath: "system.level",
          min,
          max: CONFIG.SENTIUS.maxLevel
        }
      }],
      ["type", {
        label: "SENTIUS.FACILITY.FIELDS.type.value.label",
        type: "set",
        config: {
          choices: Object.fromEntries(Object.entries(CONFIG.SENTIUS.facilities.types).map(([k, v]) => [k, v.label])),
  ...
  _createOrderActivity(id, order) {
    const activity = new OrderActivity({
      order,
      _id: staticID(id),
      name: game.i18n.format("SENTIUS.FACILITY.Order.Issue", {
        order: game.i18n.localize(`SENTIUS.FACILITY.Orders.${order}.inf`)
      })
    }, { parent: this.parent });
  ...
  prepareBaseData() {
    ...
    // Activities
    if ( (this.type.value === "special") && this.order ) this._createOrderActivity("sentiusFacOrder", this.order);
    if ( this.enlargeable ) this._createOrderActivity("sentiusFacEnlarge", "enlarge");
    if ( !this.building.built ) this._createOrderActivity("sentiusFacBuild", "build");

    // TODO: Allow order activities to be user-creatable and configurable to avoid having to hard-code this.
    if ( this.type.subtype === "garden" ) this._createOrderActivity("sentiusFacChange", "change");
  }
  ...
  prepareDerivedData() {
    ...
    // Type
    const config = CONFIG.SENTIUS.facilities.types[this.type.value];
    this.type.label = config?.subtypes?.[this.type.subtype] ?? config?.label;

    // Price
    if ( this.type.value === "basic" ) {
      const { value, days } = CONFIG.SENTIUS.facilities.sizes[this.size];
      this.price = { value, days, denomination: "gp" };
    }

    // Squares
    this.squares = CONFIG.SENTIUS.facilities.sizes[this.size].squares;
    ...
    if ( this.order ) labels.order = game.i18n.localize(`SENTIUS.FACILITY.Orders.${this.order}.present`);
  }
  ...
  async getSheetData(context) {
    context.facilitySubtypes = CONFIG.SENTIUS.facilities.types[this.type.value]?.subtypes ?? {};
    context.singleDescription = true;
    context.subtitles = [
      { label: this.type.label },
      { label: CONFIG.SENTIUS.facilities.sizes[this.size].label }
    ];
    context.parts = ["sentius.details-facility"];
  }
  ...
  async _preCreate(data, options, user) {
    ...
    if ( !actor ) return;
    const { basic } = CONFIG.SENTIUS.facilities.advancement;
