...
export default class ActionTemplate extends ItemDataModel {
  ...
  static defineSchema() {
    ...
    return {
      ability: new StringField({required: true, nullable: true, initial: null, label: "SENTIUS.AbilityModifier"}),
      actionType: new StringField({required: true, nullable: true, initial: null, label: "SENTIUS.ItemActionType"}),
      attack: new SchemaField({
        bonus: new FormulaField({required: true, label: "SENTIUS.ItemAttackBonus"}),
        flat: new BooleanField({label: "SENTIUS.ItemAttackFlat"})
      }),
      chatFlavor: new StringField({required: true, label: "SENTIUS.ChatFlavor"}),
      critical: new SchemaField({
        threshold: new NumberField({
          required: true, integer: true, initial: null, positive: true, label: "SENTIUS.ItemCritThreshold"
        }),
        damage: new FormulaField({required: true, label: "SENTIUS.ItemCritExtraDamage"})
      }),
      damage: new SchemaField({
        parts: new ArrayField(new ArrayField(new StringField({nullable: true})), {required: true}),
        versatile: new FormulaField({required: true, label: "SENTIUS.VersatileDamage"})
      }, {label: "SENTIUS.Damage"}),
      enchantment: new EnchantmentField(),
      formula: new FormulaField({required: true, label: "SENTIUS.OtherFormula"}),
      save: new SchemaField({
        ability: new StringField({required: true, blank: true, label: "SENTIUS.Ability"}),
        dc: new NumberField({required: true, min: 0, integer: true, label: "SENTIUS.AbbreviationDC"}),
        scaling: new StringField({required: true, blank: false, initial: "spell", label: "SENTIUS.ScalingFormula"})
      }, {label: "SENTIUS.SavingThrow"}),
