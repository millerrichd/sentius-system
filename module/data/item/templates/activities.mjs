...
export default class ActivitiesTemplate extends SystemDataModel {
  ...
  /** @override */
  static LOCALIZATION_PREFIXES = ["SENTIUS.USES"];
  ...
  get appliedEnchantments() {
    return sentius.registry.enchantments.applied(this.parent.uuid);
  }
  ...
  static #migrateUses(source) {
    ...
    // If period is not blank, set an appropriate recovery type
      ...
      if ( CONFIG.SENTIUS.limitedUsePeriods[source.uses.per]?.formula && source.uses.recovery ) {
  ...
  static initializeActivities(source) {
    ...
    if ( source._id && source.type && ("value" in uses) && uses.max ) {
      foundry.utils.setProperty(source, "flags.sentius.migratedUses", uses.value);
    }
  }
  ...
  static #createInitialActivity(source) {
    ...
    const cls = CONFIG.SENTIUS.activityTypes[type].documentClass;
    cls.createInitialActivity(source);

    if ( (type !== "save") && source.system.save?.ability ) {
      CONFIG.SENTIUS.activityTypes.save.documentClass.createInitialActivity(source, { offset: 1 });
    }
    if ( (source.type !== "weapon") && source.system.damage?.versatile ) {
      CONFIG.SENTIUS.activityTypes.damage.documentClass.createInitialActivity(source, { offset: 2, versatile: true });
    }
    if ( (type !== "utility") && source.system.formula ) {
      CONFIG.SENTIUS.activityTypes.utility.documentClass.createInitialActivity(source, { offset: 3 });
    }
  }
  ...
  async preUpdateActivities(changed, options, user) {
    ...
    foundry.utils.setProperty(changed, "flags.sentius.riders", {
    ...
    if ( removed.length ) foundry.utils.setProperty(options, "sentius.removedCachedItems", removed);
  }
  ...
  async onUpdateActivities(changed, options, userId) {
    ...
    if ( options.sentius?.removedCachedItems ) {
      await this.parent.actor.deleteEmbeddedDocuments("Item", options.sentius.removedCachedItems);
    }
  ...
  static _applyActivityShims() {
    ...
    addShims("ActivatedEffectTemplate", {
      ...
      hasScalarDuration: () => this.duration.units in CONFIG.SENTIUS.scalarTimePeriods,
      hasScalarRange: () => this.range.units in CONFIG.SENTIUS.movementUnits,
