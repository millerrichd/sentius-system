 ...
 * @property {string} rarity              Item rarity as defined in `SENTIUS.itemRarity`.
 * @mixin
 */
export default class PhysicalItemTemplate extends SystemDataModel {
  ...
  static defineSchema() {
    return {
      container: new ForeignDocumentField(foundry.documents.BaseItem, {
        idOnly: true, label: "SENTIUS.Container"
      }),
      quantity: new NumberField({
        required: true, nullable: false, integer: true, initial: 1, min: 0, label: "SENTIUS.Quantity"
      }),
      weight: new SchemaField({
        value: new NumberField({
          required: true, nullable: false, initial: 0, min: 0, label: "SENTIUS.Weight"
        }),
        units: new StringField({
          required: true, blank: false, label: "SENTIUS.UNITS.WEIGHT.Label", initial: () => defaultUnits("weight")
        })
      }, {label: "SENTIUS.Weight"}),
      price: new SchemaField({
        value: new NumberField({
          required: true, nullable: false, initial: 0, min: 0, label: "SENTIUS.Price"
        }),
        denomination: new StringField({
          required: true, blank: false, initial: "gp", label: "SENTIUS.Currency"
        })
      }, {label: "SENTIUS.Price"}),
      rarity: new StringField({required: true, blank: true, label: "SENTIUS.Rarity"})
    };
  }
  ...
  static get compendiumBrowserPhysicalItemFilters() {
    return [
      ...
      ["rarity", {
        label: "SENTIUS.Rarity",
        type: "set",
        config: {
          blank: game.i18n.localize("SENTIUS.ItemRarityMundane").capitalize(),
          choices: Object.entries(CONFIG.SENTIUS.itemRarity).reduce((obj, [key, label]) => {
  ...
  get priceLabel() {
    ...
    const hasPrice = value && (denomination in CONFIG.SENTIUS.currencies);
    return hasPrice ? `${value} ${CONFIG.SENTIUS.currencies[denomination].label}` : null;
  }
  ...
  get physicalItemSheetFields() {
    return [{
      label: CONFIG.SENTIUS.itemRarity[this.rarity],
      value: this._source.rarity,
      requiresIdentification: true,
      field: this.schema.getField("rarity"),
      choices: CONFIG.SENTIUS.itemRarity,
      blank: "SENTIUS.Rarity",
      classes: "item-rarity"
    }];
  }
   ...
   */
  preparePhysicalData() {
    if ( !("gp" in CONFIG.SENTIUS.currencies) ) return;
    const { value, denomination } = this.price;
    const { conversion } = CONFIG.SENTIUS.currencies[denomination] ?? {};
    const { gp } = CONFIG.SENTIUS.currencies;
    if ( conversion ) {
  ...
  static #migrateRarity(source) {
    if ( !("rarity" in source) || CONFIG.SENTIUS.itemRarity[source.rarity] ) return;
    source.rarity = Object.keys(CONFIG.SENTIUS.itemRarity).find(key =>
      CONFIG.SENTIUS.itemRarity[key].toLowerCase() === source.rarity.toLowerCase()
    ) ?? "";
  }
