 ...
 *
 * @property {string} attunement  Attunement information as defined in `SENTIUS.attunementTypes`.
...
export default class EquippableItemTemplate extends SystemDataModel {
  ...
  static defineSchema() {
    return {
      attunement: new StringField({required: true, label: "SENTIUS.Attunement"}),
      attuned: new BooleanField({label: "SENTIUS.Attuned"}),
      equipped: new BooleanField({required: true, label: "SENTIUS.Equipped"})
    };
  }
  ...
  get equippableItemCardProperties() {
    return [
      this.attunement === "required" ? CONFIG.SENTIUS.attunementTypes.required : null,
      game.i18n.localize(this.equipped ? "SENTIUS.Equipped" : "SENTIUS.Unequipped"),
      ("proficient" in this) ? CONFIG.SENTIUS.proficiencyLevels[this.prof?.multiplier || 0] : null
    ];
  }
