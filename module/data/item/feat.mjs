...
export default class FeatData extends ItemDataModel.mixin(
  ...
  /** @override */
  static LOCALIZATION_PREFIXES = ["SENTIUS.ENCHANTMENT", "SENTIUS.Prerequisites", "SENTIUS.SOURCE"];
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      ...
      type: new ItemTypeField({baseItem: false}, {label: "SENTIUS.ItemFeatureType"}),
      prerequisites: new SchemaField({
        level: new NumberField({integer: true, min: 0})
      }),
      properties: new SetField(new StringField(), {
        label: "SENTIUS.ItemFeatureProperties"
      }),
      requirements: new StringField({required: true, nullable: true, label: "SENTIUS.Requirements"})
    });
  }
  ...
  static get compendiumBrowserFilters() {
    return new Map([
      ["category", {
        label: "SENTIUS.Item.Category.Label",
        type: "set",
        config: {
          choices: CONFIG.SENTIUS.featureTypes,
          keyPath: "system.type.value"
        }
      }],
      ["subtype", {
        label: "SENTIUS.ItemFeatureType",
        type: "set",
        config: {
          choices: Object.values(CONFIG.SENTIUS.featureTypes).reduce((obj, config) => {
  ...
  prepareDerivedData() {
    ...
    if ( this.type.value ) {
      const config = CONFIG.SENTIUS.featureTypes[this.type.value];
    ...
    if ( activation === "legendary" ) label = game.i18n.localize("SENTIUS.LegendaryActionLabel");
    else if ( activation === "lair" ) label = game.i18n.localize("SENTIUS.LairActionLabel");
    else if ( activation === "action" && this.hasAttack ) label = game.i18n.localize("SENTIUS.Attack");
    else if ( activation ) label = game.i18n.localize("SENTIUS.Action");
    else label = game.i18n.localize("SENTIUS.Passive");
    this.parent.labels ??= {};
  ...
  async getSheetData(context) {
    context.subtitles = [
      ...
      { label: this.requirements, value: this._source.requirements, field: this.schema.getField("requirements"),
        placeholder: "SENTIUS.Requirements" }
    ];
    context.parts = ["sentius.details-feat", "sentius.field-uses"];
  }
  ...
  get isEnchantmentSource() {
    return CONFIG.SENTIUS.featureTypes[this.type?.value]?.subtypes?.[this.type?.subtype]
      && (this.type?.subtype in CONFIG.SENTIUS.featureTypes.enchantment.subtypes);
  }
