...
export default class RaceData extends ItemDataModel.mixin(ItemDescriptionTemplate) {
  ...
  static get compendiumBrowserFilters() {
    return new Map([
      ["hasDarkvision", {
        label: "SENTIUS.CompendiumBrowser.Filters.HasDarkvision",
        type: "boolean",
  ...
  get movementLabels() {
    ...
    return Object.entries(CONFIG.SENTIUS.movementTypes).reduce((obj, [k, label]) => {
  ...
  get sensesLabels() {
    ...
    return Object.entries(CONFIG.SENTIUS.senses).reduce((arr, [k, label]) => {
  ...
  async getSheetData(context) {
    ...
    context.info = [{
      ...
      config: "type",
      tooltip: "SENTIUS.CreatureTypeTitle"
    },
    {
      label: "SENTIUS.Movement",
      classes: "info-sm info-grid",
      config: "movement",
      tooltip: "SENTIUS.MovementConfig",
      value: Object.entries(CONFIG.SENTIUS.movementTypes).reduce((str, [k, label]) => {
        const value = this.movement[k];
        if ( !value ) return str;
        return `${str}
    ...
    {
      label: "SENTIUS.Senses",
      classes: "info-sm info-grid",
      config: "senses",
      tooltip: "SENTIUS.SensesConfig",
      value: Object.entries(CONFIG.SENTIUS.senses).reduce((str, [k, label]) => {
        const value = this.senses[k];
        if ( !value ) return str;
        return `${str}
          <span class="key">${label}</span>
    ...
    context.parts = ["sentius.details-species"];
  }
  ...
  async _preCreate(data, options, user) {
    ...
    this.parent.updateSource({"system.advancement": toCreate.map(c => {
      const config = CONFIG.SENTIUS.advancementTypes[c.type];
