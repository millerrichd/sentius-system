...
export default class TargetField extends SchemaField {
  ...
  static prepareData(rollData, labels) {
    this.target.affects.scalar = this.target.affects.type
      && (CONFIG.SENTIUS.individualTargetTypes[this.target.affects.type]?.scalar !== false);
    if ( this.target.affects.scalar ) {
      prepareFormulaValue(this, "target.affects.count", "SENTIUS.TARGET.FIELDS.target.affects.count.label", rollData);
    ...
    if ( this.target.template.type ) {
      ...
      prepareFormulaValue(this, "target.template.count", "SENTIUS.TARGET.FIELDS.target.template.count.label", rollData);
      prepareFormulaValue(this, "target.template.size", "SENTIUS.TARGET.FIELDS.target.template.size.label", rollData);
      prepareFormulaValue(this, "target.template.width", "SENTIUS.TARGET.FIELDS.target.template.width.label", rollData);
      prepareFormulaValue(this, "target.template.height", "SENTIUS.TARGET.FIELDS.target.template.height.label", rollData);
    } else {
    ...
    const templateConfig = CONFIG.SENTIUS.areaTargetTypes[this.target.template.type];
    if ( templateConfig ) {
      const parts = [];
      if ( this.target.template.count > 1 ) parts.push(`${this.target.template.count} ×`);
      if ( this.target.template.units in CONFIG.SENTIUS.movementUnits ) {
    ...
    const affectsConfig = CONFIG.SENTIUS.individualTargetTypes[this.target.affects.type];
      ...
      sheet: affectsConfig?.counted ? game.i18n.format(
        `${affectsConfig.counted}.${this.target.affects.count ? pr.select(this.target.affects.count) : "other"}`, {
          number: this.target.affects.count ? formatNumber(this.target.affects.count)
            : game.i18n.localize(`SENTIUS.TARGET.Count.${this.target.template.type ? "Every" : "Any"}`)
        }
      ).trim().capitalize() : (affectsConfig?.label ?? ""),
      statblock: game.i18n.format(
        `${affectsConfig?.counted ?? "SENTIUS.TARGET.Type.Target.Counted"}.${pr.select(this.target.affects.count || 1)}`,
  ...
  static templateDimensions(type) {
    const sizes = CONFIG.SENTIUS.areaTargetTypes[type]?.sizes;
    const dimensions = { size: "SENTIUS.AreaOfEffect.Size.Label" };
    if ( sizes ) {
      dimensions.width = sizes.includes("width") && (sizes.includes("length") || sizes.includes("radius"));
      dimensions.height = sizes.includes("height");
      if ( sizes.includes("radius") ) dimensions.size = "SENTIUS.AreaOfEffect.Size.Radius";
      else if ( sizes.includes("length") ) dimensions.size = "SENTIUS.AreaOfEffect.Size.Length";
      else if ( sizes.includes("width") ) dimensions.size = "SENTIUS.AreaOfEffect.Size.Width";
      if ( sizes.includes("thickness") ) dimensions.width = "SENTIUS.AreaOfEffect.Size.Thickness";
      else if ( dimensions.width ) dimensions.width = "SENTIUS.AreaOfEffect.Size.Width";
      if ( dimensions.height ) dimensions.height = "SENTIUS.AreaOfEffect.Size.Height";
    }
    return dimensions;
  }
}
