...
export default class UsesField extends SchemaField {
  ...
  static prepareData(rollData, labels) {
    prepareFormulaValue(this, "uses.max", "SENTIUS.USES.FIELDS.uses.max.label", rollData);
    ...
    for ( const recovery of this.uses.recovery ) {
      if ( recovery.period === "recharge" ) {
        ...
        if ( labels ) labels.recharge ??= `${game.i18n.localize("SENTIUS.Recharge")} [${
          recovery.formula}${parseInt(recovery.formula) < 6 ? "+" : ""}]`;
      } else if ( recovery.period in CONFIG.SENTIUS.limitedUsePeriods ) {
        const config = CONFIG.SENTIUS.limitedUsePeriods[recovery.period];
  ...
  static getStatblockLabel() {
    ...
    if ( recovery.period === "recharge" ) {
      ...
      return `${game.i18n.localize("SENTIUS.Recharge")} ${value === 6 ? "6" : `${value}–6`}`;
    }

    // Recharge after a Short or Long Rest
    if ( ["lr", "sr"].includes(recovery.period) && (this.uses.max === 1) ) {
      return game.i18n.localize(`SENTIUS.Recharge${recovery.period === "sr" ? "Short" : "Long"}`);
    }

    // X/Day
    const period = CONFIG.SENTIUS.limitedUsePeriods[recovery.period === "sr" ? "sr" : "day"]?.label ?? "";
    if ( !period ) return "";
    ...
    if ( profile.type === "recoverAll" ) updates.spent = 0;
      ...
      try {
        ...
        if ( ["day", "dawn", "dusk"].includes(profile.period)
          && (game.settings.get("sentius", "restVariant") === "gritty") ) {
          roll.alter(7, 0, { multiplyNumeric: true });
        }
        total = (await roll.evaluate()).total;
      } catch(err) {
        Hooks.onError("UsesField#recoverUses", err, {
          msg: game.i18n.format("SENTIUS.ItemRecoveryFormulaWarning", {
      ...
      if ( newSpent !== this.uses.spent ) {
        ...
        if ( !roll.isDeterministic ) {
          ...
          const localizationKey = `SENTIUS.Item${diff < 0 ? "Loss" : "Recovery"}Roll${isMax ? "Max" : ""}`;
  ...
  static async rollRecharge(config={}, dialog={}, message={}) {
    ...
    if ( "sentius.preRollRecharge" in Hooks.events ) {
      foundry.utils.logCompatibilityWarning(
        "The `sentius.preRollRecharge` hook has been deprecated and replaced with `sentius.preRollRechargeV2`.",
      ...
      if ( Hooks.call("sentius.preRollRecharge", this, hookData) === false ) return;
    ...
    messageConfig.data.flavor = game.i18n.format("SENTIUS.ItemRechargeCheck", {
      name: this.name,
      result: game.i18n.localize(`SENTIUS.ItemRecharge${rolls[0].isSuccess ? "Success" : "Failure"}`)
    });
     ...
     * been made.
     * @function sentius.rollRechargeV2
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("sentius.rollRechargeV2", rolls, { subject: this, updates }) === false ) return rolls;

    if ( "sentius.rollRecharge" in Hooks.events ) {
      foundry.utils.logCompatibilityWarning(
        "The `sentius.rollRecharge` hook has been deprecated and replaced with `sentius.rollRechargeV2`.",
        { since: "SENTIUS", until: "SENTIUS"" }
      );
      if ( Hooks.call("sentius.rollRecharge", this, rolls[0]) === false ) return rolls;
    }
     ...
     * @function sentius.postRollRecharge
     * @memberof hookEvents
     * @param {BasicRoll[]} rolls     The resulting rolls.
     * @param {object} data
     * @param {Actor5e} data.subject  Item or Activity for which the roll was performed.
     */
    Hooks.callAll("sentius.postRollRecharge", rolls, { subject: this });
