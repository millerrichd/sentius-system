...
export default class DurationField extends SchemaField {
  ...
  static prepareData(rollData, labels) {
    this.duration.scalar = this.duration.units in CONFIG.SENTIUS.scalarTimePeriods;
    if ( this.duration.scalar ) {
      prepareFormulaValue(this, "duration.value", "SENTIUS.DURATION.FIELDS.duration.value.label", rollData);
    } else this.duration.value = null;

    if ( labels && this.duration.units ) {
      let duration = CONFIG.SENTIUS.timePeriods[this.duration.units] ?? "";
      ...
      labels.concentrationDuration = this.properties?.has("concentration")
        ? game.i18n.format("SENTIUS.ConcentrationDuration", { duration }) : duration;
    }
