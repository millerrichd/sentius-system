...
export default class RangeField extends SchemaField {
  ...
  static prepareData(rollData, labels) {
    this.range.scalar = this.range.units in CONFIG.SENTIUS.movementUnits;
    if ( this.range.scalar ) {
      prepareFormulaValue(this, "range.value", "SENTIUS.RANGE.FIELDS.range.value.label", rollData);
    ...
    if ( labels && this.range.units ) {
      if ( this.range.scalar && this.range.value ) {
        ...
        labels.range = CONFIG.SENTIUS.distanceUnits[this.range.units];
      }
    } else if ( labels ) labels.range = game.i18n.localize("SENTIUS.DistSelf");
  }
}
