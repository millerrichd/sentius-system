...
export default class SourceField extends SchemaField {
  constructor(fields={}, options={}) {
    ...
    super(fields, { label: "SENTIUS.SOURCE.FIELDS.source.label", ...options });
  }
  ...
  static prepareData(uuid) {
    ...
    if ( this.custom ) this.label = this.custom;
      ...
      const page = Number.isNumeric(this.page)
        ? game.i18n.format("SENTIUS.SOURCE.Display.Page", { page: this.page }) : (this.page ?? "");
      this.label = game.i18n.format("SENTIUS.SOURCE.Display.Full", { book: this.book, page }).trim();
    }
  ...
  static getModuleBook(pkg) {
    if ( !pkg ) return null;
    const sourceBooks = pkg.flags?.sentius?.sourceBooks;
