...
export default class SystemDataModel extends foundry.abstract.TypeDataModel {
   ...
   * @property {typeof DataModel} [systemFlagsModel]  Model that represents flags data within the sentius namespace.
   */
  ...
  static defineSchema() {
    ...
    for ( const template of this._schemaTemplates ) {
      if ( !template.defineSchema ) {
        throw new Error(`Invalid sentius template mixin ${template} defined on class ${this.constructor}`);
      }
...
export class ItemDataModel extends SystemDataModel {
   ...
   * @type {string}
   */
  static ITEM_TOOLTIP_TEMPLATE = "systems/sentius/templates/items/parts/item-tooltip.hbs";
  ...
  prepareBaseData() {
    if ( this.parent.isEmbedded && this.parent.actor?.items.has(this.parent.id) ) {
      const sourceId = this.parent.flags.sentius?.sourceId ?? this.parent._stats.compendiumSource
  ...
  async richTooltip(enrichmentOptions={}) {
    return {
      ...
      classes: ["sentius2", "sentius-tooltip", "item-tooltip"]
    };
  }
  ...
  async getCardData({ activity, ...enrichmentOptions }={}) {
    ...
    const context = {
      ...
      controlHints: game.settings.get("sentius", "controlHints"),
      ...
      description: {
        ...
        concealed: game.user.isGM && game.settings.get("sentius", "concealItemDescriptions") && !description.chat
      }
    };
  ...
  async getCraftCost({ baseItem="craft" }={}) {
    ...
    if ( !this.properties.has("mgc") || !rarity ) {
      const { mundane } = CONFIG.SENTIUS.crafting;
    ...
    const { magic } = CONFIG.SENTIUS.crafting;
