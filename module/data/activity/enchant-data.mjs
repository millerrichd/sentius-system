...
export default class EnchantActivityData extends BaseActivityData {
  ...
  get appliedEnchantments() {
    return sentius.registry.enchantments.applied(this.uuid);
  }
  ...
  static transformEffectsData(source, options) {
    ...
    for ( const effect of source.effects ) {
      if ( (effect.type !== "enchantment") && (effect.flags?.sentius?.type !== "enchantment") ) continue;
      effects.push({ _id: effect._id, ...(effect.flags?.sentius?.enchantment ?? {}) });
      delete effect.flags?.sentius?.enchantment;
    }
    return effects;
  }
