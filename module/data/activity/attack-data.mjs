...
export default class AttackActivityData extends BaseActivityData {
  ...
  get ability() {
    ...
    if ( this.attack.ability in CONFIG.SENTIUS.abilities ) return this.attack.ability;
  ...
  getActionLabel(attackMode) {
    ...
    if ( attackMode ) {
      ...
      attackModeLabel = game.i18n.localize(`SENTIUS.ATTACK.Mode.${key}`);
    }
    let actionType = this.actionType;
    if ( (actionType === "mwak") && (attackMode?.startsWith("thrown")) ) actionType = "rwak";
    let actionTypeLabel = game.i18n.localize(`SENTIUS.Action${actionType.toUpperCase()}`);
    const isLegacy = game.settings.get("sentius", "rulesVersion") === "legacy";
    const isUnarmed = this.attack.type.classification === "unarmed";
    if ( isUnarmed ) attackModeLabel = game.i18n.localize("SENTIUS.ATTACK.Classification.Unarmed");
    const isSpell = (actionType === "rsak") || (actionType === "msak");
    if ( isLegacy || isSpell ) return [actionTypeLabel, attackModeLabel].filterJoin(" &bull; ");
    actionTypeLabel = game.i18n.localize(`SENTIUS.ATTACK.Attack.${actionType}`);
    if ( isUnarmed ) return [actionTypeLabel, attackModeLabel].filterJoin(" &bull; ");
    const weaponType = CONFIG.SENTIUS.weaponTypeMap[this.item.system.type?.value];
    const weaponTypeLabel = weaponType
      ? game.i18n.localize(`SENTIUS.ATTACK.Weapon.${weaponType.capitalize()}`)
      : CONFIG.SENTIUS.weaponTypes[this.item.system.type?.value];
  ...
  getDamageConfig(config={}) {
    ...
    if ( ammo ) {
      const properties = Array.from(ammo.properties).filter(p => CONFIG.SENTIUS.itemProperties[p]?.isPhysical);
  ...
  getRangeLabel() {
    ...
    // TODO: Use proper unit formatting with these once https://github.com/foundryvtt/sentius/issues/3958 is resolved
    ...
    if ( this.validAttackTypes.has("melee") ) {
      ...
      parts.push(game.i18n.format("SENTIUS.RANGE.Formatted.Reach", { reach: `${reach ?? 5} ${units}` }));
    }
    ...
    if ( this.validAttackTypes.has("ranged") ) {
      ...
      parts.push(game.i18n.format("SENTIUS.RANGE.Formatted.Range", { range }));
    }
  ...
  _processDamagePart(damage, rollConfig, rollData, index=0) {
    ...
    const criticalBonusDice = this.actor?.getFlag("sentius", "meleeCriticalDamageDice") ?? 0;
