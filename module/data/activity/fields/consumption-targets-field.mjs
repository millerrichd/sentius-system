...
export class ConsumptionTargetData extends foundry.abstract.DataModel {
   ...
   */
  get validTargets() {
    const config = CONFIG.SENTIUS.activityConsumptionTypes[this.type];
  ...
  async consume(config, updates) {
    const typeConfig = CONFIG.SENTIUS.activityConsumptionTypes[this.type];
  ...
  static async consumeActivityUses(config, updates) {
    const result = await this._usesConsumption(config, {
      ...
      type: game.i18n.format("SENTIUS.CONSUMPTION.Type.ActivityUses.Warning", {
  ...
  static async consumeAttribute(config, updates) {
    ...
    if ( !foundry.utils.hasProperty(this.actor, keyPath) ) throw new ConsumptionError(
      game.i18n.format("SENTIUS.CONSUMPTION.Warning.MissingAttribute", {
    ...
    let warningMessage;
    if ( (cost > 0) && !current ) warningMessage = "SENTIUS.CONSUMPTION.Warning.None";
    else if ( current < cost ) warningMessage = "SENTIUS.CONSUMPTION.Warning.NotEnough";
    if ( warningMessage ) throw new ConsumptionError(game.i18n.format(warningMessage, {
      ...
      type: game.i18n.format("SENTIUS.CONSUMPTION.Type.Attribute.Warning", { attribute: this.target })
    }));
  ...
  static async consumeHitDice(config, updates) {
    ...
    let warningMessage;
    if ( !validClasses.length ) warningMessage = "SENTIUS.CONSUMPTION.Warning.MissingHitDice";
    else if ( (cost > 0) && !total ) warningMessage = "SENTIUS.CONSUMPTION.Warning.None";
    else if ( total < cost ) warningMessage = "SENTIUS.CONSUMPTION.Warning.NotEnough";
    if ( warningMessage ) {
      ...
      throw new ConsumptionError(game.i18n.format(warningMessage, {
        ...
        type: game.i18n.format("SENTIUS.CONSUMPTION.Type.HitDice.Warning", { denomination })
      }));
    }
  ...
  static async consumeItemUses(config, updates) {
    ...
    if ( !item ) throw new ConsumptionError(game.i18n.format("SENTIUS.CONSUMPTION.Warning.MissingItem", {
      activity: this.activity.name, item: this.item.name
    }));

    const result = await this._usesConsumption(config, {
      uses: item.system.uses,
      type: game.i18n.format("SENTIUS.CONSUMPTION.Type.ItemUses.Warning", { name: this.item.name }),
      rolls: updates.rolls
    });
    if ( !result ) return;
  ...
  static async consumeMaterial(config, updates) {
    ...
    let warningMessage;
    if ( cost > 0 && !item.system.quantity ) warningMessage = "SENTIUS.CONSUMPTION.Warning.None";
    else if ( cost > item.system.quantity ) warningMessage = "SENTIUS.CONSUMPTION.Warning.NotEnough";
    if ( warningMessage ) throw new ConsumptionError(game.i18n.format(warningMessage, {
      available: formatNumber(item.system.quantity), cost: formatNumber(cost),
      type: game.i18n.format("SENTIUS.CONSUMPTION.Type.Material.Warning", { name: item.name })
    }));
  ...
  static async consumeSpellSlots(config, updates) {
    ...
    const levelNumber = Math.clamp(
      this.resolveLevel({ config, rolls: updates.rolls }), 1, Object.keys(CONFIG.SENTIUS.spellLevels).length - 1
    );
    ...
    let warningMessage;
    if ( !levelData?.max ) warningMessage = "SENTIUS.CONSUMPTION.Warning.MissingSpellSlot";
    else if ( (cost > 0) && !levelData.value ) warningMessage = "SENTIUS.CONSUMPTION.Warning.None";
    else if ( newValue < 0 ) warningMessage = "SENTIUS.CONSUMPTION.Warning.NotEnough";
    if ( warningMessage ) {
      const level = CONFIG.SENTIUS.spellLevels[levelNumber];
      const type = game.i18n.format("SENTIUS.CONSUMPTION.Type.SpellSlots.Warning", { level });
      throw new ConsumptionError(game.i18n.format(warningMessage, {
  ...
  async _usesConsumption(config, { uses, type, rolls }) {
    ...
    let warningMessage;
    if ( cost > 0 && !uses.value ) warningMessage = "SENTIUS.CONSUMPTION.Warning.None";
    else if ( cost > uses.value ) warningMessage = "SENTIUS.CONSUMPTION.Warning.NotEnough";
  ...
  static consumptionLabelsActivityUses(config, { consumed }={}) {
    ...
    return {
      label: game.i18n.localize(`SENTIUS.CONSUMPTION.Type.ActivityUses.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `SENTIUS.CONSUMPTION.Type.ActivityUses.PromptHint${increaseKey}`,
        {
          cost,
          use: game.i18n.localize(`SENTIUS.CONSUMPTION.Type.Use.${pluralRule}`),
          available: formatNumber(uses.value),
          availableUse: game.i18n.localize(`SENTIUS.CONSUMPTION.Type.Use.${usesPluralRule}`)
        }
      ),
  ...
  static consumptionLabelsAttribute(config, { consumed }={}) {
    ...
    return {
      label: game.i18n.localize(`SENTIUS.CONSUMPTION.Type.Attribute.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `SENTIUS.CONSUMPTION.Type.Attribute.PromptHint${increaseKey}`,
  ...
  static consumptionLabelsHitDice(config, { consumed }={}) {
    const { cost, simplifiedCost, increaseKey, pluralRule } = this._resolveHintCost(config);
    let denomination;
    if ( this.target === "smallest" ) denomination = game.i18n.localize("SENTIUS.ConsumeHitDiceSmallest");
    else if ( this.target === "largest" ) denomination = game.i18n.localize("SENTIUS.ConsumeHitDiceLargest");
    ...
    return {
      label: game.i18n.localize(`SENTIUS.CONSUMPTION.Type.HitDice.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `SENTIUS.CONSUMPTION.Type.HitDice.PromptHint${increaseKey}`,
        {
          cost, denomination: denomination.toLowerCase(),
          die: game.i18n.localize(`SENTIUS.CONSUMPTION.Type.HitDie.${pluralRule}`),
  ...
  static consumptionLabelsItemUses(config, { consumed }={}) {
    ...
    const itemName = item ? item.name : game.i18n.localize("SENTIUS.CONSUMPTION.Target.ThisItem").toLowerCase();
    ...
    if ( simplifiedCost > uses.value ) warn = true;
      ...
      type: "warn",
      message: game.i18n.format("SENTIUS.CONSUMPTION.Warning.WillDestroy", { item: itemName })
    });

    return {
      label: game.i18n.localize(`SENTIUS.CONSUMPTION.Type.ItemUses.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `SENTIUS.CONSUMPTION.Type.ItemUses.PromptHint${increaseKey}`,
        {
          cost,
          use: game.i18n.localize(`SENTIUS.CONSUMPTION.Type.Use.${pluralRule}`),
          available: formatNumber(uses.value),
  ...
  static consumptionLabelsMaterial(config, { consumed }={}) {
    ...
    return {
      label: game.i18n.localize(`SENTIUS.CONSUMPTION.Type.Material.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `SENTIUS.CONSUMPTION.Type.Material.PromptHint${increaseKey}`,
        {
          cost,
          item: item ? `<em>${item.name}</em>` : game.i18n.localize("SENTIUS.CONSUMPTION.Target.ThisItem").toLowerCase(),
  ...
  static consumptionLabelsSpellSlots(config, { consumed }={}) {
    const { cost, simplifiedCost, increaseKey, pluralRule } = this._resolveHintCost(config);
    const levelNumber = Math.clamp(this.resolveLevel({ config }), 1, Object.keys(CONFIG.SENTIUS.spellLevels).length - 1);
    const level = CONFIG.SENTIUS.spellLevels[levelNumber].toLowerCase();
    const available = this.actor.system.spells?.[`spell${levelNumber}`]?.value ?? 0;
    return {
      label: game.i18n.localize(`SENTIUS.CONSUMPTION.Type.SpellSlots.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `SENTIUS.CONSUMPTION.Type.SpellSlots.PromptHint${increaseKey}`,
        {
          cost,
          slot: game.i18n.format(`SENTIUS.CONSUMPTION.Type.SpellSlot.${pluralRule}`, { level }),
          available: formatNumber(available)
        }
      ),
      warn: simplifiedCost > available
  ...
  static validAttributeTargets() {
    ...
    return TokenDocument.implementation.getConsumedAttributes(this.actor.type).map(attr => {
      let group;
      if ( attr.startsWith("abilities.") ) group = game.i18n.localize("SENTIUS.AbilityScorePl");
      else if ( attr.startsWith("currency.") ) group = game.i18n.localize("SENTIUS.Currency");
      else if ( attr.startsWith("spells.") ) group = game.i18n.localize("SENTIUS.CONSUMPTION.Type.SpellSlots.Label");
      else if ( attr.startsWith("attributes.movement.") ) group = game.i18n.localize("SENTIUS.Speed");
      else if ( attr.startsWith("attributes.senses.") ) group = game.i18n.localize("SENTIUS.Senses");
      else if ( attr.startsWith("resources.") ) group = game.i18n.localize("SENTIUS.Resources");
  ...
  static validHitDiceTargets() {
    return [
      { value: "smallest", label: game.i18n.localize("SENTIUS.ConsumeHitDiceSmallest") },
      ...CONFIG.SENTIUS.hitDieTypes.map(d => ({ value: d, label: d })),
      { value: "largest", label: game.i18n.localize("SENTIUS.ConsumeHitDiceLargest") }
    ];
  }
  ...
  static validItemUsesTargets() {
    const makeLabel = (name, item) => {
      ...
      if ( uses.max && (uses.recovery?.length === 1) && (uses.recovery[0].type === "recoverAll")
        ...
        const per = CONFIG.SENTIUS.limitedUsePeriods[uses.recovery[0].period]?.abbreviation;
        label = game.i18n.format("SENTIUS.AbilityUseConsumableLabel", { max: uses.max, per });
      }
      else label = game.i18n.format("SENTIUS.AbilityUseChargesLabel", { value: uses.value });
      return `${name} (${label})`;
    };
    return [
      { value: "", label: makeLabel(game.i18n.localize("SENTIUS.CONSUMPTION.Target.ThisItem"), this.item) },
      { rule: true },
      ...(this.actor?.items ?? [])
  ...
  static validSpellSlotsTargets() {
    return Object.entries(CONFIG.SENTIUS.spellLevels).reduce((arr, [value, label]) => {
