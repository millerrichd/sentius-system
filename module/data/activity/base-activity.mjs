...
export default class BaseActivityData extends foundry.abstract.DataModel {
  ...
  static LOCALIZATION_PREFIXES = [
    "SENTIUS.ACTIVITY", "SENTIUS.ACTIVATION", "SENTIUS.CONSUMPTION",
    "SENTIUS.DURATION", "SENTIUS.RANGE", "SENTIUS.TARGET", "SENTIUS.USES"
  ];
  ...
  get canScale() {
    return this.consumption.scaling.allowed || (this.isSpell && this.item.system.level > 0
      && CONFIG.SENTIUS.spellPreparationModes[this.item.system.preparation.mode]?.upcast);
  }
   ...
   */
  get isScaledScroll() {
    return !!this.item.getFlag("sentius", "spellLevel");
  }
   ...
   * @type {string}
   */
  static INITIAL_ID = staticID("sentiusactivity");
  ...
  static createInitialActivity(source, { offset=0, ...options }={}) {
    ...
    foundry.utils.setProperty(source, "flags.sentius.persistSourceMigration", true);
  }
  ...
  static transformDamagePartData(source, [formula, type]) {
    ...
    if ( parsed && CONFIG.SENTIUS.dieSteps.includes(Number(parsed[2])) ) {
  ...
  static transformEffectsData(source, options) {
    return source.effects
      .filter(e => !e.transfer && (e.type !== "enchantment") && (e.flags?.sentius?.type !== "enchantment"))
      .map(e => ({ _id: e._id }));
  }
  ...
  static transformTargetData(source, options) {
    ...
    if ( source.system.target?.type in CONFIG.SENTIUS.areaTargetTypes ) foundry.utils.mergeObject(data, {
      template: {
  ...
  prepareFinalData(rollData) {
    ...
    for ( const target of this.consumption.targets ) {
      ...
      if ( !actor.items.get(target.target) ) {
        const message = game.i18n.format("SENTIUS.CONSUMPTION.Warning.MissingItem", {
      ...
      if ( part.types.size ) {
        label = `${formula} ${game.i18n.getListFormatter({ type: "conjunction" }).format(
          Array.from(part.types)
            .map(p => CONFIG.SENTIUS.damageTypes[p]?.label ?? CONFIG.SENTIUS.healingTypes[p]?.label)
            .filter(t => t)
        )}`;
      }
  ...
  _processDamagePart(damage, rollConfig, rollData, index=0) {
    ...
    const lastType = this.item.getFlag("sentius", `last.${this.id}.damageType.${index}`);
    ...
    return {
      ...
      options: {
        ...
        properties: Array.from(this.item.system.properties ?? [])
          .filter(p => CONFIG.SENTIUS.itemProperties[p]?.isPhysical)
      }
    };
  }
