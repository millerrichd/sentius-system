...
export default class CheckActivityData extends BaseActivityData {
  ...
  /** @override */
  get ability() {
    if ( this.check.dc.calculation in CONFIG.SENTIUS.abilities ) return this.check.dc.calculation;
  ...
  static transformTypeData(source, activityData, options) {
    return foundry.utils.mergeObject(activityData, {
      check: {
        ability: source.system.ability ?? Object.keys(CONFIG.SENTIUS.abilities)[0]
      }
    });
  }
  ...
  getAbility(associated) {
    ...
    if ( associated in CONFIG.SENTIUS.skills ) return CONFIG.SENTIUS.skills[associated]?.ability ?? null;
    else if ( associated in CONFIG.SENTIUS.tools ) {
      if ( (this.item.type === "tool") && this.item.system.ability ) return this.item.system.ability;
      return CONFIG.SENTIUS.tools[associated]?.ability ?? null;
    }
    return null;
  }
}
