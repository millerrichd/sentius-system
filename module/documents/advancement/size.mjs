...
export default class SizeAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      typeIcon: "systems/sentius/icons/svg/size.svg",
      title: game.i18n.localize("SENTIUS.AdvancementSizeTitle"),
      hint: game.i18n.localize("SENTIUS.AdvancementSizeHint"),
      apps: {
        config: SizeConfig,
        flow: SizeFlow
      }
  ...
  get automaticHint() {
    ...
    if ( this.configuration.sizes.size === 1 ) return game.i18n.format("SENTIUS.AdvancementSizeFlowHintSingle", {
      size: CONFIG.SENTIUS.actorSizes[this.configuration.sizes.first()].label
    });

    const listFormatter = new Intl.ListFormat(game.i18n.lang, { type: "disjunction" });
    return game.i18n.format("SENTIUS.AdvancementSizeflowHintMultiple", {
      sizes: listFormatter.format(this.configuration.sizes.map(s => CONFIG.SENTIUS.actorSizes[s].label))
    });
  }
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    return sizes.map(s => `<span class="tag">${CONFIG.SENTIUS.actorSizes[s].label}</span>`).join("");
  }
