...
export default class ItemGrantAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      typeIcon: "systems/sentius/icons/svg/item-grant.svg",
      title: game.i18n.localize("SENTIUS.AdvancementItemGrantTitle"),
      hint: game.i18n.localize("SENTIUS.AdvancementItemGrantHint"),
      apps: {
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    if ( !this.value.added || configMode ) return this.configuration.items.filter(i => fromUuidSync(i.uuid))
      .reduce((html, i) => html + sentius.utils.linkForUuid(i.uuid), "");
  ...
  restore(level, data) {
    ...
    for ( const item of data.items ) {
      ...
      updates[item._id] = item.flags.sentius.sourceId;
    }
    this.updateSource({
      "value.ability": data.ability,
      [this.storagePath(level)]: updates
  ...
  _validateItemType(item, { strict=true }={}) {
    ...
    if ( strict ) throw new Error(game.i18n.format("SENTIUS.AdvancementItemTypeInvalidWarning", {type}));
    return false;
  }
}
