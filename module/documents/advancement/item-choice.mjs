...
export default class ItemChoiceAdvancement extends ItemGrantAdvancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      typeIcon: "systems/sentius/icons/svg/item-choice.svg",
      title: game.i18n.localize("SENTIUS.AdvancementItemChoiceTitle"),
      hint: game.i18n.localize("SENTIUS.AdvancementItemChoiceHint"),
      multiLevel: true,
      apps: {
  ...
  titleForLevel(level, { configMode=false }={}) {
    ...
    let tag;
    if ( data.count ) tag = game.i18n.format("SENTIUS.AdvancementItemChoiceChoose", { count: data.count });
    else if ( data.replacement ) tag = game.i18n.localize("SENTIUS.AdvancementItemChoiceReplacementTitle");
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    return Object.values(items).reduce((html, uuid) => html + game.sentius.utils.linkForUuid(uuid), "");
  }
  ...
  restore(level, data) {
    ...
    if ( data.replaced ) {
      if ( !original ) {
        throw new ItemChoiceAdvancement.ERROR(game.i18n.localize("SENTIUS.AdvancementItemChoiceNoOriginalError"));
      }
  ...
  _validateItemType(item, { type, restriction, strict=true }={}) {
    ...
    if ( type && (type !== item.type) ) {
      ...
      if ( strict ) throw new Error(game.i18n.format("SENTIUS.AdvancementItemChoiceTypeWarning", {type: typeLabel}));
      return false;
    }

    // If additional type restrictions applied, make sure they are valid
    if ( (type === "feat") && restriction.type ) {
      const typeConfig = CONFIG.SENTIUS.featureTypes[restriction.type];
      ...
      if ( errorLabel ) {
        if ( strict ) throw new Error(game.i18n.format("SENTIUS.AdvancementItemChoiceTypeWarning", {type: errorLabel}));
        return false;
      }
    }
    ...
    if ( (type === "spell") && !Number.isNaN(l) && (item.system.level !== l) ) {
      const level = CONFIG.SENTIUS.spellLevels[l];
      if ( strict ) throw new Error(game.i18n.format("SENTIUS.AdvancementItemChoiceSpellLevelSpecificWarning", {level}));
      return false;
    }
