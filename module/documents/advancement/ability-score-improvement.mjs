...
export default class AbilityScoreImprovementAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      typeIcon: "systems/sentius/icons/svg/ability-score-improvement.svg",
      title: game.i18n.localize("SENTIUS.ADVANCEMENT.AbilityScoreImprovement.Title"),
      hint: game.i18n.localize("SENTIUS.ADVANCEMENT.AbilityScoreImprovement.Hint"),
      apps: {
   ...
   */
  get allowFeat() {
    return (this.item.type === "class") && game.settings.get("sentius", "allowFeats");
  }
   ...
   */
  canImprove(ability) {
    return CONFIG.SENTIUS.abilities[ability]?.improvement !== false;
  }
  ...
  titleForLevel(level, { configMode=false }={}) {
    ...
    return game.i18n.localize("SENTIUS.Feature.Feat");
  }
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    if ( configMode ) {
      const entries = Object.entries(this.configuration.fixed).map(([key, value]) => {
        ...
        const name = CONFIG.SENTIUS.abilities[key]?.label ?? key;
      ...
      if ( this.configuration.points ) entries.push(`<span class="tag">${
        game.i18n.localize("SENTIUS.ADVANCEMENT.AbilityScoreImprovement.FIELDS.points.label")}: <strong>${
  ...
  async apply(level, data) {
    if ( data.type === "asi" ) {
      const assignments = Object.keys(CONFIG.SENTIUS.abilities).reduce((obj, key) => {
