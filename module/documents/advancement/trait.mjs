...
export default class TraitAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      typeIcon: "systems/sentius/icons/svg/trait.svg",
      title: game.i18n.localize("SENTIUS.AdvancementTraitTitle"),
      hint: game.i18n.localize("SENTIUS.AdvancementTraitHint"),
      apps: {
        config: TraitConfig,
        flow: TraitFlow
      }
  ...
  prepareData() {
    ...
    const traitConfig = rep.size === 1 ? CONFIG.SENTIUS.traits[rep.first()] : null;
  ...
  sortingValueForLevel(levels) {
    const traitOrder = Object.keys(CONFIG.SENTIUS.traits).findIndex(k => k === this.representedTraits().first());
    const modeOrder = Object.keys(CONFIG.SENTIUS.traitModes).findIndex(k => k === this.configuration.mode);
  ...
  async actorSelected() {
    ...
    const traitTypes = this.configuration.mode === "default" ? Object.keys(CONFIG.SENTIUS.traits).filter(k => k !== "dm")
      : filteredKeys(CONFIG.SENTIUS.traits, t => t[this.configuration.mode === "mastery" ? "mastery" : "expertise"]);
  ...
  async availableChoices(chosen) {
    ...
    if ( this.configuration.allowReplacements && (unfilteredLength > available.length) ) {
      ...
      if ( rep.size === 1 ) return {
        ...
        label: game.i18n.format("SENTIUS.AdvancementTraitChoicesRemaining", {
