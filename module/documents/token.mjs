...
export default class TokenDocument5e extends SystemFlagsMixin(TokenDocument) {
  ...
  static getConsumedAttributes(data) {
    return CONFIG.SENTIUS.consumableResources;
  }
  ...
  static getTrackedAttributeChoices(attributes) {
    ...
    if ( abilities.length ) groups[game.i18n.localize("SENTIUS.AbilityScorePl")] = abilities;
    if ( movement.length ) groups[game.i18n.localize("SENTIUS.MovementSpeeds")] = movement;
    if ( senses.length ) groups[game.i18n.localize("SENTIUS.Senses")] = senses;
    if ( skills.length ) groups[game.i18n.localize("SENTIUS.SkillPassives")] = skills;
    if ( slots.length ) groups[game.i18n.localize("JOURNALENTRYPAGE.SENTIUS.Class.SpellSlots")] = slots;
    return groups;
  }
  ...
  prepareData() {
    ...
    if ( !size ) return;
    const dts = CONFIG.SENTIUS.actorSizes[size].dynamicTokenScale ?? 1;
  ...
  getRingColors() {
    ...
    if ( this.hasStatusEffect(CONFIG.specialStatusEffects.DEFEATED) ) {
      colors.ring = CONFIG.SENTIUS.tokenRingColors.defeated;
    }
    return colors;
  }
  ...
  flashRing(type) {
    ...
    const color = CONFIG.SENTIUS.tokenRingColors[type];
    if ( !color ) return;
    const options = {};
    if ( type === "damage" ) {
  ...
  _onDelete(options, userId) {
    ...
    const origin = this.actor?.getFlag("sentius", "summon.origin");
    // TODO: Replace with parseUuid once V11 support is dropped
    if ( origin ) sentius.registry.summons.untrack(origin.split(".Item.")[0], this.actor.uuid);
  }
}
