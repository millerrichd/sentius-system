...
export default class Item5e extends SystemDocumentMixin(Item) {
  ...
  static compendiumBrowserTypes({ chosen=new Set() }={}) {
    ...
    choices.physical = {
      label: game.i18n.localize("SENTIUS.Item.Category.Physical"),
  ...
  _initializeSource(data, options={}) {
    ...
    if ( data.type === "backpack" ) {
      data.type = "container";
      foundry.utils.setProperty(data, "flags.sentius.persistSourceMigration", true);
    }

    /**
     * A hook event that fires before source data is initialized for an Item in a compendium.
     * @function sentius.initializeItemSource
     * @memberof hookEvents
     ...
     */
    if ( options.pack || options.parent?.pack ) Hooks.callAll("sentius.initializeItemSource", this, data, options);
   ...
   */
  get canDelete() {
    return !this.flags.sentius?.cachedFor;
  }
   ...
   * @deprecated since SENTIUS, targeted for removal in SENTIUS
   */
  get hasAbilityCheck() {
   ...
   */
  get scalingIncrease() {
    return this.system?.scalingIncrease ?? this.getFlag("sentius", "scaling") ?? 0;
  }
  ...
  get usageScaling() {
    ...
    if ( isLeveled && CONFIG.SENTIUS.spellPreparationModes[preparation.mode]?.upcast ) return "slot";
   ...
   * @property {string} type              Spellcasting type as defined in ``CONFIG.SENTIUS.spellcastingTypes`.
  ...
  *allApplicableEffects() {
  ...
  prepareDerivedData() {
    ...
    if ( this.system.properties ) {
      this.labels.properties = this.system.properties.reduce((acc, prop) => {
        ...
        acc.push({
          abbr: prop,
          label: CONFIG.SENTIUS.itemProperties[prop]?.label,
          icon: CONFIG.SENTIUS.itemProperties[prop]?.icon
        });
        return acc;
      }, []);
    }
  ...
  _prepareAdvancement() {
    ...
    this.advancement = {
      ...
      byLevel: Object.fromEntries(
        Array.fromRange(CONFIG.SENTIUS.maxLevel + 1).slice(minAdvancementLevel).map(l => [l, []])
      ),
      byType: {},
  ...
  async use(config={}, dialog={}, message={}) {
    ...
    const activities = this.system.activities?.filter(a =>
      !this.getFlag("sentius", "riders.activity")?.includes(a.id) && a.canUse
    );
  ...
  async displayCard(message={}) {
    ...
    const messageConfig = foundry.utils.mergeObject({
      ...
      data: {
        content: await renderTemplate("systems/sentius/templates/chat/item-card.hbs", context),
        flags: {
          "core.canPopout": true,
          "sentius.item": { id: this.id, uuid: this.uuid, type: this.type }
        },
     ...
     * @function sentius.preDisplayCardV2
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("sentius.preDisplayCardV2", this, messageConfig) === false ) return;

    if ( "sentius.preDisplayCard" in Hooks.events ) {
      foundry.utils.logCompatibilityWarning(
        "The `sentius.preDisplayCard` hook has been deprecated and replaced with `sentius.preDisplayCardV2`.",
        { since: "SENTIUS", until: "SENTIUS"" }
      );
      const hookData = { createMessage: messageConfig.create };
      Hooks.callAll("sentius.preDisplayCard", this, messageConfig.data, hookData);
     ...
     */
    Hooks.callAll("sentius.displayCard", this, card);
  ...
  async rollAttack({ spellLevel, ...options }={}) {
    ...
    if ( spellLevel && (this.type === "spell") ) {
      item = item.clone({ "flags.sentius.scaling": Math.max(0, spellLevel - item.system.level) }, { keepId: true });
    }
  ...
  async createActivity(type, data={}, { renderSheet=true }={}) {
    ...
    const config = CONFIG.SENTIUS.activityTypes[type];
    if ( !config ) throw new Error(`${type} not found in CONFIG.SENTIUS.activityTypes`);
  ...
  createAdvancement(type, data={}, { showConfig=true, source=false }={}) {
    ...
    const config = CONFIG.SENTIUS.advancementTypes[type];
    if ( !config ) throw new Error(`${type} not found in CONFIG.SENTIUS.advancementTypes`);
    const cls = config.documentClass;
  ...
  async deleteDialog(options={}) {
    ...
    if ( this.actor?.system.metadata?.supportsAdvancement && !game.settings.get("sentius", "disableAdvancements") ) {
    ...
    if ( count ) {
      return Dialog.confirm({
        title: `${game.i18n.format("DOCUMENT.Delete", {type: game.i18n.localize("SENTIUS.Container")})}: ${this.name}`,
        content: `<h4>${game.i18n.localize("AreYouSure")}</h4>
          <p>${game.i18n.format("SENTIUS.ContainerDeleteMessage", {count})}</p>
          <label>
            <input type="checkbox" name="deleteContents">
            ${game.i18n.localize("SENTIUS.ContainerDeleteContents")}
          </label>`,
        yes: html => {
  ...
  static addCompendiumContextOptions(html, entryOptions) {
    ...
    entryOptions.push({
      name: "SENTIUS.Scroll.CreateScroll",
  ...
  static async createWithContents(items, { container, keepId=false, transformAll, transformFirst }={}) {
    ...
    if ( container ) {
      ...
      if ( depth > PhysicalItemTemplate.MAX_DEPTH ) {
        ui.notifications.warn(game.i18n.format("SENTIUS.ContainerMaxDepth", { depth: PhysicalItemTemplate.MAX_DEPTH }));
        return;
      }
    }
  ...
  static async createScrollFromSpell(spell, options={}, config={}) {
    const values = {};
    if ( (spell instanceof Item5e) && spell.isOwned && (game.settings.get("sentius", "rulesVersion") === "modern") ) {
    ...
    config = foundry.utils.mergeObject({
      explanation: game.user.getFlag("sentius", "creation.scrollExplanation") ?? "reference",
    ...
    if ( config.dialog !== false ) {
      ...
      await game.user.setFlag("sentius", "creation.scrollExplanation", config.explanation);
    }
    ...
    if ( Number.isNumeric(config.level) ) {
      flags.sentius ??= {};
      flags.sentius.scaling = Math.max(0, config.level - spell.system.level);
      flags.sentius.spellLevel = {
        value: config.level,
     ...
     * @function sentius.preCreateScrollFromSpell
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("sentius.preCreateScrollFromSpell", itemData, options, config) === false ) return;

    let { activities, description, level, properties, source } = itemData.system;

    // Get scroll data
    let scrollUuid;
    const id = CONFIG.SENTIUS.spellScrollIds[level];
    if ( foundry.data.validators.isValidId(id) ) {
      scrollUuid = game.packs.get(CONFIG.SENTIUS.sourcePacks.ITEMS).index.get(id).uuid;
    } else {
      scrollUuid = id;
    }
    ...
    switch ( config.explanation ) {
      case "full":
        ...
        desc = [
          scrollIntro,
          "<hr>",
          `<h3>${itemData.name} (${game.i18n.format("SENTIUS.LevelNumber", {level})})</h3>`,
          isConc ? `<p><em>${game.i18n.localize("SENTIUS.Scroll.RequiresConcentration")}</em></p>` : null,
          "<hr>",
          description.value,
          "<hr>",
          `<h3>${game.i18n.localize("SENTIUS.Scroll.Details")}</h3>`,
          "<hr>",
          scrollDetails
        ].filterJoin("");
        break;
      case "reference":
        desc = [
          "<p><em>",
          CONFIG.SENTIUS.spellLevels[level] ?? level,
          " &Reference[Spell Scroll]",
          isConc ? `, ${game.i18n.localize("SENTIUS.Scroll.RequiresConcentration")}` : null,
          "</em></p>",
          description.value
        ].filterJoin("");
        break;
    ...
    for ( const level of Array.fromRange(itemData.system.level + 1).reverse() ) {
      const values = CONFIG.SENTIUS.spellScrollValues[level];
      if ( values ) {
    ...
    const spellScrollData = foundry.utils.mergeObject(scrollData, {
      name: `${game.i18n.localize("SENTIUS.SpellScroll")}: ${itemData.name}`,
     ...
     * @function sentius.createScrollFromSpell
     * @memberof hookEvents
     * @param {Item5e|object} spell              The spell or item data to be made into a scroll.
     * @param {object} spellScrollData           The final item data used to make the scroll.
     * @param {SpellScrollConfiguration} config  Configuration options for scroll creation.
     */
    Hooks.callAll("sentius.createScrollFromSpell", spell, spellScrollData, config);
  ...
  static async createDialog(data={}, { parent=null, pack=null, types=null, ...options }={}) {
    ...
    const content = await renderTemplate("systems/sentius/templates/apps/document-create.hbs", {
      folders, name, type,
      folder: data.folder,
    ...
    return Dialog.prompt({
      ...
      rejectClose: false,
      options: { ...options, jQuery: false, width: 350, classes: ["sentius2", "create-document", "dialog"] }
    });
  }
  ...
  static getDefaultArtwork(itemData={}) {
    ...
    return { img: CONFIG.SENTIUS.defaultArtwork.Item[type] ?? img };
  }
