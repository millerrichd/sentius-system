 ...
 * @param {string} trait   Trait as defined in `CONFIG.SENTIUS.traits`.
 * @returns {object|void}
 */
...
export function actorKeyPath(trait) {
  const traitConfig = CONFIG.SENTIUS.traits[trait];
 ...
 * @param {string} [trait]  Trait as defined in `CONFIG.SENTIUS.traits`, only needed if key isn't prefixed.
 * @returns {string|void}
 */
...
export async function categories(trait) {
  const traitConfig = CONFIG.SENTIUS.traits[trait];
  const config = foundry.utils.deepClone(CONFIG.SENTIUS[traitConfig.configKey ?? trait]);
  ...
  for ( const key of Object.keys(config) ) {
    ...
    if ( traitConfig.children?.[key] ) {
      ...
      for ( const [childKey, value] of Object.entries(CONFIG.SENTIUS[traitConfig.children[key]]) ) {
  ...
  if ( traitConfig.subtypes ) {
    const map = CONFIG.SENTIUS[`${trait}ProficienciesMap`];

    // Merge all ID lists together
    const ids = traitConfig.subtypes.ids.reduce((obj, key) => {
      foundry.utils.mergeObject(obj, CONFIG.SENTIUS[key] ?? {});
      return obj;
    }, {});
...
export function getBaseItemUUID(identifier) {
  ...
  let pack = CONFIG.SENTIUS.sourcePacks.ITEMS;
...
export function traitIndexFields() {
  ...
  for ( const traitConfig of Object.values(CONFIG.SENTIUS.traits) ) {
...
export function traitLabel(trait, count) {
  ...
  if ( !traitConfig ) return game.i18n.localize(`SENTIUS.TraitGenericPlural.${pluralRule}`);
 ...
 * @param {string} [config.trait]   Trait as defined in `CONFIG.SENTIUS.traits` if not using a prefixed key.
...
export function keyLabel(key, config={}) {
  ...
  const traitData = CONFIG.SENTIUS[traitConfig.configKey ?? trait] ?? {};
  ...
  if ( lastKey === "*" ) {
    ...
    const localization = `SENTIUS.TraitConfigChoose${final ? "Other" : `Any${count ? "Counted" : "Uncounted"}`}`;
    ...
    for ( const childrenKey of Object.values(traitConfig.children ?? {}) ) {
      const childLabel = CONFIG.SENTIUS[childrenKey]?.[lastKey];
      if ( childLabel ) return childLabel;
    }

    // Base item (e.g. "Shortsword")
    for ( const idsKey of traitConfig.subtypes?.ids ?? [] ) {
      const baseItemId = CONFIG.SENTIUS[idsKey]?.[lastKey];
...
export function choiceLabel(choice, { only=false, final=false }={}) {
  ...
  return game.i18n.format("SENTIUS.TraitConfigChooseList", {
    count: choice.count,
...
export function localizedList({ grants=new Set(), choices=[] }) {
  ...
  return game.i18n.format("SENTIUS.TraitConfigChooseWrapper", {
