...
export default class CheckActivity extends ActivityMixin(CheckActivityData) {
  ...
  /** @inheritDoc */
  static LOCALIZATION_PREFIXES = [...super.LOCALIZATION_PREFIXES, "SENTIUS.CHECK"];
  ...
  static metadata = Object.freeze(
    foundry.utils.mergeObject(super.metadata, {
      type: "check",
      img: "systems/sentius/icons/svg/activity/check.svg",
      title: "SENTIUS.CHECK.Title",
  ...
  _usageChatButtons(message) {
    ...
    const createButton = (abilityKey, associated) => {
      const ability = CONFIG.SENTIUS.abilities[abilityKey]?.label;
      const checkType = (associated in CONFIG.SENTIUS.skills) ? "skill"
        : (associated in CONFIG.SENTIUS.toolIds) ? "tool": "ability";
      ...
      let label = ability;
      let type;
      if ( checkType === "skill" ) type = CONFIG.SENTIUS.skills[associated]?.label;
      else if ( checkType === "tool" ) type = Trait.keyLabel(associated, { trait: "tool" });
      if ( type ) label = game.i18n.format("EDITOR.SENTIUS.Inline.SpecificCheck", { ability, type });
      else label = ability;

      buttons.push({
        label: dc ? `
          <span class="visible-dc">${game.i18n.format("EDITOR.SENTIUS.Inline.DC", { dc, check: wrap(label) })}</span>
          <span class="hidden-dc">${wrap(label)}</span>
        ` : wrap(label),
        icon: checkType === "tool" ? '<i class="fa-solid fa-hammer" inert></i>'
          : '<i class="sentius-icon" data-src="systems/sentius/icons/svg/ability-score-improvement.svg" inert></i>',
        dataset
      });
    };
    const wrap = check => game.i18n.format("EDITOR.SENTIUS.Inline.CheckShort", { check });
  ...
  static async #rollCheck(event, target, message) {
    ...
    if ( !targets.length ) ui.notifications.warn("SENTIUS.ActionWarningNoToken", { localize: true });
    let { ability, dc, skill, tool } = target.dataset;
    dc = parseInt(dc);
    const rollData = { event, target: Number.isFinite(dc) ? dc : this.check.dc.value };
    if ( ability in CONFIG.SENTIUS.abilities ) rollData.ability = ability;
