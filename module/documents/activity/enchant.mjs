...
export default class EnchantActivity extends ActivityMixin(EnchantActivityData) {
  ...
  /** @inheritDoc */
  static LOCALIZATION_PREFIXES = [...super.LOCALIZATION_PREFIXES, "SENTIUS.ENCHANT"];
  ...
  static metadata = Object.freeze(
    foundry.utils.mergeObject(super.metadata, {
      type: "enchant",
      img: "systems/sentius/icons/svg/activity/enchant.svg",
      title: "SENTIUS.ENCHANT.Title",
  ...
  static localize() {
    super.localize();
    this._localizeSchema(this.schema.fields.effects.element, ["SENTIUS.ENCHANT.FIELDS.effects"]);
  }
  ...
  async _prepareUsageScaling(usageConfig, messageConfig, item) {
    ...
    if ( usageConfig.enchantmentProfile ) foundry.utils.setProperty(
      messageConfig, "data.flags.sentius.use.enchantmentProfile", usageConfig.enchantmentProfile
    );
  }
  ...
  canEnchant(item) {
    ...
    if ( !this.restrictions.allowMagical && item.system.properties?.has("mgc") ) {
      errors.push(new EnchantmentError(game.i18n.localize("SENTIUS.ENCHANT.Warning.NoMagicalItems")));
    }

    if ( this.restrictions.type && (item.type !== this.restrictions.type) ) {
      errors.push(new EnchantmentError(game.i18n.format("SENTIUS.ENCHANT.Warning.WrongType", {
    ...
    if ( this.restrictions.categories.size && !this.restrictions.categories.has(item.system.type?.value) ) {
      ...
      errors.push(new EnchantmentError(game.i18n.format(
        `SENTIUS.ENCHANT.Warning.${item.system.type?.value ? "WrongType" : "NoSubtype"}`,
        {
    ...
    if ( this.restrictions.properties.size
      ...
      errors.push(new EnchantmentError(game.i18n.format("SENTIUS.Enchantment.Warning.MissingProperty", {
        validProperties: game.i18n.getListFormatter({ type: "disjunction" }).format(
          Array.from(this.restrictions.properties).map(p => CONFIG.SENTIUS.itemProperties[p]?.label ?? p)
        )
      })));
    }
     ...
     */
    Hooks.callAll("sentius.canEnchant", this, item, errors);
