...
export default class OrderActivity extends ActivityMixin(OrderActivityData) {
  ...
  static metadata = Object.freeze(foundry.utils.mergeObject(super.metadata, {
    type: "order",
    img: "systems/sentius/icons/svg/activity/order.svg",
    title: "SENTIUS.FACILITY.Order.Issue",
    usage: {
      actions: {
        pay: OrderActivity.#onPayOrder
      },
      chatCard: "systems/sentius/templates/chat/order-activity-card.hbs",
  ...
  _prepareUsageScaling(usageConfig, messageConfig, item) {
    ...
    messageConfig.data.flags.sentius.order = { costs, craft, trade };
  }
  ...
  _usageChatButtons(message) {
    const { costs } = message.data.flags.sentius.order;
    if ( !costs.gold || costs.paid ) return [];
    return [{
      label: game.i18n.localize("SENTIUS.FACILITY.Costs.Automatic"),
      icon: '<i class="fas fa-coins"></i>',
      dataset: { action: "pay", method: "automatic" }
    }, {
      label: game.i18n.localize("SENTIUS.FACILITY.Costs.Manual"),
  ...
  async _usageChatContext(message) {
    const { costs, craft, trade } = message.data.flags.sentius.order;
    const { type } = this.item.system;
    const supplements = [];
    if ( costs.days ) supplements.push(`
      <strong>${game.i18n.localize("SENTIUS.DurationTime")}</strong>
      ${game.i18n.format("SENTIUS.FACILITY.Costs.Days", { days: costs.days })}
    `);
    if ( costs.gold ) supplements.push(`
      <strong>${game.i18n.localize("SENTIUS.CurrencyGP")}</strong>
      ${formatNumber(costs.gold)}
      (${game.i18n.localize(`SENTIUS.FACILITY.Costs.${costs.paid ? "Paid" : "Unpaid"}`)})
    `);
    if ( craft?.item ) {
    ...
    if ( trade?.stock?.value && trade.sell ) supplements.push(`
      <strong>${game.i18n.localize("SENTIUS.FACILITY.Trade.Sell.Supplement")}</strong>
      ${formatNumber(trade.stock.value)}
      ${CONFIG.SENTIUS.currencies.gp?.abbreviation ?? ""}
    `);
    if ( trade?.creatures ) {
      const creatures = [];
      if ( trade.sell ) {
      ...
      supplements.push(`
        <strong>${game.i18n.localize(`SENTIUS.FACILITY.Trade.${trade.sell ? "Sell" : "Buy"}.Supplement`)}</strong>
        ${game.i18n.getListFormatter({ style: "narrow" }).format(creatures.map(a => a.toAnchor().outerHTML))}
      `);
    }
    const facilityType = game.i18n.localize(`SENTIUS.FACILITY.Types.${type.value.titleCase()}.Label.one`);
    const buttons = this._usageChatButtons(message);
    return {
      supplements,
      buttons: buttons.length ? buttons : null,
      description: game.i18n.format("SENTIUS.FACILITY.Use.Description", {
        order: game.i18n.localize(`SENTIUS.FACILITY.Orders.${this.order}.inf`),
  ...
  static async #onPayOrder(event, target, message) {
    ...
    const order = message.getFlag("sentius", "order");
    const config = foundry.utils.expandObject({ "data.flags.sentius.order": order });
    if ( method === "automatic" ) {
      try {
    ...
    foundry.utils.setProperty(config, "data.flags.sentius.order.costs.paid", true);
