...
export default function ActivityMixin(Base) {
  class Activity extends PseudoDocumentMixin(Base) {
    ...
    static metadata = Object.freeze({
      ...
      usage: {
        actions: {},
        chatCard: "systems/sentius/templates/chat/activity-card.hbs",
    ...
    static localize() {
      ...
      if ( fields.damage?.fields.parts ) {
        this._localizeSchema(fields.damage.fields.parts.element, ["SENTIUS.DAMAGE.FIELDS.damage.parts"]);
      }
      if ( fields.consumption ) {
        this._localizeSchema(fields.consumption.fields.targets.element, [
          "SENTIUS.CONSUMPTION.FIELDS.consumption.targets"
        ]);
      }
      if ( fields.uses ) this._localizeSchema(fields.uses.fields.recovery.element, ["SENTIUS.USES.FIELDS.uses.recovery"]);
    }
     ...
     */
    get damageFlavor() {
      return game.i18n.localize("SENTIUS.DamageRoll");
    }
    ...
    get validConsumptionTypes() {
      const types = new Set(Object.keys(CONFIG.SENTIUS.activityConsumptionTypes));
    ...
    async use(usage={}, dialog={}, message={}) {
      ...
      if ( !this.item.isOwner ) {
        ui.notifications.error("SENTIUS.DocumentUseWarn", { localize: true });
        return;
      }
      if ( !this.canUse ) {
        ui.notifications.error("SENTIUS.ACTIVITY.Warning.UsageNotAllowed", { localize: true });
        return;
      }
       ...
       * @function sentius.preUseActivity
       * @memberof hookEvents
       ...
       */
      if ( Hooks.call("sentius.preUseActivity", activity, usageConfig, dialogConfig, messageConfig) === false ) return;

      if ( "sentius.preUseItem" in Hooks.events ) {
        foundry.utils.logCompatibilityWarning(
          "The `sentius.preUseItem` hook has been deprecated and replaced with `sentius.preUseActivity`.",
          { since: "SENTIUS", until: "SENTIUS"" }
        );
        const { config, options } = this._createDeprecatedConfigs(usageConfig, dialogConfig, messageConfig);
        if ( Hooks.call("sentius.preUseItem", item, config, options) === false ) return;
      ...
      if ( usageConfig.concentration?.begin ) {
        const effect = await item.actor.beginConcentrating(activity, { "flags.sentius.scaling": usageConfig.scaling });
        if ( effect ) {
          results.effects ??= [];
          results.effects.push(effect);
          foundry.utils.setProperty(messageConfig.data, "flags.sentius.use.concentrationId", effect.id);
        }
       ...
       * @function sentius.postUseActivity
       * @memberof hookEvents
       ...
       */
      if ( Hooks.call("sentius.postUseActivity", activity, usageConfig, results) === false ) return results;

      if ( "sentius.useItem" in Hooks.events ) {
        foundry.utils.logCompatibilityWarning(
          "The `sentius.useItem` hook has been deprecated and replaced with `sentius.postUseActivity`.",
          { since: "SENTIUS", until: "SENTIUS"" }
        );
        const { config, options } = this._createDeprecatedConfigs(usageConfig, dialogConfig, messageConfig);
        Hooks.callAll("sentius.itemUsageConsumption", item, config, options, results.templates, results.effects, null);
      }
    ...
    async consume(usageConfig, messageConfig) {
       ...
       * @function sentius.preActivityConsumption
       * @memberof hookEvents
       * @param {Activity} activity                           Activity being activated.
       * @param {ActivityUseConfiguration} usageConfig        Configuration data for the activation.
       * @param {ActivityMessageConfiguration} messageConfig  Configuration info for the created chat message.
       ...
       */
      if ( Hooks.call("sentius.preActivityConsumption", this, usageConfig, messageConfig) === false ) return;

      if ( "sentius.preItemUsageConsumption" in Hooks.events ) {
        foundry.utils.logCompatibilityWarning(
          "The `sentius.preItemUsageConsumption` hook has been deprecated and replaced with `sentius.preActivityConsumption`.",
          { since: "SENTIUS", until: "SENTIUS"" }
        );
        const { config, options } = this._createDeprecatedConfigs(usageConfig, {}, messageConfig);
        if ( Hooks.call("sentius.preItemUsageConsumption", this.item, config, options) === false ) return;
       ...
       * performed.
       * @function sentius.activityConsumption
       * @memberof hookEvents
       * @param {Activity} activity                           Activity being activated.
       * @param {ActivityUseConfiguration} usageConfig        Configuration data for the activation.
       * @param {ActivityMessageConfiguration} messageConfig  Configuration info for the created chat message.
       ...
       * @returns {boolean}  Explicitly return `false` to prevent activity from being activated.
       */
      if ( Hooks.call("sentius.activityConsumption", this, usageConfig, messageConfig, updates) === false ) return;

      if ( "sentius.itemUsageConsumption" in Hooks.events ) {
        foundry.utils.logCompatibilityWarning(
          "The `sentius.itemUsageConsumption` hook has been deprecated and replaced with `sentius.activityConsumption`.",
          { since: "SENTIUS", until: "SENTIUS"" }
        );
        const { config, options } = this._createDeprecatedConfigs(usageConfig, {}, messageConfig);
        const usage = {
        ...
        if ( Hooks.call("sentius.itemUsageConsumption", this.item, config, options, usage) === false ) return;
        this._applyDeprecatedConfigs(usageConfig, {}, messageConfig, config, options);
      ...
      if ( !foundry.utils.isEmpty(consumed) ) {
        foundry.utils.setProperty(messageConfig, "data.flags.sentius.use.consumed", consumed);
      }
      if ( usageConfig.cause?.activity ) {
        foundry.utils.setProperty(messageConfig, "data.flags.sentius.use.cause", usageConfig.cause.activity);
      }

      /**
       * A hook event that fires after an item's resource consumption is calculated and applied.
       * @function sentius.postActivityConsumption
       * @memberof hookEvents
       * @param {Activity} activity                           Activity being activated.
       * @param {ActivityUseConfiguration} usageConfig        Configuration data for the activation.
       * @param {ActivityMessageConfiguration} messageConfig  Configuration info for the created chat message.
       ...
       * @returns {boolean}  Explicitly return `false` to prevent activity from being activated.
       */
      if ( Hooks.call("sentius.postActivityConsumption", this, usageConfig, messageConfig, updates) === false ) return;
    ...
    _prepareUsageConfig(config) {
      ...
      const levelingFlag = this.item.getFlag("sentius", "spellLevel");
      if ( levelingFlag ) {
      ...
      if ( this.requiresConcentration && !game.settings.get("sentius", "disableConcentration") ) {
        ...
        if ( limit && (limit <= effects.size) ) config.concentration.end ??= effects.find(e => {
          const data = e.flags.sentius?.item?.data ?? {};
    ...
    async _prepareUsageScaling(usageConfig, messageConfig, item) {
      ...
      if ( levelingFlag ) {
        ...
        if ( level ) {
          ...
          foundry.utils.setProperty(messageConfig, "data.flags.sentius.use.spellLevel", level);
        }
      }

      if ( usageConfig.scaling ) {
        foundry.utils.setProperty(messageConfig, "data.flags.sentius.scaling", usageConfig.scaling);
        item.updateSource({ "flags.sentius.scaling": usageConfig.scaling });
    ...
    async _prepareUsageUpdates(config, { returnErrors=false }={}) {
      ...
      if ( ((config.consume === true) || config.consume.action) && (this.activation.type === "legendary") ) {
        ...
        if ( legendary && !containsLegendaryConsumption ) {
          let message;
          if ( legendary.value === 0 ) message = "SENTIUS.ACTIVATION.Warning.NoActions";
          else if ( count > legendary.value ) message = "SENTIUS.ACTIVATION.Warning.NotEnoughActions";
          if ( message ) {
            const err = new ConsumptionError(game.i18n.format(message, {
              type: game.i18n.localize("SENTIUS.LegAct"),
        ...
        if ( linkedActivity ) {
          ...
          if ( foundry.utils.getType(results) === "Object" ) {
            ...
            if ( updates.delete.includes(linkedActivity.item.id)
              && (this.item.getFlag("sentius", "cachedFor") === linkedActivity.relativeUUID) ) {
      ...
      // Handle spell slot consumption
        ...
        if ( slotData ) {
          if ( slotData.value ) {
            ...
            const err = new ConsumptionError(game.i18n.format("SENTIUS.SpellCastNoSlots", {
      ...
      if ( config.concentration?.begin ) {
        ...
        if ( config.concentration.end ) {
          ...
          if ( !replacedEffect ) errors.push(
            new ConsumptionError(game.i18n.localize("SENTIUS.ConcentratingMissingItem"))
          );
        }
        ...
        // Cannot begin more concentrations than the limit
          ...
          new ConsumptionError(game.i18n.localize("SENTIUS.ConcentratingLimited"))
        );
      }
    ...
    async _usageChatContext(message) {
      ...
      if ( this.activation.condition ) {
        supplements.push(`<strong>${game.i18n.localize("SENTIUS.Trigger")}</strong> ${this.activation.condition}`);
      }
      if ( data.materials?.value ) {
        supplements.push(`<strong>${game.i18n.localize("SENTIUS.Materials")}</strong> ${data.materials.value}`);
      }
      const buttons = this._usageChatButtons(message);

      // Include spell level in the subtitle.
      if ( this.item.type === "spell" ) {
        const spellLevel = foundry.utils.getProperty(message, "data.flags.sentius.use.spellLevel");
        const { spellLevels, spellSchools } = CONFIG.SENTIUS;
    ...
    _usageChatButtons(message) {
      ...
      if ( this.target?.template?.type ) buttons.push({
        label: game.i18n.localize("SENTIUS.TARGET.Action.PlaceTemplate"),
      ...
      if ( message.hasConsumption ) buttons.push({
        label: game.i18n.localize("SENTIUS.CONSUMPTION.Action.ConsumeResource"),
        icon: '<i class="fa-solid fa-cubes-stacked" inert></i>',
        dataset: {
          action: "consumeResource"
        }
      }, {
        label: game.i18n.localize("SENTIUS.CONSUMPTION.Action.RefundResource"),
    ...
    shouldHideChatButton(button, message) {
      const flag = message.getFlag("sentius", "use.consumed");
    ...
    async _createUsageMessage(message) {
       ...
       * @function sentius.preCreateUsageMessage
       * @memberof hookEvents
       * @param {Activity} activity                     Activity for which the card will be created.
       * @param {ActivityMessageConfiguration} message  Configuration info for the created message.
       */
      Hooks.callAll("sentius.preCreateUsageMessage", this, messageConfig);
       ...
       * @function sentius.postCreateUsageMessage
       * @memberof hookEvents
       ...
       */
      Hooks.callAll("sentius.postCreateUsageMessage", this, card);
    ...
    async rollDamage(config={}, dialog={}, message={}) {
      ...
      if ( "sentius.preRollDamage" in Hooks.events ) {
        foundry.utils.logCompatibilityWarning(
          "The `sentius.preRollDamage` hook has been deprecated and replaced with `sentius.preRollDamageV2`.",
          { since: "SENTIUS", until: "SENTIUS"" }
        );
        const oldRollConfig = {
          actor: this.actor,
        ...
        if ( Hooks.call("sentius.preRollDamage", this.item, oldRollConfig) === false ) return;
      ...
      if ( !foundry.utils.isEmpty(lastDamageTypes) && this.actor.items.has(this.item.id) ) {
        await this.item.setFlag("sentius", `last.${this.id}.damageType`, lastDamageTypes);
      }

      /**
       * A hook event that fires after damage has been rolled.
       * @function sentius.rollDamageV2
       * @memberof hookEvents
       * @param {DamageRoll[]} rolls       The resulting rolls.
       * @param {object} [data]
       * @param {Activity} [data.subject]  The activity that performed the roll.
       */
      Hooks.callAll("sentius.rollDamageV2", rolls, { subject: this });

      if ( "sentius.rollDamage" in Hooks.events ) {
        foundry.utils.logCompatibilityWarning(
          "The `sentius.rollDamage` hook has been deprecated and replaced with `sentius.rollDamageV2`.",
          { since: "SENTIUS", until: "SENTIUS"" }
        );
        Hooks.callAll("sentius.rollDamage", this.item, returnMultiple ? rolls : rolls[0]);
      }
    ...
    getContextMenuOptions() {
      ...
      if ( this.item.isOwner && !this.item.compendium?.locked ) {
        entries.push({
          ...
          name: "SENTIUS.ContextMenuActionDelete",
          icon: '<i class="fas fa-trash fa-fw"></i>',
          callback: () => this.deleteDialog()
        });
      } else {
        entries.push({
          name: "SENTIUS.ContextMenuActionView",
      ...
      if ( "favorites" in (this.actor?.system ?? {}) ) {
        ...
        entries.push({
          name: isFavorited ? "SENTIUS.FavoriteRemove" : "SENTIUS.Favorite",
    ...
    async #onChatAction(event, target, message) {
      const scaling = message.getFlag("sentius", "scaling") ?? 0;
      const item = scaling ? this.item.clone({ "flags.sentius.scaling": scaling }, { keepId: true }) : this.item;
    ...
    static onContextMenu(item, target) {
       ...
       * @function sentius.getItemActivityContext
       * @memberof hookEvents
       ...
       */
      Hooks.callAll("sentius.getItemActivityContext", activity, target, menuItems);
    ...
    async #consumeResource(event, target, message) {
      const messageConfig = {};
      const scaling = message.getFlag("sentius", "scaling");
      const usageConfig = { consume: true, event, scaling };
      const linkedActivity = this.getLinkedActivity(message.getFlag("sentius", "use.cause"));
    ...
    async #refundResource(event, target, message) {
      const consumed = message.getFlag("sentius", "use.consumed");
      if ( !foundry.utils.isEmpty(consumed) ) {
        await this.refund(consumed);
        await message.unsetFlag("sentius", "use.consumed");
      }
    }
    ...
    async #placeTemplate() {
      ...
      try {
        ...
        Hooks.onError("Activity#placeTemplate", err, {
          msg: game.i18n.localize("SENTIUS.TARGET.Warning.PlaceTemplate"),
          log: "error",
          notify: "error"
        });
      }
    ...
    getLinkedActivity(relativeUUID) {
      ...
      relativeUUID ??= this.item.getFlag("sentius", "cachedFor");
