...
export default class SummonActivity extends ActivityMixin(SummonActivityData) {
  ...
  /** @inheritDoc */
  static LOCALIZATION_PREFIXES = [...super.LOCALIZATION_PREFIXES, "SENTIUS.SUMMON"];
  ...
  static metadata = Object.freeze(
    foundry.utils.mergeObject(super.metadata, {
      type: "summon",
      img: "systems/sentius/icons/svg/activity/summon.svg",
      title: "SENTIUS.SUMMON.Title",
  ...
  static localize() {
    super.localize();
    this._localizeSchema(this.schema.fields.profiles.element, ["SENTIUS.SUMMON.FIELDS.profiles"]);
  }
   ...
   */
  get canSummon() {
    return game.user.can("TOKEN_CREATE") && (game.user.isGM || game.settings.get("sentius", "allowSummoning"));
  }
  ...
  _usageChatButtons(message) {
    ...
    return [{
      label: game.i18n.localize("SENTIUS.SUMMON.Action.Summon"),
  ...
  async placeSummons(options) {
    ...
    if ( !profile ) throw new Error(
      game.i18n.format("SENTIUS.SUMMON.Warning.NoProfile", { profileId: options.profile, item: this.item.name })
    );

    /**
     * A hook event that fires before summoning is performed.
     * @function sentius.preSummon
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("sentius.preSummon", this, profile, options) === false ) return;
    ...
    if ( !actor.isOwner ) {
      throw new Error(game.i18n.format("SENTIUS.SUMMON.Warning.NoOwnership", { actor: actor.name }));
    }
    ...
    try {
      ...
      for ( const placement of placements ) {
         ...
         * @function sentius.preSummonToken
         * @memberof hookEvents
         * @param {SummonActivity} activity         The activity that is performing the summoning.
         * @param {SummonsProfile} profile          Profile used for summoning.
         ...
         */
        if ( Hooks.call("sentius.preSummonToken", this, profile, tokenUpdateData, options) === false ) continue;
         ...
         * @param {SummoningConfiguration} options  Additional summoning options.
         */
        Hooks.callAll("sentius.summonToken", this, profile, tokenData, options);
     ...
     * @param {SummoningConfiguration} options  Additional summoning options.
     */
    Hooks.callAll("sentius.postSummon", this, profile, createdTokens, options);
  ...
  async fetchActor(uuid) {
    ...
    if ( !actor ) throw new Error(game.i18n.format("SENTIUS.SUMMON.Warning.NoActor", { uuid }));

    const actorLink = actor.prototypeToken.actorLink;
    if ( !actor.pack && (!actorLink || actor.getFlag("sentius", "summon.origin") === this.item?.uuid )) return actor;
    ...
    const localActor = game.actors.find(a =>
      ...
      a.getFlag("sentius", "summonedCopy")
      ...
      && ((a.getFlag("sentius", "summon.origin") === this.item?.uuid) || !a.prototypeToken.actorLink)
    );
    if ( localActor ) return localActor;

    // Check permissions to create actors before importing
    if ( !game.user.can("ACTOR_CREATE") ) throw new Error(game.i18n.localize("SENTIUS.SUMMON.Warning.CreateActor"));
    ...
    if ( actor.pack ) {
      ...
      return game.actors.importFromCompendium(game.packs.get(actor.pack), actor.id, {
        "flags.sentius.summonedCopy": true
      });
    } else {
  ...
  async getChanges(actor, profile, options) {
    ...
    // Add flags
    actorUpdates["flags.sentius.summon"] = {
    ...
    if ( this.match.proficiency ) {
      const proficiencyEffect = new ActiveEffect({
        _id: staticID("sentiusMatchProficiency"),
        changes: [{
        ...
        name: game.i18n.localize("SENTIUS.SUMMON.FIELDS.match.proficiency.label")
      });
      ...
      if ( acBonus.total ) {
        if ( actor.system.attributes.ac.calc === "flat" ) {
          ...
          actorUpdates.effects.push((new ActiveEffect({
            ...
            name: game.i18n.localize("SENTIUS.SUMMON.FIELDS.bonuses.ac.label")
          })).toObject());
        }
      }
    }
    ...
    if ( this.bonuses.hd && (actor.type === "npc") ) {
      ...
      if ( hdBonus.total ) {
        actorUpdates.effects.push((new ActiveEffect({
          ...
          name: game.i18n.localize("SENTIUS.SUMMON.FIELDS.bonuses.hd.label")
        })).toObject());
      }
    }
      ...
      if ( hpBonus.total ) {
        ...
        const maxHpEffect = hpField => {
          return (new ActiveEffect({
            ...
            name: game.i18n.localize("SENTIUS.SUMMON.FIELDS.bonuses.hp.label")
          })).toObject();
        };
    ...
    if ( this.creatureSizes.size ) {
      ...
      const config = CONFIG.SENTIUS.actorSizes[size];
      if ( config ) {
    ...
    for ( const item of actor.items ) {
      ...
      if ( changes.length ) {
        const effect = (new ActiveEffect({
          _id: staticID("sentiusItemChanges"),
          changes,
          disabled: false,
          icon: "icons/skills/melee/strike-slashes-orange.webp",
          name: game.i18n.localize("SENTIUS.SUMMON.ItemChanges.Label"),
          origin: this.uuid,
          type: "enchantment"
        })).toObject();
  ...
  async getTokenData({ actor, placement, tokenUpdates, actorUpdates }) {
    if ( actor.prototypeToken.randomImg && !game.user.can("FILES_BROWSE") ) {
      ...
      ui.notifications.warn("SENTIUS.SUMMON.Warning.Wildcard", { localize: true });
    }
      ...
      try {
        await SummonUsageDialog.create(this, config, {
          button: {
            ...
            label: "SENTIUS.SUMMON.Action.Summon"
          },
          display: {
            all: false,
            create: { summons: true }
