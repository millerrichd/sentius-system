...
function _getRulesVersion(options) {
  ...
  return options.relativeTo?.parent?.system?.source?.rules
    ...
    || (game.settings.get("sentius", "rulesVersion") === "modern" ? "2024" : "2014");
}
...
async function enrichAttack(config, label, options) {
  ...
  for ( const value of config.values ) {
    if ( value in CONFIG.SENTIUS.attackModes ) config.attackMode = value;
  ...
  span.innerHTML = game.i18n.format(`EDITOR.SENTIUS.Inline.Attack${config._rules === "2014" ? "Long" : "Short"}`, {
    formula: createRollLink(displayFormula).outerHTML
  });

  if ( config.format === "extended" ) {
    const type = game.i18n.format(`SENTIUS.ATTACK.Formatted.${config._rules}`, {
      type: game.i18n.getListFormatter({ type: "disjunction" }).format(
        Array.from(activity?.validAttackTypes ?? []).map(t => CONFIG.SENTIUS.attackTypes[t]?.label)
      ),
      classification: CONFIG.SENTIUS.attackClassifications[activity?.attack.type.classification]?.label ?? ""
    }).trim();
    ...
    full.innerHTML = game.i18n.format("EDITOR.SENTIUS.Inline.AttackExtended", {
...
async function enrichAward(config, label, options) {
  ...
  block.classList.add("award-block", "sentius2");
  block.dataset.awardCommand = command;

  const entries = [];
  for ( let [key, amount] of Object.entries(parsed.currency) ) {
    const label = CONFIG.SENTIUS.currencies[key].label;
  ...
  if ( parsed.xp ) entries.push(`
    <span class="award-entry">
      ${formatNumber(parsed.xp)} ${game.i18n.localize("SENTIUS.ExperiencePointsAbbr")}
    </span>
  `);

  let award = game.i18n.getListFormatter({ type: "unit" }).format(entries);
  if ( parsed.each ) award = game.i18n.format("EDITOR.SENTIUS.Inline.AwardEach", { award });

  block.innerHTML += `
    ${award}
    <a class="award-link" data-action="awardRequest">
      <i class="fa-solid fa-trophy"></i> ${label ?? game.i18n.localize("SENTIUS.Award.Action")}
    </a>
  `;
...
async function enrichCheck(config, label, options) {
  for ( let value of config.values ) {
    ...
    if ( value in CONFIG.SENTIUS.enrichmentLookup.abilities ) config.ability = value;
    else if ( value in CONFIG.SENTIUS.enrichmentLookup.skills ) config.skill = value;
    else if ( value in CONFIG.SENTIUS.enrichmentLookup.tools ) config.tool = value;
  ...
  const skillConfig = CONFIG.SENTIUS.enrichmentLookup.skills[slugify(config.skill)];
  ...
  const toolConfig = CONFIG.SENTIUS.tools[slugify(config.tool)];
  const toolUUID = CONFIG.SENTIUS.enrichmentLookup.tools[slugify(config.tool)];
  ...
  let abilityConfig = CONFIG.SENTIUS.enrichmentLookup.abilities[slugify(config.ability)];
...
async function enrichSave(config, label, options) {
  ...
  const abilityConfig = CONFIG.SENTIUS.enrichmentLookup.abilities[config.ability];
...
async function enrichDamage(configs, label, options) {
  ...
  for ( const c of configs ) {
    ...
    for ( const value of c.values ) {
      if ( value in CONFIG.SENTIUS.damageTypes ) c.type = value;
      else if ( value in CONFIG.SENTIUS.healingTypes ) c.type = value;
      else if ( value in CONFIG.SENTIUS.attackModes ) config.attackMode = value;
  ...
  for ( const [idx, formula] of config.formulas.entries() ) {
    ...
    const types = type?.split("|")
      .map(t => CONFIG.SENTIUS.damageTypes[t]?.label ?? CONFIG.SENTIUS.healingTypes[t]?.label)
      .filter(_ => _);
    ...
    parts.push(game.i18n.format(`EDITOR.SENTIUS.Inline.Damage${localizationType}`, localizationData));
  }
  ...
  if ( config.average && (parts.length === 2) ) {
    link.innerHTML = game.i18n.format("EDITOR.SENTIUS.Inline.DamageDouble", { first: parts[0], second: parts[1] });
  } else {
  ...
  if ( config.format === "extended" ) {
    ...
    span.innerHTML = game.i18n.format("EDITOR.SENTIUS.Inline.DamageExtended", { damage: link.outerHTML });
    return span;
  }
 ...
 * ```[[lookup @target.template.size activity=sentiusactivity000]]```
 * becomes
 * ```html
 ...
 *      data-uuid="Compendium.sentius.rules.JournalEntry.w7eitkpD7QQTB6j0.JournalEntryPage.UWw13ISmMxDzmwbd"
 ...
 *      data-tooltip="EDITOR.SENTIUS.Inline.ApplyStatus" aria-label="Apply Status to Selected Tokens">
...
async function enrichReference(config, label, options) {
  ...
  const type = Object.keys(config).find(k => k in CONFIG.SENTIUS.ruleTypes);
  if ( type ) {
    key = slugify(config[type]);
    source = foundry.utils.getProperty(CONFIG.SENTIUS, CONFIG.SENTIUS.ruleTypes[type].references)?.[key];
  } else if ( config.values.length ) {
    key = slugify(config.values.join(""));
    for ( const [type, { references }] of Object.entries(CONFIG.SENTIUS.ruleTypes) ) {
      source = foundry.utils.getProperty(CONFIG.SENTIUS, references)[key];
      if ( source ) {
  ...
  if ( isCondition && (config.apply !== false) ) {
    ...
    apply.dataset.tooltip = "EDITOR.SENTIUS.Inline.ApplyStatus";
...
export function createRollLabel(config) {
  const { label: ability, abbreviation } = CONFIG.SENTIUS.abilities[config.ability] ?? {};
  const skill = CONFIG.SENTIUS.skills[config.skill]?.label;
  const toolUUID = CONFIG.SENTIUS.enrichmentLookup.tools[config.tool];
  ...
  switch ( config.type ) {
    ...
    case "tool":
      if ( ability && (skill || tool) ) {
        label = game.i18n.format("EDITOR.SENTIUS.Inline.SpecificCheck", { ability, type: skill ?? tool });
      } else {
        label = ability;
      }
      if ( config.passive ) {
        label = game.i18n.format(
          `EDITOR.SENTIUS.Inline.${showDC ? "DC" : ""}Passive${longSuffix}`, { dc: config.dc, check: label }
        );
      } else {
        if ( showDC ) label = game.i18n.format("EDITOR.SENTIUS.Inline.DC", { dc: config.dc, check: label });
        label = game.i18n.format(`EDITOR.SENTIUS.Inline.Check${longSuffix}`, { check: label });
      }
      break;
    case "concentration":
    case "save":
      if ( config.type === "save" ) label = ability;
      else label = `${game.i18n.localize("SENTIUS.Concentration")} ${ability ? `(${abbreviation})` : ""}`;
      if ( showDC ) label = game.i18n.format("EDITOR.SENTIUS.Inline.DC", { dc: config.dc, check: label });
      label = game.i18n.format(`EDITOR.SENTIUS.Inline.Save${longSuffix}`, { save: label });
      break;
    default:
      return "";
  }
  ...
  if ( config.icon ) {
    switch ( config.type ) {
      case "check":
      case "skill":
        label = `<i class="sentius-icon" data-src="systems/sentius/icons/svg/ability-score-improvement.svg"></i>${label}`;
        break;
      case "tool":
...
function createRequestLink(label, dataset) {
  ...
  if ( game.user.isGM ) {
    ...
    gmLink.dataset.tooltip = "EDITOR.SENTIUS.Inline.RequestRoll";
...
async function rollAction(event) {
  ...
  const options = { event };
  if ( ability in CONFIG.SENTIUS.abilities ) options.ability = ability;
  ...
  if ( (action === "roll") || !game.user.isGM ) {
    ...
    try {
      ...
      if ( !actors.length ) {
        ui.notifications.warn("EDITOR.SENTIUS.Inline.Warning.NoActor", { localize: true });
        return;
      }
  ...
  // Roll request
    ...
    const chatData = {
      ...
      flavor: game.i18n.localize("EDITOR.SENTIUS.Inline.RollRequest"),
...
async function rollAttack(event) {
  ...
  const messageConfig = {
    data: {
      ...
      flavor: game.i18n.localize("SENTIUS.AttackRoll"),
  ...
  if ( rolls?.length ) {
    Hooks.callAll("sentius.rollAttackV2", rolls, { subject: null, ammoUpdate: null });
    Hooks.callAll("sentius.postRollAttack", rolls, { subject: null });
  }
}
...
async function rollDamage(event) {
  ...
  const messageConfig = {
    ...
    data: {
      ...
      flavor: game.i18n.localize(`SENTIUS.${rollType === "healing" ? "Healing" : "Damage"}Roll`),
      speaker: ChatMessage.implementation.getSpeaker()
    }
  };
  ...
  Hooks.callAll("sentius.rollDamageV2", rolls);
}
...
async function useItem({ rollActivityUuid, rollActivityName, rollItemUuid, rollItemName, rollItemActor }={}) {
  ...
  if ( item ) {
    if ( rollActivityName ) {
      ...
      else ui.notifications.warn(game.i18n.format("EDITOR.SENTIUS.Inline.Warning.NoActivityOnItem", {
  ...
  else ui.notifications.warn(game.i18n.format("EDITOR.SENTIUS.Inline.Warning.NoItemOnActor", {
