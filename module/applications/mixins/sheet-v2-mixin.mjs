...
export default function DocumentSheetV2Mixin(Base) {
  return class DocumentSheetV2 extends Base {
    ...
    /** @inheritDoc */
    static _customElements = super._customElements.concat(["sentius-checkbox", "proficiency-cycle", "slide-toggle"]);
    ...
    async _renderOuter() {
      ...
      if ( this.isEditable ) {
        ...
        toggle.dataset.tooltip = "SENTIUS.SheetModeEdit";
        toggle.setAttribute("aria-label", game.i18n.localize("SENTIUS.SheetModeEdit"));
    ...
    _renderSourceOuter([html]) {
      ...
      elements.innerHTML = `
        <div class="source-book">
          <a class="config-button" data-action="source" data-tooltip="SENTIUS.SOURCE.Action.Configure"
             aria-label="${game.i18n.localize("SENTIUS.SOURCE.Action.Configure")}">
    ...
    _renderSource() {
      ...
      elements.querySelector(".source-book > span").innerText = editable
        ? (source.label || game.i18n.localize("SENTIUS.SOURCE.FIELDS.source.label"))
        : source.label;
    }
    ...
    _disableFields(form) {
      ...
      form.querySelectorAll("sentius-checkbox:not(.interface-only)").forEach(input => input.disabled = true);
    }
    ...
    async _onChangeSheetMode(event) {
      ...
      const label = game.i18n.localize(`SENTIUS.SheetMode${toggle.checked ? "Play" : "Edit"}`);
    ...
    async _onToggleDescription(event) {
      ...
      if ( expanded ) {
        ...
        const content = await renderTemplate("systems/sentius/templates/items/parts/item-summary.hbs", context);
