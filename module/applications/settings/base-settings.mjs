...
export default class BaseSettingsConfig extends Application5e {
  ...
  static PARTS = {
    config: {
      template: "systems/sentius/templates/settings/base-config.hbs"
    },
    footer: {
   ...
   * @param {string} name  Setting key within the sentius namespace.
   * @returns {object}
   */
  createSettingField(name) {
    const setting = game.settings.settings.get(`sentius.${name}`);
    if ( !setting ) throw new Error(`Setting \`sentius.${name}\` not registered.`);
    ...
    const data = {
      ...
      name,
      value: game.settings.get("sentius", name)
    };
  ...
  static async #onCommitChanges(event, form, formData) {
    for ( const [key, value] of Object.entries(foundry.utils.expandObject(formData.object)) ) {
      await game.settings.set("sentius", key, value);
    }
  }
}
