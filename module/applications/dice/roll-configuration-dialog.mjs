...
export default class RollConfigurationDialog extends Dialog5e {
  ...
  static DEFAULT_OPTIONS = {
    ...
    window: {
      title: "SENTIUS.RollConfiguration.Title",
  ...
  static PARTS = {
    formulas: {
      template: "systems/sentius/templates/dice/roll-formulas.hbs"
    },
    configuration: {
      template: "systems/sentius/templates/dice/roll-configuration.hbs"
    },
    buttons: {
      template: "systems/sentius/templates/dice/roll-buttons.hbs"
    }
  };
  ...
  _identifyDiceTerms() {
    ...
    const identifyTerm = term => {
      ...
      for ( let i = 0; i < term.number; i++ ) dice.push({
        icon: `systems/sentius/icons/svg/dice/${term.denomination}.svg`,
  ...
  async _prepareButtonsContext(context, options) {
    context.buttons = {
      roll: {
        ...
        label: game.i18n.localize("SENTIUS.Roll")
      }
    };
    return context;
  }
  ...
  async _prepareConfigurationContext(context, options) {
    context.fields = [{
      field: new foundry.data.fields.StringField({ label: game.i18n.localize("SENTIUS.RollMode") }),
      name: "rollMode",
  ...
  _buildConfig(config, formData, index) {
     ...
     * @function sentius.buildRollConfig
     * @memberof hookEvents
     ...
     * @param {number} index                   Index of the roll within all rolls being prepared.
     */
    Hooks.callAll("sentius.buildRollConfig", this, config, formData, index);
  ...
  /** @override */
  _onClose(options={}) {
    if ( !options.sentius?.submitted ) this.#rolls = [];
  }
