...
export default class MovementSensesConfig extends BaseConfigSheet {
  ...
  static PARTS = {
    config: {
      template: "systems/sentius/templates/shared/config/movement-senses-config.hbs"
    }
  };
  ...
  /** @override */
  get title() {
    return game.i18n.localize(this.options.type === "movement" ? "SENTIUS.Movement" : "SENTIUS.Senses");
  }
   ...
   */
  get types() {
    if ( this.options.type === "senses" ) return Object.keys(CONFIG.SENTIUS.senses);
    if ( this.document.type === "group" ) return ["land", "water", "air"];
    return Object.keys(CONFIG.SENTIUS.movementTypes);
  }
  ...
  async _preparePartContext(partId, context, options) {
    ...
    context.unitsOptions = Object.entries(CONFIG.SENTIUS.movementUnits).map(([value, { label }]) => ({ value, label }));
    if ( (this.document.type === "pc") || ((this.document.type === "npc") && placeholderData) ) {
      const automaticUnit = CONFIG.SENTIUS.movementUnits[placeholderData?.units ?? defaultUnits("length")]?.label ?? "";
      context.unitsOptions.unshift(
        { value: "", label: game.i18n.format("SENTIUS.AutomaticValue", { value: automaticUnit.toLowerCase() }) },
        { rule: true }
      );
    }
