...
export default class ItemListControlsElement extends HTMLElement {
   ...
   */
  get prefs() {
    return game.user.getFlag("sentius", `sheetPrefs.${this.app.object.type}.tabs.${this.tab}`);
  }
      ...
      <ul class="unlist controls">
        <li>
          <button type="button" class="unbutton filter-control interface-only" data-action="clear"
                  data-tooltip="SENTIUS.FilterClear" aria-label="${game.i18n.localize("SENTIUS.FilterClear")}">
    ...
    if ( options.length ) {
      ...
      item.innerHTML = `
        <button type="button" class="unbutton filter-control filter interface-only" data-action="filter"
                aria-label="${game.i18n.localize("SENTIUS.Filter")}">
  ...
  async _onToggleMode(event) {
    ...
    const current = game.user.getFlag("sentius", flag);
    let value;
    ...
    await game.user.setFlag("sentius", flag, value);
    if ( action === "group" ) {
      this._initGrouping();
      this._applyGrouping();
    } else if ( action === "sort" ) {
