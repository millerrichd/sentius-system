...
export default class InventoryElement extends HTMLElement {
  ...
  /**
   * TODO: Remove filtering code from sentius-inventory when all sheets use item-list-controls.
  ...
  _getContextOptions(item, element) {
    ...
    const options = [
      ...
      },
      {
        ...
        condition: li => (item.type === "spell") && !item.getFlag("sentius", "cachedFor") && this.actor?.isOwner
          && !this.actor?.compendium?.locked,
        group: "action"
      },
      {
        name: "SENTIUS.ConcentrationBreak",
        icon: '<sentius-icon src="systems/sentius/icons/svg/break-concentration.svg"></sentius-icon>',
    ...
    if ( item.system.attunement ) {
      options.push({
        name: item.system.attuned ? "SENTIUS.ContextMenuActionUnattune" : "SENTIUS.ContextMenuActionAttune",
    ...
    if ( "equipped" in item.system ) options.push({
      name: item.system.equipped ? "SENTIUS.ContextMenuActionUnequip" : "SENTIUS.ContextMenuActionEquip",
    ...
    if ( item.hasRecharge ) options.push({
      name: item.isOnCooldown ? "SENTIUS.ContextMenuActionCharge" : "SENTIUS.ContextMenuActionExpendCharge",
    ...
    // Toggle Prepared State
      ...
      && !item.getFlag("sentius", "cachedFor") ) options.push({
      name: item.system?.preparation?.prepared ? "SENTIUS.ContextMenuActionUnprepare" : "SENTIUS.ContextMenuActionPrepare",
      icon: "<i class='fas fa-sun fa-fw'></i>",
      condition: () => item.isOwner && !item.compendium?.locked,
  ...
  async _onCreate(target) {
    ...
    if ( type === "class" && (this.actor.system.details.level + 1 > CONFIG.SENTIUS.maxLevel) ) {
    ...
    const itemData = {
      name: game.i18n.format("SENTIUS.ItemNew", {type: game.i18n.localize(CONFIG.Item.typeLabels[type])}),
      type,
  ...
  async _onExpand(target, item) {
    ...
    if ( this._app._expanded.has(item.id) ) {
      ...
      const summary = $(await renderTemplate("systems/sentius/templates/items/parts/item-summary.hbs", chatData));
  ...
  _onOpenContextMenu(element) {
    ...
    if ( element.closest("[data-activity-id]") ) {
      sentius.documents.activity.UtilityActivity.onContextMenu(item, element);
    } else {
      ui.context.menuItems = this._getContextOptions(item, element);
      Hooks.call("sentius.getItemContextOptions", item, ui.context.menuItems);
    }
  }
}
