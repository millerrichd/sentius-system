...
export default class EffectsElement extends HTMLElement {
  connectedCallback() {
    ...
    new MenuCls(this, "[data-effect-id]", [], {onOpen: element => {
      ...
      Hooks.call("sentius.getActiveEffectContextOptions", effect, ui.context.menuItems);
    }});
  }
  ...
  static prepareCategories(effects, { parent }={}) {
    ...
    const categories = {
      enchantment: {
        type: "enchantment",
        label: game.i18n.localize("SENTIUS.ENCHANTMENT.Category.General"),
        effects: [],
        isEnchantment: true
      },
      temporary: {
        type: "temporary",
        label: game.i18n.localize("SENTIUS.EffectTemporary"),
        effects: []
      },
      enchantmentActive: {
        type: "activeEnchantment",
        label: game.i18n.localize("SENTIUS.ENCHANTMENT.Category.Active"),
        effects: [],
        isEnchantment: true
      },
      passive: {
        type: "passive",
        label: game.i18n.localize("SENTIUS.EffectPassive"),
        effects: []
      },
      enchantmentInactive: {
        type: "inactiveEnchantment",
        label: game.i18n.localize("SENTIUS.ENCHANTMENT.Category.Inactive"),
        effects: [],
        isEnchantment: true
      },
      inactive: {
        type: "inactive",
        label: game.i18n.localize("SENTIUS.EffectInactive"),
        effects: []
      },
      suppressed: {
        type: "suppressed",
        label: game.i18n.localize("SENTIUS.EffectUnavailable"),
        effects: [],
        disabled: true,
        info: [game.i18n.localize("SENTIUS.EffectUnavailableInfo")]
      }
    };
    ...
    for ( const category of Object.values(categories) ) {
      category.localizationPrefix = category.isEnchantment ? "SENTIUS.ENCHANTMENT.Action." : "SENTIUS.Effect";
    }
  ...
  _getContextOptions(effect) {
    ...
    const options = [
      ...
      {
        name: effect.disabled ? "SENTIUS.ContextMenuActionEnable" : "SENTIUS.ContextMenuActionDisable",
  ...
  async _onToggleCondition(conditionId) {
    const existing = this.document.effects.get(staticID(`sentius${conditionId}`));
  ...
  async _onCreate(target) {
    ...
    return this.document.createEmbeddedDocuments("ActiveEffect", [{
      ...
      name: isActor ? game.i18n.localize("SENTIUS.EffectNew") : this.document.name,
  ...
  async _onClickEffectSource(event) {
    ...
    if ( !doc.testUserPermission(game.user, "LIMITED") ) {
      ui.notifications.warn("SENTIUS.DocumentViewWarn", { localize: true });
      return;
    }
