...
export default class EnchantmentApplicationElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const scaling = this.chatMessage.getFlag("sentius", "scaling");
    if ( scaling ) item = item.clone({ "flags.sentius.scaling": scaling });
    ...
    if ( maxTargets ) {
      ...
      this.countArea.innerHTML = game.i18n.format("SENTIUS.ENCHANT.Enchanted", {
   ...
   */
  buildItemList() {
    const enchantedItems = sentius.registry.enchantments.applied(this.enchantmentActivity.uuid).map(enchantment => {
      ...
      if ( item.isOwner ) {
        ...
        control.ariaLabel = game.i18n.localize("SENTIUS.ENCHANTMENT.Action.Remove");
        control.dataset.action = "removeEnchantment";
        control.dataset.tooltip = "SENTIUS.ENCHANTMENT.Action.Remove";
    ...
    else this.dropArea.innerHTML = `<p>${game.i18n.localize("SENTIUS.ENCHANT.DropArea")}</p>`;
  ...
  async _onDrop(event) {
    ...
    const effect = this.enchantmentItem.effects.get(this.chatMessage.getFlag("sentius", "use.enchantmentProfile"));
    ...
    const concentrationId = this.chatMessage.getFlag("sentius", "use.concentrationId");
    const concentration = effect.parent.actor.effects.get(concentrationId);
    if ( concentrationId && !concentration ) {
      ui.notifications.error("SENTIUS.ENCHANT.Warning.ConcentrationEnded", { console: false, localize: true });
      return;
    }
    if ( !game.user.isGM && concentration && !concentration.actor?.isOwner ) {
      ui.notifications.error("SENTIUS.EffectApplyWarningConcentration", { console: false, localize: true });
      return;
    }
