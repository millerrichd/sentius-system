...
export default class EffectApplicationElement extends TargetedApplicationMixin(ChatTrayElement) {
    ...
    if ( !this.effectsList || !this.targetList ) {
      ...
      div.innerHTML = `
        <label class="roboto-upper">
          ...
          <span>${game.i18n.localize("SENTIUS.Effects")}</span>
    ...
    for ( const effect of this.effects ) {
      ...
      li.innerHTML = `
        ...
        <button class="apply-effect" type="button" data-action="applyEffect"
                data-tooltip="SENTIUS.EffectsApplyTokens"
                aria-label="${game.i18n.localize("SENTIUS.EffectsApplyTokens")}">
  ...
  buildTargetListEntry({ uuid, name }) {
    ...
    li.innerHTML = `
      ...
      <div class="checkbox">
        <sentius-checkbox name="${uuid}"${checked}${disabled}></sentius-checkbox>
      </div>
    `;
  ...
  async _applyEffectToActor(effect, actor) {
    const concentration = this.chatMessage.getAssociatedActor()?.effects
      .get(this.chatMessage.getFlag("sentius", "use.concentrationId"));
    const origin = concentration ?? effect;
    if ( !game.user.isGM && !actor.isOwner ) {
      throw new Error(game.i18n.localize("SENTIUS.EffectApplyWarningOwnership"));
    }

    const effectFlags = {
      flags: {
        sentius: {
          scaling: this.chatMessage.getFlag("sentius", "scaling"),
          spellLevel: this.chatMessage.getFlag("sentius", "use.spellLevel")
        }
      }
    };
    ...
    if ( !game.user.isGM && concentration && !concentration.actor?.isOwner ) {
      throw new Error(game.i18n.localize("SENTIUS.EffectApplyWarningConcentration"));
    }
  ...
  async _onApplyEffect(event) {
    ...
    for ( const target of this.targetList.querySelectorAll("[data-target-uuid]") ) {
      ...
      if ( !actor || !target.querySelector("sentius-checkbox")?.checked ) continue;
      try {
