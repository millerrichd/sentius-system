...
export default class DamageApplicationElement extends TargetedApplicationMixin(ChatTrayElement) {
    ...
    if ( !this.targetList ) {
      ...
      div.innerHTML = `
        <label class="roboto-upper">
          ...
          <span>${game.i18n.localize("SENTIUS.Apply")}</span>
        ...
        <div class="collapsible-content">
          <div class="wrapper">
            <button class="apply-damage" type="button" data-action="applyDamage">
              ...
              ${game.i18n.localize("SENTIUS.Apply")}
            </button>
          </div>
        </div>
      `;
  ...
  buildTargetListEntry({ uuid, name }) {
    ...
    const changeSources = types.reduce((acc, {type, change, icon}) => {
      ...
      acc += `
        <button class="change-source unbutton" type="button" data-type="${type}" data-change="${change}"
          ...
          <sentius-icon src="${icon}" inert></sentius-icon>
    ...
    li.innerHTML = `
      ...
      <div class="calculated temp" data-tooltip="SENTIUS.HitPointsTemp">
        ${temp}
      </div>
  ...
  getChangeSourceOptions(type, change, options) {
    ...
    let label = game.i18n.format(`SENTIUS.DamageApplication.Change.${change.capitalize()}`, {
      type: CONFIG.SENTIUS.damageTypes[type]?.label ?? CONFIG.SENTIUS.healingTypes[type]?.label
    });
    if ( mode === "ignore" ) label = game.i18n.format("SENTIUS.DamageApplication.Ignoring", { source: label });
    if ( mode === "downgrade" ) label = game.i18n.format("SENTIUS.DamageApplication.Downgrading", { source: label });
  ...
  refreshListEntry(token, entry, options) {
    ...
    calculatedDamage.dataset.tooltip = `SENTIUS.${total < 0 ? "Healing" : "Damage"}`;
