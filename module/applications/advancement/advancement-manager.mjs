...
export default class AdvancementManager extends Application {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["sentius", "advancement", "flow"],
      template: "systems/sentius/templates/advancement/advancement-manager.hbs",
      width: 460,
      height: "auto"
    });
  }
  ...
  get title() {
    ...
    const step = visibleIndex < 0 ? "" : game.i18n.format("SENTIUS.AdvancementManagerSteps", {
      current: visibleIndex + 1,
      total: visibleSteps.length
    });
    return `${game.i18n.localize("SENTIUS.AdvancementManagerTitle")} ${step}`;
  }
  ...
  getData() {
    ...
    return {
      ...
      header: item.name,
      subheader: level ? game.i18n.format("SENTIUS.AdvancementLevelHeader", { level }) : "",
      steps: {
        current: visibleIndex + 1,
        total: visibleSteps.length,
  ...
  render(...args) {
     ...
     * @function sentius.preAdvancementManagerRender
     * @memberof hookEvents
     * @param {AdvancementManager} advancementManager The advancement manager about to be rendered
     */
    const allowed = Hooks.call("sentius.preAdvancementManagerRender", this);
  ...
  async close(options={}) {
    if ( !options.skipConfirmation ) {
      return new Dialog({
        title: `${game.i18n.localize("SENTIUS.AdvancementManagerCloseTitle")}: ${this.actor.name}`,
        content: game.i18n.localize("SENTIUS.AdvancementManagerCloseMessage"),
        buttons: {
          close: {
            icon: '<i class="fas fa-times"></i>',
            label: game.i18n.localize("SENTIUS.AdvancementManagerCloseButtonStop"),
            callback: () => super.close(options)
          },
          continue: {
            icon: '<i class="fas fa-chevron-right"></i>',
            label: game.i18n.localize("SENTIUS.AdvancementManagerCloseButtonContinue")
          }
        },
        default: "close"
      }).render(true);
  ...
  async _restart(event) {
    const restart = await Dialog.confirm({
      title: game.i18n.localize("SENTIUS.AdvancementManagerRestartConfirmTitle"),
      content: game.i18n.localize("SENTIUS.AdvancementManagerRestartConfirm")
    });
    if ( !restart ) return;
  ...
  async _complete(event) {
     ...
     * are applied.
     * @function sentius.preAdvancementManagerComplete
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("sentius.preAdvancementManagerComplete", this, updates, toCreate, toUpdate, toDelete) === false ) {
     ...
     * @function sentius.advancementManagerComplete
     * @memberof hookEvents
     * @param {AdvancementManager} advancementManager The advancement manager that just completed
     */
    Hooks.callAll("sentius.advancementManagerComplete", this);
