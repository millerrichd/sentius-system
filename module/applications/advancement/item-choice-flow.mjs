...
export default class ItemChoiceFlow extends ItemGrantFlow {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      template: "systems/sentius/templates/advancement/item-choice-flow.hbs"
    });
  }
  ...
  async retainData(data) {
    ...
    this.selected = new Set(data.items.map(i => foundry.utils.getProperty(i, "flags.sentius.sourceId")));
  }
  ...
  async getContext() {
    ...
    if ( !this.dropped ) {
      ...
      for ( const data of this.retainedData?.items ?? [] ) {
        const uuid = foundry.utils.getProperty(data, "flags.sentius.sourceId");
  ...
  async _onDrop(event) {
    ...
    for ( const [level, data] of Object.entries(this.advancement.value.added ?? {}) ) {
      ...
      if ( Object.values(data).includes(item.uuid) ) {
        ui.notifications.error("SENTIUS.AdvancementItemChoicePreviouslyChosenWarning", {localize: true});
        return null;
      }
    }
    ...
    if ( (item.system.prerequisites?.level ?? -Infinity) > this.level ) {
      ui.notifications.error(game.i18n.format("SENTIUS.AdvancementItemChoiceFeatureLevelWarning", {
    ...
    if ( (this.advancement.configuration.type === "spell") && spellLevel === "available" ) {
      ...
      if ( item.system.level > maxSlot ) {
        ui.notifications.error(game.i18n.format("SENTIUS.AdvancementItemChoiceSpellLevelAvailableWarning", {
          level: CONFIG.SENTIUS.spellLevels[maxSlot]
        }));
        return null;
      }
    }
