...
export default class AdvancementConfig extends FormApplication {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["sentius", "advancement", "dialog"],
      template: "systems/sentius/templates/advancement/advancement-config.hbs",
      width: 400,
      height: "auto",
      submitOnChange: true,
      closeOnSubmit: false,
  ...
  get title() {
    ...
    return `${game.i18n.format("SENTIUS.AdvancementConfigureTitle", { item: this.item.name })}: ${type}`;
  }
  ...
  /** @inheritDoc */
  getData() {
    const levels = Object.fromEntries(Array.fromRange(CONFIG.SENTIUS.maxLevel + 1).map(l => [l, l]));
    if ( ["class", "subclass"].includes(this.item.type) ) delete levels[0];
    else levels[0] = game.i18n.localize("SENTIUS.AdvancementLevelAnyHeader");
    const context = {
      appId: this.id,
      ...
      classRestrictionOptions: [
        { value: "", label: game.i18n.localize("SENTIUS.AdvancementClassRestrictionNone") },
        { value: "primary", label: game.i18n.localize("SENTIUS.AdvancementClassRestrictionPrimary") },
        { value: "secondary", label: game.i18n.localize("SENTIUS.AdvancementClassRestrictionSecondary") }
      ],
  ...
  async _onDrop(event) {
    ...
    if ( item.uuid === this.item.uuid ) {
      ui.notifications.error("SENTIUS.AdvancementItemGrantRecursiveWarning", {localize: true});
      return null;
    }

    // Abort if this uuid exists already
    if ( existingItems.find(i => i.uuid === item.uuid) ) {
      ui.notifications.warn("SENTIUS.AdvancementItemGrantDuplicateWarning", {localize: true});
      return null;
    }
