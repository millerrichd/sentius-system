...
export default class AdvancementSelection extends Dialog {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["sentius", "sheet", "advancement"],
      template: "systems/sentius/templates/advancement/advancement-selection.hbs",
      title: "SENTIUS.AdvancementSelectionTitle",
      width: 500,
      height: "auto"
    });
  }
  ...
  getData() {
    const context = { types: {} };
    for ( let [name, config] of Object.entries(CONFIG.SENTIUS.advancementTypes) ) {
    ...
    context.types = sentius.utils.sortObjectEntries(context.types, "label");
    return context;
  }
  ...
  static async createDialog(item, { rejectClose=false, options={} }={}) {
    return new Promise((resolve, reject) => {
      const dialog = new this(item, {
        title: `${game.i18n.localize("SENTIUS.AdvancementSelectionTitle")}: ${item.name}`,
        buttons: {
          submit: {
            callback: html => {
