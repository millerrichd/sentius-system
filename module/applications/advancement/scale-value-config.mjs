...
export default class ScaleValueConfig extends AdvancementConfig {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["sentius", "advancement", "scale-value", "two-column"],
      template: "systems/sentius/templates/advancement/scale-value-config.hbs",
      width: 540
    });
  }
  ...
  getData() {
    ...
    return foundry.utils.mergeObject(super.getData(), {
      ...
      movementUnits: CONFIG.SENTIUS.movementUnits
    });
  }
  ...
  _prepareLevelData() {
    let lastValue = null;
    let levels = Array.fromRange(CONFIG.SENTIUS.maxLevel + 1);
  ...
  async _updateObject(event, formData) {
    ...
    if ( typeChange && (updates.configuration.type !== this.advancement.configuration.type) ) {
      ...
      await this.advancement.update(Array.fromRange(CONFIG.SENTIUS.maxLevel, 1).reduce((obj, lvl) => {
