...
export default class TraitConfig extends AdvancementConfig {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["sentius", "advancement", "traits", "two-column"],
      template: "systems/sentius/templates/advancement/trait-config.hbs",
      width: 640
    });
  }
  ...
  async getData() {
    ...
    context.validTraitTypes = Object.entries(CONFIG.SENTIUS.traits).reduce((obj, [key, config]) => {
    ...
    if ( traitConfig ) {
      ...
      context.default.title = game.i18n.localize("SENTIUS.TraitGenericPlural.other");
    ...
    context.selectedTraitHeader = `${CONFIG.SENTIUS.traits[this.trait].labels.localization}.other`;
  ...
  async _onChangeInput(event) {
    ...
    if ( (event.target.name === "configuration.mode")
      ...
      const validTraitTypes = filteredKeys(CONFIG.SENTIUS.traits, c => c[checkKey]);
