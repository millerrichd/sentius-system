...
export default class ActorSheet5eCharacter extends ActorSheet5e {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["sentius", "sheet", "actor", "character"]
    });
  }
  ...
  async getData(options={}) {
    ...
    context.resources = ["primary", "secondary", "tertiary"].reduce((arr, r) => {
      const res = foundry.utils.mergeObject(context.actor.system.resources[r] || {}, {
        name: r,
        placeholder: game.i18n.localize(`SENTIUS.Resource${r.titleCase()}`)
      }, {inplace: false});
    ...
    return foundry.utils.mergeObject(context, {
      disableExperience: game.settings.get("sentius", "levelingMode") === "noxp",
      ...
      weightUnit: game.i18n.localize(`SENTIUS.Abbreviation${
        game.settings.get("sentius", "metricWeightUnits") ? "Kg" : "Lbs"}`),
  ...
  _prepareItems(context) {
    ...
    let {items, spells, feats, races, backgrounds, classes, subclasses} = context.items.reduce((obj, item) => {
      ...
      if ( item.system.attunement ) ctx.attunement = item.system.attuned ? {
        icon: "fa-sun",
        cls: "attuned",
        title: "SENTIUS.AttunementAttuned"
      } : {
        icon: "fa-sun",
        cls: "not-attuned",
        title: CONFIG.SENTIUS.attunementTypes[item.system.attunement]
      };
      ...
      // Item grouping
      ctx.ungroup = "passive";
      const [originId] = item.getFlag("sentius", "advancementOrigin")?.split(".") ?? [];
    ...
    const maxLevelDelta = CONFIG.SENTIUS.maxLevel - this.actor.system.details.level;
    classes = classes.reduce((arr, cls) => {
      const ctx = context.itemContext[cls.id] ??= {};
      ctx.availableLevels = Array.fromRange(CONFIG.SENTIUS.maxLevel + 1).slice(1).map(level => {
    ...
    for ( const subclass of subclasses ) {
      feats.push(subclass);
      const message = game.i18n.format("SENTIUS.SubclassMismatchWarn", {
    ...
    const features = {
      ...
      active: {
        label: "SENTIUS.FeatureActive", items: [],
        hasActions: true, dataset: {type: "feat", "activation.type": "action"} },
      passive: {
        label: "SENTIUS.FeaturePassive", items: [],
  ...
  _prepareItem(item, context) {
    if ( item.type === "spell" ) {
      ...
      if ( isAlways ) context.toggleTitle = CONFIG.SENTIUS.spellPreparationModes.always.label;
      else if ( isPrepared ) context.toggleTitle = CONFIG.SENTIUS.spellPreparationModes.prepared.label;
      else context.toggleTitle = game.i18n.localize("SENTIUS.SpellUnprepared");
    }
    else {
      const isActive = !!item.system.equipped;
      context.toggleClass = isActive ? "active" : "";
      context.toggleTitle = game.i18n.localize(isActive ? "SENTIUS.Equipped" : "SENTIUS.Unequipped");
    ...
    switch ( button.dataset.action ) {
      case "convertCurrency":
        return Dialog.confirm({
          title: `${game.i18n.localize("SENTIUS.CurrencyConvert")}`,
          content: `<p>${game.i18n.localize("SENTIUS.CurrencyConvertHint")}</p>`,
  ...
  async _onDropSingleItem(itemData, event) {
    ...
    if ( itemData.type === "class" ) {
      ...
      itemData.system.levels = Math.min(itemData.system.levels, CONFIG.SENTIUS.maxLevel - charLevel);
      if ( itemData.system.levels <= 0 ) {
        const err = game.i18n.format("SENTIUS.MaxCharacterLevelExceededWarn", { max: CONFIG.SENTIUS.maxLevel });
    ...
    // If a subclass is dropped, ensure it doesn't match another subclass with the same identifier
      ...
      if ( other ) {
        const err = game.i18n.format("SENTIUS.SubclassDuplicateError", {identifier: other.identifier});
        ui.notifications.error(err);
        return false;
      }
      const cls = this.actor.itemTypes.class.find(i => i.identifier === itemData.system.classIdentifier);
      if ( cls && cls.subclass ) {
        const err = game.i18n.format("SENTIUS.SubclassAssignmentError", {class: cls.name, subclass: cls.subclass.name});
        ui.notifications.error(err);
        return false;
      }
    }
