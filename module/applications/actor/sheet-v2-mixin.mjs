...
export default function ActorSheetV2Mixin(Base) {
  return class ActorSheetV2 extends DocumentSheetV2Mixin(Base) {
    constructor(object, options={}) {
      ...
      const { width, height } = game.user.getFlag("sentius", `sheetPrefs.${key}`) ?? {};
    ...
    async _renderOuter() {
      ...
      nav.append(...this.constructor.TABS.map(({ tab, label, icon, svg }) => {
        ...
        else if ( svg ) item.innerHTML = `<sentius-icon src="systems/sentius/icons/svg/${svg}.svg"></sentius-icon>`;
        return item;
      }));
    ...
    async getData(options) {
      ...
      context.sidebarCollapsed = !!game.user.getFlag("sentius", `${sheetPrefs}.collapseSidebar`);
      if ( context.sidebarCollapsed ) context.cssClass += " collapsed";
      const { attributes } = this.actor.system;

      // Portrait
      const showTokenPortrait = this.actor.getFlag("sentius", "showTokenPortrait") === true;
      ...
      ["success", "failure"].forEach(deathSave => {
        ...
        for ( let i = 1; i < 4; i++ ) {
          ...
          const i18nKey = `SENTIUS.DeathSave${deathSave.titleCase()}Label`;
      ...
      // Senses
      context.senses = Object.entries(CONFIG.SENTIUS.senses).reduce((obj, [k, label]) => {
      ...
      context.conditions = Object.entries(CONFIG.SENTIUS.conditionTypes).reduce((arr, [k, c]) => {
        if ( c.pseudo ) return arr; // Filter out pseudo-conditions.
        const { label: name, icon, reference } = c;
        const id = staticID(`sentius${k}`);
        conditionIds.add(id);
      ...
      for ( const category of Object.values(context.effects) ) {
        category.effects = await category.effects.reduce(async (arr, effect) => {
          ...
          if ( (source instanceof sentius.documents.ActiveEffect5e) && (source.target !== this.object) ) {
          ...
          arr.push({
            ...
            hasTooltip: source instanceof sentius.documents.Item5e
          });
          return arr;
        }, []);
      }
    ...
    _prepareTraits() {
      const traits = {};
      for ( const [trait, config] of Object.entries(CONFIG.SENTIUS.traits) ) {
        ...
        values = values.map(key => {
          ...
          if ( data.bypasses?.size && CONFIG.SENTIUS.damageTypes[key]?.isPhysical ) icons.push(...data.bypasses.map(p => {
            const type = CONFIG.SENTIUS.itemProperties[p]?.label;
            return { icon: p, label: game.i18n.format("SENTIUS.DamagePhysicalBypassesShort", { type }) };
          }));
          return value;
        });
      ...
      if ( this.document.hasConditionEffect("petrification") ) {
        traits.dr = [{ label: game.i18n.localize("SENTIUS.DamageAll") }];
      }
      ...
      if ( dm ) {
        ...
        const values = Object.entries(dm.amount).map(([k, v]) => {
          ...
          const value = {
            label: `${CONFIG.SENTIUS.damageTypes[k]?.label ?? k} ${formatNumber(total, { signDisplay: "always" })}`,
            color: total > 0 ? "maroon" : "green"
          };
          const icons = value.icons = [];
          if ( dm.bypasses.size && CONFIG.SENTIUS.damageTypes[k]?.isPhysical ) icons.push(...dm.bypasses.map(p => {
            const type = CONFIG.SENTIUS.itemProperties[p]?.label;
            return { icon: p, label: game.i18n.format("SENTIUS.DamagePhysicalBypassesShort", { type }) };
          }));
          return value;
      ...
      for ( const key of this.actor.system.traits?.weaponProf?.mastery?.value ?? [] ) {
        ...
        value.icons.push({ icon: "mastery", label: game.i18n.format("SENTIUS.WEAPON.Mastery.Label") });
      }
    ...
    _prepareItems(context) {
      ...
      context.spellbook.forEach(section => {
        section.categories = [
          { activityPartial: "sentius.activity-column-school" },
          { activityPartial: "sentius.activity-column-time" },
          { activityPartial: "sentius.activity-column-range" },
          { activityPartial: "sentius.activity-column-target" },
        ...
        section.pips = Array.fromRange(Math.max(max, value), 1).map(n => {
          ...
          const temp = n > max;
          const label = temp
            ? game.i18n.localize("SENTIUS.SpellSlotTemporary")
            : filled
              ? game.i18n.format(`SENTIUS.SpellSlotN.${plurals.select(n)}`, { n })
              : game.i18n.localize("SENTIUS.SpellSlotExpended");
          const classes = ["pip"];
          if ( filled ) classes.push("filled");
    ...
    _prepareItem(item, ctx) {
      ...
      if ( item.type === "spell" ) {
        ...
        const abbr = {
          action: "SENTIUS.ActionAbbr",
          bonus: "SENTIUS.BonusActionAbbr",
          reaction: "SENTIUS.ReactionAbbr",
          minute: "SENTIUS.TimeMinuteAbbr",
          hour: "SENTIUS.TimeHourAbbr",
          day: "SENTIUS.TimeDayAbbr"
        ...
        if ( units && (units !== "none") ) {
          if ( units in CONFIG.SENTIUS.movementUnits ) {
            ctx.range = {
              distance: true,
              value: system.range.value,
              unit: CONFIG.SENTIUS.movementUnits[units].abbreviation,
        ...
        if ( config.prepares && !linked ) {
          ...
          ctx.preparation = {
            ...
            title: isAlways
              ? CONFIG.SENTIUS.spellPreparationModes.always.label
              : prepared
                ? CONFIG.SENTIUS.spellPreparationModes.prepared.label
                : game.i18n.localize("SENTIUS.SpellUnprepared")
          };
        }
      ...
      // Gear
        ...
        if ( "equipped" in system ) {
          ctx.equip = {
            ...
            title: `SENTIUS.${system.equipped ? "Equipped" : "Unequipped"}`,
      ...
      ctx.save.ability = ctx.save.ability?.size ? ctx.save.ability.size === 1
        ? CONFIG.SENTIUS.abilities[ctx.save.ability.first()]?.abbreviation
        : game.i18n.localize("SENTIUS.AbbreviationDC") : null;

      // Activities
      ctx.activities = item.system.activities
        ?.filter(a => !item.getFlag("sentius", "riders.activity")?.includes(a.id))
      ...
      return {
        ...
        save: save ? {
          ...
          ability: save.ability?.size
            ? save.ability.size === 1
              ? CONFIG.SENTIUS.abilities[save.ability.first()]?.abbreviation
    ...
    _getLabels() {
      ...
      labels.damageAndHealing = { ...CONFIG.SENTIUS.damageTypes, ...CONFIG.SENTIUS.healingTypes };
      return labels;
    }
    ...
    _onChangeTab(event, tabs, active) {
      ...
      const sidebarCollapsed = game.user.getFlag("sentius", `${sheetPrefs}.collapseSidebar`);
    ...
    _onCreateChild() {
      ...
      if ( activeTab === "effects" ) return ActiveEffect.implementation.create({
        name: game.i18n.localize("SENTIUS.EffectNew"),
    ...
    _onResize(event) {
      ...
      game.user.setFlag("sentius", `sheetPrefs.${key}`, { width, height });
    }
    ...
    _onToggleSidebar() {
      ...
      game.user.setFlag("sentius", `sheetPrefs.${this.actor.type}.tabs.${activeTab}.collapseSidebar`, collapsed);
    }
