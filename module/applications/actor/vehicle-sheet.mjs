...
export default class ActorSheet5eVehicle extends ActorSheet5e {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["sentius", "sheet", "actor", "vehicle"]
    });
  }
  ...
  _prepareCrewedItem(item, context) {
    ...
    context.toggleTitle = game.i18n.localize(`SENTIUS.${isCrewed ? "Crewed" : "Uncrewed"}`);

    // Handle crew actions
    if ( item.type === "feat" && item.system.activation.type === "crew" ) {
      context.cover = game.i18n.localize(`SENTIUS.${item.system.cover ? "CoverTotal" : "None"}`);
  ...
  _prepareItems(context) {
    const cargoColumns = [{
      label: game.i18n.localize("SENTIUS.Quantity"),
      css: "item-qty",
      property: "quantity",
      editable: "Number"
    }];
    ...
    const equipmentColumns = [{
      ...
      label: game.i18n.localize("SENTIUS.AC"),
      css: "item-ac",
      property: "system.armor.value"
    }, {
      label: game.i18n.localize("SENTIUS.HP"),
      css: "item-hp",
      property: "system.hp.value",
      maxProperty: "system.hp.max",
      editable: "Number"
    }, {
      label: game.i18n.localize("SENTIUS.Threshold"),
      css: "item-threshold",
      property: "threshold"
    }];
    ...
    const features = {
      actions: {
        label: game.i18n.localize("SENTIUS.ActionPl"),
        items: [],
        hasActions: true,
        crewable: true,
      ...
      passive: {
        label: game.i18n.localize("SENTIUS.Features"),
        items: [],
        dataset: {type: "feat"}
      },
      reactions: {
        label: game.i18n.localize("SENTIUS.ReactionPl"),
        items: [],
    ...
    const cargo = {
      crew: {
        label: game.i18n.localize("SENTIUS.VehicleCrew"),
      ...
      passengers: {
        label: game.i18n.localize("SENTIUS.VehiclePassengers"),
      ...
      cargo: {
        label: game.i18n.localize("SENTIUS.VehicleCargo"),
        items: [],
        dataset: {type: "loot"},
        columns: [{
          label: game.i18n.localize("SENTIUS.Quantity"),
          ...
          label: game.i18n.localize("SENTIUS.Price"),
          css: "item-price",
          property: "system.price.value",
          editable: "Number"
        }, {
          label: game.i18n.localize("SENTIUS.Weight"),
          css: "item-weight",
    ...
    for ( const item of context.items ) {
      ...
      const isCargo = item.flags.sentius?.vehicleCargo === true;
      if ( isCargo ) {
  ...
  activateListeners(html) {
    ...
    html[0].querySelector('[data-tab="cargo"] sentius-inventory')
  ...
  async _onDropSingleItem(itemData, event) {
    ...
    foundry.utils.setProperty(itemData, "flags.sentius.vehicleCargo", isCargo);
