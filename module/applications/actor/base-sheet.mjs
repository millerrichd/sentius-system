...
export default class ActorSheet5e extends ActorSheetMixin(ActorSheet) {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      scrollY: [
        "sentius-inventory .inventory-list",
        "sentius-effects .effects-list",
        ".center-pane"
      ],
      tabs: [{navSelector: ".tabs", contentSelector: ".sheet-body", initial: "description"}],
      width: 720,
      height: Math.max(680, Math.max(
        237 + (Object.keys(CONFIG.SENTIUS.abilities).length * 70),
        240 + (Object.keys(CONFIG.SENTIUS.skills).length * 24)
      )),
      elements: {
        effects: "sentius-effects",
        inventory: "sentius-inventory"
      }
    });
  }
  ...
  /** @override */
  get template() {
    if ( !game.user.isGM && this.actor.limited ) return "systems/sentius/templates/actors/limited-sheet.hbs";
    return `systems/sentius/templates/actors/${this.actor.type}-sheet.hbs`;
  }
  ...
  async getData(options) {
    ...
    for ( const [a, abl] of Object.entries(context.abilities) ) {
      ...
      abl.hover = CONFIG.SENTIUS.proficiencyLevels[abl.proficient];
      abl.label = CONFIG.SENTIUS.abilities[a]?.label;
    ...
    const baseAbility = (prop, key) => {
      ...
      if ( src ) return src;
      if ( prop === "skills" ) src = CONFIG.SENTIUS.skills[key]?.ability;
      return src ?? "int";
    };
    ["skills", "tools"].forEach(prop => {
      for ( const [key, entry] of Object.entries(context[prop]) ) {
        entry.abbreviation = CONFIG.SENTIUS.abilities[entry.ability]?.abbreviation;
        entry.icon = this._getProficiencyIcon(entry.value);
        entry.hover = CONFIG.SENTIUS.proficiencyLevels[entry.value];
        entry.label = (prop === "skills") ? CONFIG.SENTIUS.skills[key]?.label : Trait.keyLabel(key, {trait: "tool"});
  ...
  _getLabels() {
    ...
    // Currency Labels
    labels.currencies = Object.entries(CONFIG.SENTIUS.currencies).reduce((obj, [k, c]) => {
      obj[k] = c.label;
      return obj;
    }, {});

    // Proficiency
    labels.proficiency = game.settings.get("sentius", "proficiencyModifier") === "dice"
  ...
  _getMovementSpeed(systemData, largestPrimary=false) {
    ...
    let speeds = [
      [movement.burrow, `${game.i18n.localize("SENTIUS.MovementBurrow")} ${movement.burrow}`],
      [movement.climb, `${game.i18n.localize("SENTIUS.MovementClimb")} ${movement.climb}`],
      [movement.fly, `${game.i18n.localize("SENTIUS.MovementFly")} ${movement.fly}${movement.hover ? ` (${game.i18n.localize("SENTIUS.MovementHover")})` : ""}`],
      [movement.swim, `${game.i18n.localize("SENTIUS.MovementSwim")} ${movement.swim}`]
    ];
    if ( largestPrimary ) {
      speeds.push([movement.walk, `${game.i18n.localize("SENTIUS.MovementWalk")} ${movement.walk}`]);
    }
  ...
  _getSenses(systemData) {
    ...
    for ( let [k, label] of Object.entries(CONFIG.SENTIUS.senses) ) {
      const v = senses[k] ?? 0;
      if ( v === 0 ) continue;
  ...
  _prepareTraits(systemData) {
    const traits = {};
    for ( const [trait, traitConfig] of Object.entries(CONFIG.SENTIUS.traits) ) {
      ...
      if ( data.bypasses?.size ) {
        values = values.filter(t => {
          if ( !CONFIG.SENTIUS.damageTypes[t]?.isPhysical ) return true;
          physical.push(t);
          return false;
        });
      }
      ...
      if ( physical.length ) {
        ...
        data.selected.physical = game.i18n.format("SENTIUS.DamagePhysicalBypasses", {
          ...
          bypassTypes: bypassFormatter.format(data.bypasses.reduce((acc, t) => {
            const v = CONFIG.SENTIUS.itemProperties[t];
      ...
      if ( (trait === "dr") && this.document.hasConditionEffect("petrification") ) {
        data.selected = { custom1: game.i18n.localize("SENTIUS.DamageAll") };
        data.cssClass = "";
      }
    }
    return traits;
  ...
  _prepareSpellbook(context, spells) {
    ...
    const sections = Object.entries(CONFIG.SENTIUS.spellPreparationModes).reduce((acc, [k, {order}]) => {
    ...
    const maxLevel = Array.fromRange(Object.keys(CONFIG.SENTIUS.spellLevels).length - 1, 1).reduce((max, i) => {
    ...
    if ( maxLevel > 0 ) {
      registerSection("spell0", 0, CONFIG.SENTIUS.spellLevels[0]);
      for (let lvl = 1; lvl <= maxLevel; lvl++) {
        const sl = `spell${lvl}`;
        registerSection(sl, lvl, CONFIG.SENTIUS.spellLevels[lvl], levels[sl]);
      }
    }

    // Create spellbook sections for all alternative spell preparation modes that have spell slots.
    for ( const [k, v] of Object.entries(CONFIG.SENTIUS.spellPreparationModes) ) {
      if ( !(k in levels) || !v.upcast || !levels[k].max ) continue;

      if ( !spellbook["0"] && v.cantrips ) registerSection("spell0", 0, CONFIG.SENTIUS.spellLevels[0]);
      const l = levels[k];
      const level = game.i18n.localize(`SENTIUS.SpellLevel${l.level}`);
    ...
    spells.forEach(spell => {
      ...
      // Spells from items
      if ( spell.getFlag("sentius", "cachedFor") ) {
        s = "item";
        if ( !spell.system.linkedActivity?.displayInSpellbook ) return;
        if ( !spellbook[s] ) {
          registerSection(null, s, game.i18n.localize("SENTIUS.CAST.SECTIONS.Spellbook"));
      ...
      // Sections for higher-level spells which the caster "should not" have, but spell items exist for
        ...
        registerSection(sl, s, CONFIG.SENTIUS.spellLevels[s], {levels: levels[sl]});
      }
  ...
  _filterItems(items, filters) {
    ...
    const spellSchools = new Set(Object.keys(CONFIG.SENTIUS.spellSchools));
   ...
   * @param {number} level  A proficiency mode defined in `CONFIG.SENTIUS.proficiencyLevels`.
  ...
  _disableOverriddenFields(html) {
    ...
    for ( const override of Object.keys(foundry.utils.flattenObject(this.actor.overrides)) ) {
      html.find(`input[name="${override}"],select[name="${override}"]`).each((i, el) => {
        el.disabled = true;
        el.dataset.tooltip = "SENTIUS.ActiveEffectOverrideWarning";
      });
      ...
      for ( const [key, regex] of Object.entries(proficiencyToggles) ) {
        ...
        if ( match ) {
          ...
          toggle.attr("data-tooltip", "SENTIUS.ActiveEffectOverrideWarning");
        }
      }

      const [, spell] = override.match(/system\.spells\.(spell\d)\.override/) || [];
      if ( spell ) {
        html.find(`.spell-max[data-level="${spell}"]`).attr("data-tooltip", "SENTIUS.ActiveEffectOverrideWarning");
      }
    }
  }
  ...
  async _onLevelChange(event) {
    ...
    if ( !game.settings.get("sentius", "disableAdvancements") ) {
  ...
  async _onDropActor(event, data) {
    const canPolymorph = game.user.isGM || (this.actor.isOwner && game.settings.get("sentius", "allowPolymorphing"));
    ...
    const rememberOptions = html => {
      ...
      const settings = foundry.utils.mergeObject(game.settings.get("sentius", "polymorphSettings") ?? {}, options);
      game.settings.set("sentius", "polymorphSettings", settings);
      return settings;
    };

    // Create and render the Dialog
    return new Dialog({
      title: game.i18n.localize("SENTIUS.PolymorphPromptTitle"),
      content: {
        options: game.settings.get("sentius", "polymorphSettings"),
        settings: CONFIG.SENTIUS.polymorphSettings,
        effectSettings: CONFIG.SENTIUS.polymorphEffectSettings,
      ...
      buttons: {
        accept: {
          ...
          label: game.i18n.localize("SENTIUS.PolymorphAcceptSettings"),
          callback: html => this.actor.transformInto(sourceActor, rememberOptions(html))
        },
        wildshape: {
          icon: CONFIG.SENTIUS.transformationPresets.wildshape.icon,
          label: CONFIG.SENTIUS.transformationPresets.wildshape.label,
          callback: html => this.actor.transformInto(sourceActor, foundry.utils.mergeObject(
            CONFIG.SENTIUS.transformationPresets.wildshape.options,
            { transformTokens: rememberOptions(html).transformTokens }
          ))
        },
        polymorph: {
          icon: CONFIG.SENTIUS.transformationPresets.polymorph.icon,
          label: CONFIG.SENTIUS.transformationPresets.polymorph.label,
          callback: html => this.actor.transformInto(sourceActor, foundry.utils.mergeObject(
            CONFIG.SENTIUS.transformationPresets.polymorph.options,
            { transformTokens: rememberOptions(html).transformTokens }
          ))
        },
        self: {
          icon: CONFIG.SENTIUS.transformationPresets.polymorphSelf.icon,
          label: CONFIG.SENTIUS.transformationPresets.polymorphSelf.label,
          callback: html => this.actor.transformInto(sourceActor, foundry.utils.mergeObject(
            CONFIG.SENTIUS.transformationPresets.polymorphSelf.options,
            { transformTokens: rememberOptions(html).transformTokens }
          ))
        },
        cancel: {
      ...
      classes: ["dialog", "sentius", "polymorph"],
      width: 900,
      template: "systems/sentius/templates/apps/polymorph-prompt.hbs"
    }).render(true);
  }
  ...
  async _onDropItemCreate(itemData, event) {
    ...
    if ( multipleAdvancements && !game.settings.get("sentius", "disableAdvancements") ) {
      ui.notifications.warn(game.i18n.format("SENTIUS.WarnCantAddMultipleAdvancements"));
  ...
  async _onDropSingleItem(itemData, event) {
    ...
    if ( this.constructor.unsupportedItemTypes.has(itemData.type) ) {
      ui.notifications.warn(game.i18n.format("SENTIUS.ActorWarningInvalidItem", {
    ...
    if ( this.actor.system.metadata?.supportsAdvancement && itemData.system.advancement?.length
        && !game.settings.get("sentius", "disableAdvancements") ) {
      ...
      if ( singleton && this.actor.itemTypes[itemData.type].length ) {
        ui.notifications.error(game.i18n.format("SENTIUS.ActorWarningSingleton", {
          itemType: game.i18n.localize(CONFIG.Item.typeLabels[itemData.type]),
          actorType: game.i18n.localize(CONFIG.Actor.typeLabels[this.actor.type])
        }));
        return false;
  ...
  _onDropSpell(itemData, event) {
    ...
    const spellcastKeys = Object.keys(CONFIG.SENTIUS.spellcastingTypes);
  ...
  _getHeaderButtons() {
    ...
    if ( this.actor.isPolymorphed ) {
      buttons.unshift({
        label: "SENTIUS.PolymorphRestoreTransformation",
