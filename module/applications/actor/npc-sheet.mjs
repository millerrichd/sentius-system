...
export default class ActorSheet5eNPC extends ActorSheet5e {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["sentius", "sheet", "actor", "npc"],
      width: 600
    });
  }
  ...
  _prepareItems(context) {
    ...
    const features = {
      weapons: { label: game.i18n.localize("SENTIUS.AttackPl"), items: [], hasActions: true,
        dataset: {type: "weapon", "weapon-type": "natural"} },
      actions: { label: game.i18n.localize("SENTIUS.ActionPl"), items: [], hasActions: true,
        dataset: {type: "feat", "activation.type": "action"} },
      passive: { label: game.i18n.localize("SENTIUS.Features"), items: [], dataset: {type: "feat"} },
      equipment: { label: game.i18n.localize("SENTIUS.Inventory"), items: [], dataset: {type: "loot"}}
    };

    // Start by classifying items into groups for rendering
    const maxLevelDelta = CONFIG.SENTIUS.maxLevel - (this.actor.system.details.level ?? 0);
    const [spells, other] = context.items.reduce((arr, item) => {
      ...
      if ( item.type === "class" ) ctx.availableLevels = Array.fromRange(CONFIG.SENTIUS.maxLevel, 1).map(level => {
  ...
  getArmorLabel() {
    ...
    const label = [];
    if ( ac.calc === "default" ) label.push(this.actor.armor?.name || game.i18n.localize("SENTIUS.ArmorClassUnarmored"));
    else label.push(game.i18n.localize(CONFIG.SENTIUS.armorClasses[ac.calc].label));
