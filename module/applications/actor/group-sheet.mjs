...
export default class GroupActorSheet extends ActorSheetMixin(ActorSheet) {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["sentius", "sheet", "actor", "group"],
      template: "systems/sentius/templates/actors/group-sheet.hbs",
      tabs: [{navSelector: ".tabs", contentSelector: ".sheet-body", initial: "members"}],
      scrollY: ["sentius-inventory .inventory-list"],
      width: 620,
      height: 620,
      elements: {
  ...
  async getData(options={}) {
    ...
    // XP
    if ( game.settings.get("sentius", "levelingMode") !== "noxp" ) context.xp = context.system.details.xp;
    ...
    // Text labels
    context.labels = {
      currencies: Object.entries(CONFIG.SENTIUS.currencies).reduce((obj, [k, c]) => {
        obj[k] = c.label;
        return obj;
      }, {})
    };
  ...
  #getSummary(stats) {
    ...
    const members = [];
    if ( stats.nMembers ) {
      members.push(`${stats.nMembers} ${game.i18n.localize(`SENTIUS.Group.Member.${rule.select(stats.nMembers)}`)}`);
    }
    if ( stats.nVehicles ) {
      members.push(`${stats.nVehicles} ${game.i18n.localize(`SENTIUS.Group.Vehicle.${rule.select(stats.nVehicles)}`)}`);
    }
    if ( !members.length ) return game.i18n.localize("SENTIUS.GroupSummaryEmpty");
    return game.i18n.format("SENTIUS.GroupSummary", {members: formatter.format(members)});
  }
  ...
  #prepareMembers() {
    ...
    const displayXP = game.settings.get("sentius", "levelingMode") !== "noxp";
    for ( const [index, memberData] of this.object.system.members.entries() ) {
      ...
      m.hp.color = sentius.documents.Actor5e.getHPColor(m.hp.current, m.hp.max).css;
  ...
  #prepareMovementSpeed() {
    ...
    let speeds = [
      [movement.land, `${game.i18n.localize("SENTIUS.MovementLand")} ${movement.land}`],
      [movement.water, `${game.i18n.localize("SENTIUS.MovementWater")} ${movement.water}`],
      [movement.air, `${game.i18n.localize("SENTIUS.MovementAir")} ${movement.air}`]
    ];
  ...
  _onClickActionButton(event) {
    ...
    switch ( button.dataset.action ) {
      case "award":
        const award = new Award(this.object, { savedDestinations: this.actor.getFlag("sentius", "awardDestinations") });
        award.render(true);
        break;
      case "longRest":
