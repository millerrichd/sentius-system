...
export default class ShortRestDialog extends Dialog {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "systems/sentius/templates/apps/short-rest.hbs",
      classes: ["sentius", "dialog"],
      height: "auto"
    });
  }
  ...
  getData() {
    ...
    if ( this.actor.type === "npc" ) {
      ...
      context.hdOptions = [{
        ...
        label: `${context.denomination} (${hd.value} ${game.i18n.localize("SENTIUS.available")})`
      }];
    }
      ...
      context.hdOptions = Object.entries(context.availableHD).map(([value, number]) => ({
        value, label: `${value} (${number} ${game.i18n.localize("SENTIUS.available")})`
      }));
    }

    // Determine rest type
    const variant = game.settings.get("sentius", "restVariant");
  ...
  static async shortRestDialog({ actor }={}) {
    return new Promise((resolve, reject) => {
      const dlg = new this(actor, {
        title: `${game.i18n.localize("SENTIUS.ShortRest")}: ${actor.name}`,
        buttons: {
          rest: {
            icon: '<i class="fas fa-bed"></i>',
            label: game.i18n.localize("SENTIUS.Rest"),
            callback: html => {
