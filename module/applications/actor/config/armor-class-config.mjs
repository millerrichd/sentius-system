...
export default class ArmorClassConfig extends BaseConfigSheet {
  ...
  static PARTS = {
    config: {
      template: "systems/sentius/templates/actors/config/armor-class-config.hbs"
    }
  };
  ...
  /** @override */
  get title() {
    return game.i18n.localize("SENTIUS.ArmorClass");
  }
  ...
  async _preparePartContext(partId, context, options) {
    ...
    context.calculationOptions = Object.entries(CONFIG.SENTIUS.armorClasses).reduce((arr, [value, config]) => {
    ...
    const config = CONFIG.SENTIUS.armorClasses[context.source.calc];
    context.formula = {
