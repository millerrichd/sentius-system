...
export default class DamagesConfig extends TraitsConfig {
  ...
  static PARTS = {
    traits: {
      template: "systems/sentius/templates/actors/config/damages-config.hbs"
    }
  };
  ...
  /** @override */
  get otherLabel() {
    return game.i18n.localize("SENTIUS.DamageTypes");
  }
  ...
  async _preparePartContext(partId, context, options) {
    ...
    context.bypasses = new SelectChoices(Object.entries(CONFIG.SENTIUS.itemProperties).reduce((obj, [k, v]) => {
    ...
    if ( this.options.trait === "dm" ) {
      ...
      context.bypassHint = "SENTIUS.DamageModification.BypassHint";
      context.hint = "SENTIUS.DamageModification.Hint";
      context.value.field = new FormulaField({ determinstic: true });
      context.value.key = "amount";
    } else {
      context.bypassHint = "SENTIUS.DamagePhysicalBypassHint";
  ...
  _processChoice(data, key, choice, categoryChosen=false) {
    ...
    const config = CONFIG.SENTIUS.damageTypes[key];
