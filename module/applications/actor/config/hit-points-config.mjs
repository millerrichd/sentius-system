...
export default class HitPointsConfig extends BaseConfigSheet {
  ...
  static PARTS = {
    config: {
      template: "systems/sentius/templates/actors/config/hit-points-config.hbs"
    }
  };
  ...
  /** @override */
  get title() {
    return game.i18n.localize("SENTIUS.HitPoints");
  }
  ...
  async _preparePartContext(partId, context, options) {
    ...
    const ability = CONFIG.SENTIUS.abilities[CONFIG.SENTIUS.defaultAbilities.hitPoints];
    const mod = this.document.system.abilities?.[CONFIG.SENTIUS.defaultAbilities.hitPoints]?.mod ?? 0;
  ...
  static async #rollFormula(event, target) {
    try {
      ...
      ui.notifications.error("SENTIUS.HPFormulaError", { localize: true });
      throw error;
    }
  }
