...
export default function ItemSheetV2Mixin(Base) {
  return class ItemSheetV2 extends DocumentSheetV2Mixin(Base) {
    /** @override */
    static TABS = [
      { tab: "contents", label: "SENTIUS.Contents", condition: this.itemHasContents.bind(this) },
      { tab: "description", label: "SENTIUS.Description" },
      { tab: "details", label: "SENTIUS.Details", condition: this.isItemIdentified.bind(this) },
      { tab: "activities", label: "SENTIUS.ACTIVITY.Title.other", condition: this.itemHasActivities.bind(this) },
      { tab: "effects", label: "SENTIUS.Effects", condition: this.itemHasEffects.bind(this) },
      { tab: "advancement", label: "SENTIUS.AdvancementTitle", condition: this.itemHasAdvancements.bind(this) }
    ];
    ...
    async _render(force=false, options={}) {
      ...
      if ( identified ) {
        ...
        const label = isIdentified ? "SENTIUS.Identified" : "SENTIUS.Unidentified.Title";
      ...
      if ( equipped ) {
        ...
        const label = isEquipped ? "SENTIUS.Equipped" : "SENTIUS.Unequipped";
    ...
    async getData(options) {
      ...
      context.inputs = { ...foundry.applications.fields, ...sentius.applications.fields };
      ...
      context.properties = {
        ...
        options: (validProperties ?? []).reduce((arr, k) => {
          const { label } = CONFIG.SENTIUS.itemProperties[k];
      ...
      if ( ["feat", "loot", "consumable"].includes(this.item.type) ) {
        ...
        const itemTypes = CONFIG.SENTIUS[`${name}Types`][this.item.system.type.value];
    ...
    _onCreateChild() {
      ...
      if ( activeTab === "advancement" ) {
        return game.sentius.applications.advancement.AdvancementSelection.createDialog(this.item);
      }

      if ( activeTab === "activities" ) {
        return sentius.documents.activity.UtilityActivity.createDialog({}, {
          parent: this.item,
          types: Object.entries(CONFIG.SENTIUS.activityTypes).filter(([, { configurable }]) => {
    ...
    static itemHasContents(item) {
      return item.system instanceof sentius.dataModels.item.ContainerData;
    }
