...
export default class ItemSheet5e extends ItemSheet {
  ...
  /** @inheritDoc */
  get template() {
    return `systems/sentius/templates/items/${this.item.type}.hbs`;
  }
  ...
  async getData(options) {
    ...
    foundry.utils.mergeObject(context, {
      ...
      isFormulaRecharge: !!CONFIG.SENTIUS.limitedUsePeriods[item.system.uses?.per]?.formula,
      isCostlessAction: item.system.activation?.type in CONFIG.SENTIUS.staticAbilityActivationTypes,
    ...
    if ( !item.isEmbedded && foundry.utils.isEmpty(context.abilityConsumptionTargets) ) {
      context.abilityConsumptionHint = (this.item.system.consume?.type === "attribute")
        ? "SENTIUS.ConsumeHint.Attribute" : "SENTIUS.ConsumeHint.Item";
    }

    if ( ("properties" in item.system) && (item.type in CONFIG.SENTIUS.validProperties) ) {
      context.properties = item.system.validProperties.reduce((obj, k) => {
        const v = CONFIG.SENTIUS.itemProperties[k];
        obj[k] = {
          label: v.label,
    ...
    if ( ["feat", "loot", "consumable"].includes(item.type) ) {
      ...
      const itemTypes = CONFIG.SENTIUS[`${name}Types`][item.system.type.value];
      if ( itemTypes ) {
  ...
  async _getItemBaseTypes() {
    const baseIds = this.item.type === "equipment" ? {
      ...CONFIG.SENTIUS.armorIds,
      ...CONFIG.SENTIUS.shieldIds
    } : CONFIG.SENTIUS[`${this.item.type}Ids`];
  ...
  _getItemStatus() {
    switch ( this.item.type ) {
      case "class":
        return game.i18n.format("SENTIUS.LevelCount", {ordinal: this.item.system.levels.ordinalString()});
      case "equipment":
      case "weapon":
        return game.i18n.localize(this.item.system.equipped ? "SENTIUS.Equipped" : "SENTIUS.Unequipped");
      case "feat":
      case "consumable":
        return this.item.system.type.label;
      case "spell":
        return CONFIG.SENTIUS.spellPreparationModes[this.item.system.preparation.mode]?.label;
      case "tool":
        return CONFIG.SENTIUS.proficiencyLevels[this.item.system.prof?.multiplier || 0];
    }
    return null;
  }
  ...
  _getItemProperties() {
    ...
    switch ( this.item.type ) {
      ...
      case "weapon":
        ...
        const ip = CONFIG.SENTIUS.itemProperties;
        const vp = CONFIG.SENTIUS.validProperties[this.item.type];
        this.item.system.properties.forEach(k => {
          if ( vp.has(k) ) props.push(ip[k].label);
        });
        break;
      case "equipment":
        props.push(CONFIG.SENTIUS.equipmentTypes[this.item.system.type.value]);
    ...
    if ( this.item.system.actionType ) {
      props.push(CONFIG.SENTIUS.itemActionTypes[this.item.system.actionType]);
    }
  ...
  _getSubmitData(updateData={}) {
    ...
    if ( uses?.max ) {
      ...
      if ( !maxRoll.isDeterministic ) {
        ...
        ui.notifications.error(game.i18n.format("SENTIUS.FormulaCannotContainDiceError", {
          name: game.i18n.localize("SENTIUS.LimitedUses")
        }));
        return {};
      }
    }
    ...
    if ( duration?.value ) {
      ...
      if ( !durationRoll.isDeterministic ) {
        ...
        ui.notifications.error(game.i18n.format("SENTIUS.FormulaCannotContainDiceError", {
          name: game.i18n.localize("SENTIUS.Duration")
        }));
        return {};
      }
    }
    ...
    if ( formData.system?.identifier && !sentius.utils.validators.isValidIdentifier(formData.system.identifier) ) {
      ...
      ui.notifications.error("SENTIUS.IdentifierError", {localize: true});
      return {};
    }
  ...
  activateListeners(html) {
    ...
    if ( this.isEditable ) {
      ...
      for ( const override of this._getItemOverrides() ) {
        for ( const element of html[0].querySelectorAll(`[name="${override}"]`) ) {
          ...
          element.dataset.tooltip = "SENTIUS.ENCHANTMENT.Warning.Override";
        }
     ...
     * @function sentius.getItemAdvancementContext
     * @memberof hookEvents
     * @param {jQuery} html                      The HTML element to which the context options are attached.
     * @param {ContextMenuEntry[]} entryOptions  The context menu entries.
     */
    Hooks.call("sentius.getItemAdvancementContext", html, contextOptions);
  ...
  _getAdvancementContextMenuOptions() {
    ...
    return [
      {
        name: "SENTIUS.AdvancementControlEdit",
        icon: "<i class='fas fa-edit fa-fw'></i>",
        condition,
        callback: li => this._onAdvancementAction(li[0], "edit")
      },
      {
        name: "SENTIUS.AdvancementControlDuplicate",
      ...
      {
        name: "SENTIUS.AdvancementControlDelete",
  ...
  _onDrop(event) {
     ...
     * @function sentius.dropItemSheetData
     * @memberof hookEvents
     ...
     */
    const allowed = Hooks.call("sentius.dropItemSheetData", item, this, data);
  ...
  async _onDropAdvancement(event, data) {
    ...
    advancements = advancements.filter(a => {
      const validItemTypes = CONFIG.SENTIUS.advancementTypes[a.constructor.typeName]?.validItemTypes
    ...
    if ( this.item.actor?.system.metadata?.supportsAdvancement && !game.settings.get("sentius", "disableAdvancements") ) {
  ...
  _onAdvancementAction(target, action) {
    ...
    switch (action) {
      case "add": return game.sentius.applications.advancement.AdvancementSelection.createDialog(this.item);
