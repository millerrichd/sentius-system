...
export default class ContainerSheet extends ItemSheet5e {
  ...
  /** @inheritDoc */
  get template() {
    return "systems/sentius/templates/items/container.hbs";
  }
  ...
  _onDrop(event) {
    ...
    if ( Hooks.call("sentius.dropItemSheetData", this.item, this, data) === false ) return;
  ...
  async _onDropFolder(event, data) {
    ...
    if ( recursiveWarning ) ui.notifications.warn("SENTIUS.ContainerRecursiveError", { localize: true });
  ...
  async _onDropItem(event, data) {
    ...
    const parentContainers = await this.item.system.allContainers();
    if ( (this.item.uuid === item.uuid) || parentContainers.includes(item) ) {
      ui.notifications.error("SENTIUS.ContainerRecursiveError", { localize: true });
      return;
    }
