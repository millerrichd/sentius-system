...
export default class CreateScrollDialog extends Dialog5e {
  ...
  static DEFAULT_OPTIONS = {
    ...
    window: {
      title: "SENTIUS.Scroll.CreateScroll",
    ...
    buttons: [{
      action: "create",
      label: "SENTIUS.Scroll.CreateScroll",
  ...
  static PARTS = {
    ...super.PARTS,
    content: {
      template: "systems/sentius/templates/apps/spell-scroll-dialog.hbs"
    }
  };
  ...
  async _prepareContentContext(context, options) {
    ...
    context.fields = [{
      field: new StringField({
        label: game.i18n.localize("SENTIUS.Scroll.Explanation.Label"),
        hint: game.i18n.localize("SENTIUS.Scroll.Explanation.Hint")
      }),
      name: "explanation",
      options: [
        { value: "full", label: game.i18n.localize("SENTIUS.Scroll.Explanation.Complete") },
        { value: "reference", label: game.i18n.localize("SENTIUS.Scroll.Explanation.Reference") },
        { value: "none", label: game.i18n.localize("SENTIUS.None") }
      ],
      value: this.config.explanation ?? "reference"
    }, {
      field: new NumberField({ label: game.i18n.localize("SENTIUS.SpellLevel") }),
      name: "level",
      options: Object.entries(CONFIG.SENTIUS.spellLevels)
        .map(([value, label]) => ({ value, label }))
        .filter(l => Number(l.value) >= this.spell.system.level),
      value: this.config.level ?? this.spell.system.level
    }];
    context.values = {
      bonus: new NumberField({ label: game.i18n.localize("SENTIUS.BonusAttack") }),
      dc: new NumberField({ label: game.i18n.localize("SENTIUS.Scroll.SaveDC") })
    };
    context.valuePlaceholders = {};
    for ( const level of Array.fromRange(this.config.level + 1).reverse() ) {
      context.valuePlaceholders = CONFIG.SENTIUS.spellScrollValues[level];
  ...
  static async #handleFormSubmission(event, form, formData) {
    ...
    await this.close({ sentius: { submitted: true } });
  }
  ...
  /** @override */
  _onClose(options={}) {
    if ( !options.sentius?.submitted ) this.#config = null;
  }
