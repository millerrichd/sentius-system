...
export default class StartingEquipmentConfig extends DocumentSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["sentius", "starting-equipment"],
      dragDrop: [{ dragSelector: ".drag-bar", dropSelector: "form" }],
      template: "systems/sentius/templates/apps/starting-equipment-config.hbs",
      width: 480,
      height: "auto",
      sheetConfig: false,
      closeOnSubmit: false,
  ...
  /** @inheritDoc */
  get title() {
    return `${game.i18n.localize("SENTIUS.StartingEquipment.Action.Configure")}: ${this.document.name}`;
  }
  ...
  async _onDrop(event) {
    ...
    if ( !item.system.constructor._schemaTemplates?.includes(PhysicalItemTemplate) ) {
      ui.notifications.error(game.i18n.format("SENTIUS.StartingEquipment.Warning.ItemTypeInvalid", {
  ...
  _onSortEntry(event, data) {
    ...
    if ( dropEntry?.type in EquipmentEntryData.GROUPING_TYPES ) {
      ...
      if ( depth > 3 ) {
        ui.notifications.warn("SENTIUS.StartingEquipment.Warning.Depth", { localize: true });
        return;
      }
