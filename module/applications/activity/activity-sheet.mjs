...
export default class ActivitySheet extends Application5e {
  ...
  static PARTS = {
    ...
    identity: {
      template: "systems/sentius/templates/activity/identity.hbs",
      templates: [
        "systems/sentius/templates/activity/parts/activity-identity.hbs"
      ]
    },
    activation: {
      template: "systems/sentius/templates/activity/activation.hbs",
      templates: [
        "systems/sentius/templates/activity/parts/activity-time.hbs",
        "systems/sentius/templates/activity/parts/activity-targeting.hbs",
        "systems/sentius/templates/activity/parts/activity-consumption.hbs"
      ]
    },
    effect: {
      template: "systems/sentius/templates/activity/effect.hbs",
      templates: [
        "systems/sentius/templates/activity/parts/activity-effects.hbs"
      ]
    }
  };
  ...
  async _prepareActivationContext(context) {
    ...
    context.activationTypes = [
      ...Object.entries(CONFIG.SENTIUS.activityActivationTypes).map(([value, config]) => ({
        value,
        label: game.i18n.localize(config.label),
        group: game.i18n.localize(config.group)
      })),
      { value: "", label: game.i18n.localize("SENTIUS.NoneActionLabel") }
    ];
    context.affectsPlaceholder = game.i18n.localize(
      `SENTIUS.TARGET.Count.${context.data.target?.template?.type ? "Every" : "Any"}`
    );
    context.durationUnits = [
      { value: "inst", label: game.i18n.localize("SENTIUS.TimeInst") },
      ...Object.entries(CONFIG.SENTIUS.scalarTimePeriods).map(([value, label]) => ({
        value, label, group: game.i18n.localize("SENTIUS.DurationTime")
      })),
      ...Object.entries(CONFIG.SENTIUS.permanentTimePeriods).map(([value, label]) => ({
        value, label, group: game.i18n.localize("SENTIUS.DurationPermanent")
      })),
      { value: "spec", label: game.i18n.localize("SENTIUS.Special") }
    ];
    context.rangeUnits = [
      ...
      ...Object.entries(CONFIG.SENTIUS.movementUnits).map(([value, { label }]) => ({
        value, label, group: game.i18n.localize("SENTIUS.RangeDistance")
      }))
    ];
    ...
    const consumptionTypeOptions = Array.from(this.activity.validConsumptionTypes).map(value => ({
      value,
      label: CONFIG.SENTIUS.activityConsumptionTypes[value].label
    }));
    context.consumptionTargets = context.source.consumption.targets.map((data, index) => {
      const typeConfig = CONFIG.SENTIUS.activityConsumptionTypes[data.type] ?? {};
      ...
      return {
        ...
        scalingModes: canScale ? [
          { value: "", label: game.i18n.localize("SENTIUS.CONSUMPTION.Scaling.None") },
          { value: "amount", label: game.i18n.localize("SENTIUS.CONSUMPTION.Scaling.Amount") },
          ...(typeConfig.scalingModes ?? []).map(({ value, label }) => ({ value, label: game.i18n.localize(label) }))
        ] : null,
        showTargets: "validTargets" in typeConfig,
        targetPlaceholder: data.type === "itemUses" ? game.i18n.localize("SENTIUS.CONSUMPTION.Target.ThisItem") : null,
    ...
    context.recoveryPeriods = [
      ...Object.entries(CONFIG.SENTIUS.limitedUsePeriods)
        ...
        .map(([value, config]) => ({
          value, label: config.label, group: game.i18n.localize("SENTIUS.DurationTime")
        })),
      { value: "recharge", label: game.i18n.localize("SENTIUS.USES.Recovery.Recharge.Label") }
    ];
    context.recoveryTypes = [
      { value: "recoverAll", label: game.i18n.localize("SENTIUS.USES.Recovery.Type.RecoverAll") },
      { value: "loseAll", label: game.i18n.localize("SENTIUS.USES.Recovery.Type.LoseAll") },
      { value: "formula", label: game.i18n.localize("SENTIUS.USES.Recovery.Type.Formula") }
    ];
    context.usesRecovery = context.source.uses.recovery.map((data, index) => ({
      ...
      formulaOptions: data.period === "recharge" ? Array.fromRange(5, 2).reverse().map(min => ({
        value: min,
        label: game.i18n.format("SENTIUS.USES.Recovery.Recharge.Range", {
  ...
  async _prepareEffectContext(context) {
    ...
    if ( context.activity.damage?.parts ) {
      const scalingOptions = [
        ...
        ...Object.entries(CONFIG.SENTIUS.damageScalingModes).map(([value, config]) => ({ value, label: config.label }))
      ];
      let indexOffset = 0;
      context.damageParts = context.activity.damage.parts.map((data, index) => {
        ...
        const part = {
          ...
          scalingOptions,
          typeOptions: Object.entries(CONFIG.SENTIUS.damageTypes).map(([value, config]) => ({
  ...
  _getTabs() {
    return this._markTabs({
      ...
      activation: {
        ...
        label: "SENTIUS.ACTIVITY.SECTIONS.Activation",
        tabs: {
          time: {
            id: "time", group: "activation", icon: "fa-solid fa-clock",
            label: "SENTIUS.ACTIVITY.SECTIONS.Time"
          },
          consumption: {
          ...
          targeting: {
            ...
            label: "SENTIUS.TARGET.FIELDS.target.label"
          }
        }
      },
      effect: {
  ...
  _canRender(options) {
    if ( !this.isVisible ) throw new Error(game.i18n.format("SHEETS.DocumentSheetPrivate", {
      type: game.i18n.localize("SENTIUS.ACTIVITY.Title.one")
    }));
  }
  ...
  static #addRecovery(event, target) {
    const periods = new Set(
      Object.entries(CONFIG.SENTIUS.limitedUsePeriods).filter(([, config]) => !config.deprecated).map(([k]) => k)
    );
  ...
  _prepareSubmitData(event, formData) {
    ...
    this.element.querySelectorAll("fieldset legend :is(input, select, sentius-checkbox)").forEach(input => {
