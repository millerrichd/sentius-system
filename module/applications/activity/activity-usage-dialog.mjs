...
export default class ActivityUsageDialog extends Dialog5e {
  ...
  static PARTS = {
    scaling: {
      template: "systems/sentius/templates/activity/activity-usage-scaling.hbs"
    },
    concentration: {
      template: "systems/sentius/templates/activity/activity-usage-concentration.hbs"
    },
    consumption: {
      template: "systems/sentius/templates/activity/activity-usage-consumption.hbs"
    },
    creation: {
      template: "systems/sentius/templates/activity/activity-usage-creation.hbs"
    },
    footer: {
  ...
  async _prepareContext(options) {
    if ( "scaling" in this.config ) this.#item = this.#item.clone({ "flags.sentius.scaling": this.config.scaling });
    return {
  ...
  async _prepareConcentrationContext(context, options) {
    if ( !this.activity.requiresConcentration || game.settings.get("sentius", "disableConcentration")
      || !this._shouldDisplay("concentration") ) return context;
    context.hasConcentration = true;
    context.notes = [];

    context.fields = [{
      field: new BooleanField({ label: game.i18n.localize("SENTIUS.Concentration") }),
    ...
    if ( this.config.concentration?.begin ) {
      const existingConcentration = Array.from(this.actor.concentration.effects).map(effect => {
        ...
        return {
          ...
          label: data?.data?.name ?? this.actor.items.get(data?.id)?.name
            ?? game.i18n.localize("SENTIUS.ConcentratingItemless")
        };
      });
      if ( existingConcentration.length ) {
        ...
        context.fields.push({
          field: new StringField({ label: game.i18n.localize("SENTIUS.ConcentratingEnd") }),
        ...
        context.notes.push({
          type: "info", message: game.i18n.localize(`SENTIUS.ConcentratingWarnLimit${optional ? "Optional" : ""}`)
        });
      } else if ( !this.actor.system.attributes?.concentration?.limit ) {
        context.notes.push({
          type: "warn", message: game.i18n.localize("SENTIUS.ConcentratingWarnLimitZero")
        });
      }
    }
  ...
  async _prepareConsumptionContext(context, options) {
    ...
    if ( (this.activity.activation.type === "legendary") && this.actor.system.resources?.legact
      ...
      context.fields.push({
        field: new BooleanField({
          label: game.i18n.format("SENTIUS.CONSUMPTION.Type.Action.Prompt", {
            type: game.i18n.localize("SENTIUS.LegAct")
          }),
          hint: game.i18n.format("SENTIUS.CONSUMPTION.Type.Action.PromptHint", {
            available: game.i18n.format(
              `SENTIUS.ACTIVATION.Type.Legendary.Counted.${pr.select(this.actor.system.resources.legact.value)}`,
              { number: `<strong>${formatNumber(this.actor.system.resources.legact.value)}</strong>` }
            ),
            cost: game.i18n.format(
              `SENTIUS.ACTIVATION.Type.Legendary.Counted.${pr.select(this.activity.activation.value)}`,
    ...
    if ( this.activity.requiresSpellSlot && this.activity.consumption.spellSlot
      ...
      field: new BooleanField({ label: game.i18n.localize("SENTIUS.SpellCastConsume") }),
      input: context.inputs.createCheckboxInput,
  ...
  async _prepareCreationContext(context, options) {
    ...
    if ( this.activity.target?.template?.type && this._shouldDisplay("create.measuredTemplate") ) {
      ...
      context.template = {
        field: new BooleanField({ label: game.i18n.localize("SENTIUS.TARGET.Action.PlaceTemplate") }),
  ...
  async _prepareFooterContext(context, options) {
    context.buttons = [{
      ...
      label: this.options.button.label ?? `SENTIUS.AbilityUse${this.activity.isSpell ? "Cast" : "Use"}`,
      type: "button"
    }];
    return context;
  }
  ...
  async _prepareScalingContext(context, options) {
    ...
    if ( this.activity.requiresSpellSlot && context.linkedActivity && (this.config.scaling !== false) ) {
      ...
      const spellSlotOptions = Object.entries(CONFIG.SENTIUS.spellLevels).map(([level, label]) => {
        if ( (Number(level) < minimumLevel) || (Number(level) > maximumLevel) ) return null;
        return { value: `spell${level}`, label };
      }).filter(_ => _);
      context.spellSlots = {
        field: new StringField({ label: game.i18n.localize("SENTIUS.SpellCastUpcast") }),
        name: "spell.slot",
      ...
      const spellSlotOptions = Object.entries(this.actor.system.spells).map(([value, slot]) => {
        ...
        if ( slot.type === "leveled" ) {
          label = game.i18n.format("SENTIUS.SpellLevelSlot", { level: slot.label, n: slot.value });
        } else {
          label = game.i18n.format(`SENTIUS.SpellLevel${slot.type.capitalize()}`, { level: slot.level, n: slot.value });
        }
      ...
      if ( !spellSlotOptions.some(o => !o.disabled) ) context.notes.push({
        type: "warn", message: game.i18n.format("SENTIUS.SpellCastNoSlotsLeft", {
          name: this.item.name
        })
      });
    }
      ...
      if ( max > 1 ) context.scaling = {
        field: new NumberField({ min: 1, max, label: game.i18n.localize("SENTIUS.ScalingValue") }),
        name: "scalingValue",
