...
export default class ActivityChoiceDialog extends Application5e {
  ...
  static PARTS = {
    activities: {
      template: "systems/sentius/templates/activity/activity-choices.hbs"
    }
  };
  ...
  async _prepareContext(options) {
    let controlHint;
    if ( game.settings.get("sentius", "controlHints") ) {
      controlHint = game.i18n.localize("SENTIUS.Controls.Activity.FastForwardHint");
      controlHint = controlHint.replace(
        "<left-click>",
        `<img src="systems/sentius/icons/svg/mouse-left.svg" alt="${game.i18n.localize("SENTIUS.Controls.LeftClick")}">`
      );
    }
    const activities = this.#item.system.activities
      .filter(a => !this.#item.getFlag("sentius", "riders.activity")?.includes(a.id) && a.canUse)
