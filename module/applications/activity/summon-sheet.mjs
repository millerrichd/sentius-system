...
export default class SummonSheet extends ActivitySheet {
  ...
  static PARTS = {
    ...super.PARTS,
    identity: {
      template: "systems/sentius/templates/activity/summon-identity.hbs",
      templates: super.PARTS.identity.templates
    },
    effect: {
      template: "systems/sentius/templates/activity/summon-effect.hbs",
      templates: [
        "systems/sentius/templates/activity/parts/activity-effects.hbs",
        "systems/sentius/templates/activity/parts/summon-changes.hbs",
        "systems/sentius/templates/activity/parts/summon-profiles.hbs"
      ]
    }
  };
  ...
  async _prepareEffectContext(context) {
    ...
    context.abilityOptions = [
      { value: "", label: this.activity.isSpell ? game.i18n.localize("SENTIUS.Spellcasting") : "" },
      { rule: true },
      ...Object.entries(CONFIG.SENTIUS.abilities).map(([value, { label }]) => ({ value, label }))
    ];
    context.creatureSizeOptions = Object.entries(CONFIG.SENTIUS.actorSizes).map(([value, config]) => ({
      value, label: config.label, selected: this.activity.creatureSizes.has(value)
    }));
    context.creatureTypeOptions = Object.entries(CONFIG.SENTIUS.creatureTypes).map(([value, config]) => ({
      value, label: config.label, selected: this.activity.creatureTypes.has(value)
    }));

    context.profileModes = [
      { value: "", label: game.i18n.localize("SENTIUS.SUMMON.FIELDS.summon.mode.Direct") },
      { value: "cr", label: game.i18n.localize("SENTIUS.SUMMON.FIELDS.summon.mode.CR") }
    ];
  ...
  _getTabs() {
    ...
    tabs.effect.label = "SENTIUS.SUMMON.SECTIONS.Summoning";
    ...
    tabs.effect.tabs = this._markTabs({
      profiles: {
        ...
        label: "SENTIUS.SUMMON.SECTIONS.Profiles"
      },
      changes: {
        id: "changes", group: "effect", icon: "fa-solid fa-sliders",
        label: "SENTIUS.SUMMON.SECTIONS.Changes"
      }
    });
    return tabs;
  }
