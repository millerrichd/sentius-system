...
export default class JournalSpellListPageSheet extends JournalPageSheet {
  ...
  /** @inheritDoc */
  get template() {
    if ( this.options.displayAsTable ) return "systems/sentius/templates/journal/page-spell-list-table.hbs";
    return `systems/sentius/templates/journal/page-spell-list-${this.isEditable ? "edit" : "view"}.hbs`;
  }
    ...
    for ( const data of context.spells ) {
      ...
      switch ( context.grouping ) {
        case "level":
          ...
          section = context.sections[level] ??= { header: CONFIG.SENTIUS.spellLevels[level], spells: [] };
          break;
        case "school":
          const school = spell.system.school;
          section = context.sections[school] ??= { header: CONFIG.SENTIUS.spellSchools[school]?.label, spells: [] };
          break;
        case "alphabetical":
