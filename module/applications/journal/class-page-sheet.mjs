...
export default class JournalClassPageSheet extends JournalPageSheet {
  ...
  /** @inheritDoc */
  get template() {
    return `systems/sentius/templates/journal/page-${this.document.type}-${this.isEditable ? "edit" : "view"}.hbs`;
  }
  ...
  async getData(options) {
    ...
    context.styleOptions = [
      { value: "", label: game.i18n.localize("JOURNALENTRYPAGE.SENTIUS.Class.Style.Inferred") },
      { rule: true },
      { value: "2024", label: game.i18n.localize("JOURNALENTRYPAGE.SENTIUS.Class.Style.Modern") },
      { value: "2014", label: game.i18n.localize("JOURNALENTRYPAGE.SENTIUS.Class.Style.Legacy") }
    ];
    ...
    if ( linked.system.primaryAbility ) {
      ...
      ).format(Array.from(linked.system.primaryAbility.value).map(v => CONFIG.SENTIUS.abilities[v]?.label));
    }
  ...
  async _getTable(item, { initialLevel=1, modernStyle }={}) {
    ...
    const headers = [[{content: game.i18n.localize("SENTIUS.Level")}]];
    if ( item.type === "class" ) headers[0].push({content: game.i18n.localize("SENTIUS.ProficiencyBonus")});
    if ( hasFeatures ) headers[0].push({content: game.i18n.localize("SENTIUS.Features")});
    ...
    const rows = [];
    for ( const level of Array.fromRange((CONFIG.SENTIUS.maxLevel - (initialLevel - 1)), initialLevel) ) {
      let features = [];
      for ( const advancement of item.advancement.byLevel[level] ) {
        switch ( advancement.constructor.typeName ) {
          case "AbilityScoreImprovement":
            features.push(game.i18n.localize("SENTIUS.ADVANCEMENT.AbilityScoreImprovement.Title"));
            continue;
          case "ItemGrant":
  ...
  async _getSpellProgression(item) {
    ...
    if ( spellcasting.type === "leveled" ) {
      const spells = {};
      const maxSpellLevel = CONFIG.SENTIUS.SPELL_SLOT_TABLE[CONFIG.SENTIUS.SPELL_SLOT_TABLE.length - 1].length;
      Array.fromRange(maxSpellLevel, 1).forEach(l => spells[`spell${l}`] = {});

      let largestSlot;
      for ( const level of Array.fromRange(CONFIG.SENTIUS.maxLevel, 1).reverse() ) {
      ...
      table.headers = [
        [{content: game.i18n.localize("JOURNALENTRYPAGE.SENTIUS.Class.SpellSlotsPerSpellLevel"), colSpan: largestSlot}],
      ...
      table.headers = [[
        { content: game.i18n.localize("JOURNALENTRYPAGE.SENTIUS.Class.SpellSlots") },
        { content: game.i18n.localize("JOURNALENTRYPAGE.SENTIUS.Class.SpellSlotLevel") }
      ]];
      table.cols = [{class: "spellcasting", span: 2}];

      // Loop through each level, gathering "Spell Slots" & "Slot Level" for each one
      for ( const level of Array.fromRange(CONFIG.SENTIUS.maxLevel, 1) ) {
       ...
       * The actual hook names include the spellcasting type (e.g. `sentius.buildPsionicSpellcastingTable`).
       ...
       * @function sentius.buildSpellcastingTable
       * @memberof hookEvents
       */
      Hooks.callAll(
        `sentius.build${spellcasting.type.capitalize()}SpellcastingTable`, table, item, spellcasting
      );
    }
  ...
  async _getOptionalTable(item, { modernStyle }) {
    const headers = [[
      { content: game.i18n.localize("SENTIUS.Level") },
      { content: game.i18n.localize("SENTIUS.Features") }
    ]];
  ...
  async _getFeatures(item, { modernStyle, optional=false }) {
    const prepareFeature = async (f, level) => {
      ...
      return {
        document,
        name: modernStyle ? game.i18n.format("JOURNALENTRYPAGE.SENTIUS.Class.Features.Name", {
