...
export default class Award extends DialogMixin(FormApplication) {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["sentius2", "award", "dialog"],
      template: "systems/sentius/templates/apps/award.hbs",
      title: "SENTIUS.Award.Title",
      width: 350,
      height: "auto",
      currency: null,
      xp: null,
  ...
  getData(options={}) {
    ...
    context.currency = Object.entries(CONFIG.SENTIUS.currencies).reduce((obj, [k, { label, icon }]) => {
      obj[k] = {
        label, icon,
    ...
    context.hideXP = game.settings.get("sentius", "levelingMode") === "noxp";
    context.noPrimaryParty = !game.settings.get("sentius", "primaryParty")?.actor && !this.isPartyAward;
  ...
  static prepareDestinations(destinations, savedDestinations) {
    const icons = {
      container: '<sentius-icon class="fa-fw" src="systems/sentius/icons/svg/backpack.svg"></sentius-icon>',
  ...
  _saveDestinations(destinations) {
    ...
    target.setFlag("sentius", "awardDestinations", destinations);
  }
  ...
  static async displayAwardMessages(results) {
    ...
    for ( const [destination, result] of results ) {
      ...
      if ( result.xp ) entries.push(`
        <span class="award-entry">
          ${formatNumber(result.xp)} ${game.i18n.localize("SENTIUS.ExperiencePointsAbbr")}
        </span>
      `);
      if ( !entries.length ) continue;

      const content = game.i18n.format("SENTIUS.Award.Message", {
        name: destination.name, award: `<span class="sentius2">${game.i18n.getListFormatter().format(entries)}</span>`
      });
  ...
  static async handleAward(message) {
    if ( !game.user.isGM ) {
      ui.notifications.error("SENTIUS.Award.NotGMError", { localize: true });
      return;
    }
    ...
    try {
      ...
      // Otherwise show the UI with defaults
        ...
        const savedDestinations = game.user.getFlag("sentius", "awardDestinations");
  ...
  static parseAwardCommand(message) {
    ...
    for ( const part of command.split(" ") ) {
      ...
      try {
        new Roll(amount);
        if ( label in CONFIG.SENTIUS.currencies ) currency[label] = amount;
    ...
    if ( unrecognized.length ) throw new Error(game.i18n.format("SENTIUS.Award.UnrecognizedWarning", {
