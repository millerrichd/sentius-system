...
export default class CompendiumBrowser extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  static DEFAULT_OPTIONS = {
    ...
    classes: ["sentius2", "compendium-browser", "vertical-tabs"],
    tag: "form",
    window: {
      title: "SENTIUS.CompendiumBrowser.Title",
      minimizable: true,
      resizable: true
    },
    actions: {
  ...
  static PARTS = {
    header: {
      id: "header",
      classes: ["header"],
      template: "systems/sentius/templates/compendium/browser-header.hbs"
    },
    search: {
      id: "sidebar-search",
      classes: ["sidebar-part", "filter-element"],
      template: "systems/sentius/templates/compendium/browser-sidebar-search.hbs"
    },
    types: {
      id: "sidebar-types",
      classes: ["sidebar-part"],
      template: "systems/sentius/templates/compendium/browser-sidebar-types.hbs"
    },
    filters: {
      id: "sidebar-filters",
      classes: ["sidebar-part"],
      template: "systems/sentius/templates/compendium/browser-sidebar-filters.hbs",
      templates: ["systems/sentius/templates/compendium/browser-sidebar-filter-set.hbs"]
    },
    results: {
      id: "results",
      classes: ["results"],
      template: "systems/sentius/templates/compendium/browser-results.hbs",
      templates: ["systems/sentius/templates/compendium/browser-entry.hbs"],
      scrollable: [""]
    },
    footer: {
      id: "footer",
      classes: ["footer"],
      template: "systems/sentius/templates/compendium/browser-footer.hbs"
    },
    tabs: {
      id: "tabs",
      classes: ["tabs", "tabs-left"],
      template: "systems/sentius/templates/compendium/browser-tabs.hbs"
    }
  };
  ...
  static TABS = [
    {
      ...
      svg: "systems/sentius/icons/svg/items/class.svg",
      documentClass: "Item",
      types: ["class"]
    },
    {
      ...
      svg: "systems/sentius/icons/svg/items/subclass.svg",
      documentClass: "Item",
      types: ["subclass"]
    },
    {
      ...
      svg: "systems/sentius/icons/svg/items/race.svg",
      documentClass: "Item",
      types: ["race"]
    },
    {
      tab: "feats",
      label: "SENTIUS.CompendiumBrowser.Tabs.Feat.other",
      icon: "fas fa-star",
      documentClass: "Item",
      types: ["feat"]
    },
    {
      ...
      svg: "systems/sentius/icons/svg/items/background.svg",
      documentClass: "Item",
      types: ["background"]
    },
    {
      tab: "physical",
      label: "SENTIUS.CompendiumBrowser.Tabs.Item.other",
      svg: "systems/sentius/icons/svg/backpack.svg",
      documentClass: "Item",
      types: ["physical"]
    },
    {
    ...
    {
      tab: "monsters",
      label: "SENTIUS.CompendiumBrowser.Tabs.Monster.other",
      svg: "systems/sentius/icons/svg/monster.svg",
      documentClass: "Actor",
      types: ["npc"]
    },
    {
      ...
      svg: "systems/sentius/icons/svg/vehicle.svg",
      documentClass: "Actor",
      types: ["vehicle"]
    },
    {
  ...
  _configureRenderOptions(options) {
    ...
    if ( options.isFirstRender ) {
      const tab = this.constructor.TABS.find(t => t.tab === this.options.tab);
      if ( tab ) foundry.utils.setProperty(options, "sentius.browser.types", tab.types);
    }
  }
  ...
  async _prepareContext(options) {
    ...
    context.filterDefinitions.set("source", {
      label: "SENTIUS.SOURCE.FIELDS.source.label",
      type: "set",
      config: {
        ...
        choices: foundry.utils.mergeObject(
          ...
          Object.fromEntries(Object.keys(this.options.filters?.locked?.additional?.source ?? {}).map(k => {
            return [k.slugify({ strict: true }), CONFIG.SENTIUS.sourceBooks[k] ?? k];
          })), { inplace: false }
        )
      }
    });
  ...
  async _prepareFooterContext(context, options) {
    ...
    context.summary = suffix ? game.i18n.format(
      `SENTIUS.CompendiumBrowser.Selection.Summary.${suffix}`, { max, min, value }
    ) : value;
    return context;
  }
  ...
  async _prepareSidebarContext(partId, context, options) {
    ...
    const types = foundry.utils.getProperty(options, "sentius.browser.types") ?? [];
    ...
    if ( game.user.isGM ) {
      frame.querySelector('[data-action="close"]').insertAdjacentHTML("beforebegin", `
        <button type="button" class="header-control fas fa-cog icon" data-action="configureSources"
                data-tooltip="SENTIUS.CompendiumBrowser.Sources.Label"
                aria-label="${game.i18n.localize("SENTIUS.CompendiumBrowser.Sources.Label")}"></button>
      `);
    }
    return frame;
  }
  ...
  async _renderResult(entry, documentClass) {
    ...
    const html = await renderTemplate("systems/sentius/templates/compendium/browser-entry.hbs", context);
    ...
    `;
    element.dataset.tooltipClass = "sentius2 sentius-tooltip item-tooltip";
  ...
  async _renderSourceFilters() {
    ...
    for ( const result of this.#results ) {
      ...
      sources.push({ slug, value: CONFIG.SENTIUS.sourceBooks[value] ?? value });
    }
    ...
    const filter = await renderTemplate("systems/sentius/templates/compendium/browser-sidebar-filter-set.hbs", {
      locked,
      value: locked,
      key: "source",
      label: "SENTIUS.SOURCE.FIELDS.source.label",
  ...
  changeTab(tab, group, options={}) {
    ...
    this.render({ parts: ["results", "filters", "types"], sentius: { browser: { types } }, changedTab: true });
  }
  ...
  #adjustCheckboxStates(htmlElement) {
    for ( const groupArea of htmlElement.querySelectorAll(".type-group") ) {
      const group = groupArea.querySelector(".type-group-header sentius-checkbox");
      const children = groupArea.querySelectorAll(".wrapper sentius-checkbox");
  ...
  static async #onHandleSubmit(event, form, formData) {
    ...
    if ( (value < (min || -Infinity)) || (value > (max || Infinity)) ) {
      ...
      throw new Error(game.i18n.format(`SENTIUS.CompendiumBrowser.Selection.Warning.${suffix}`, {
        max, min, value,
        document: game.i18n.localize(`SENTIUS.CompendiumBrowser.Selection.Warning.Document.${pr.select(max || min)}`)
      }));
    }

    /**
     * Hook event that calls when a compendium browser is submitted with selected items.
     * @function sentius.compendiumBrowserSelection
     * @memberof hookEvents
     ...
     */
    Hooks.callAll("sentius.compendiumBrowserSelection", this, this.#selected);
  }
  ...
  static async #onSetType(event, target) {
    ...
    if ( target.defaultValue ) {
      ...
      for ( const child of target.closest(".type-group").querySelectorAll("sentius-checkbox[value]") ) {
  ...
  static #onToggleMode(event, target) {
    ...
    this.render({ parts: ["results", "filters", "types", "tabs"], sentius: { browser: { types } }, changedTab: true });
  }
  ...
  static async fetch(documentClass, { types=new Set(), filters=[], index=true, indexFields=new Set(), sort=true }={}) {
    ...
    let documents = game.packs
      ...
      .filter(p => (p.metadata.type === documentClass.metadata.name)
        ...
        && (!types.size || !p.metadata.flags.sentius?.types || new Set(p.metadata.flags.sentius.types).intersects(types)))
      ...
      .map(async p => await Promise.all((await p.getIndex({ fields: Array.from(indexFields) })
        ...
        .then(index => game.sentius.moduleArt.apply(index)))
  ...
  static injectSidebarButton(html) {
    ...
    button.innerHTML = `
      ...
      ${game.i18n.localize("SENTIUS.CompendiumBrowser.Action.Open")}
    `;
