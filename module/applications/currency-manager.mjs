...
export default class CurrencyManager extends DialogMixin(FormApplication) {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["sentius2", "currency-manager", "dialog"],
      tabs: [{navSelector: "nav", contentSelector: ".sheet-content", initial: "transfer"}],
      template: "systems/sentius/templates/apps/currency-manager.hbs",
      title: "SENTIUS.CurrencyManager.Title",
      width: 350,
      height: "auto"
    });
  }
   ...
   * See CONFIG.SENTIUS.currencies for configuration.
  ...
  static convertCurrency(doc) {
    ...
    const currencies = Object.entries(CONFIG.SENTIUS.currencies);
  ...
  static deductActorCurrency(actor, amount, denomination, options={}) {
    ...
    if ( remainder ) throw new Error(game.i18n.format("SENTIUS.CurrencyManager.Error.InsufficientFunds", {
      denomination,
  ...
  static getActorCurrencyUpdates(actor, amount, denomination, { recursive=false, priority="low", exact=true }={}) {
    ...
    const currencies = Object.entries(CONFIG.SENTIUS.currencies).map(([denom, { conversion }]) => {
      return [denom, conversion];
    }).sort(([, a], [, b]) => priority === "high" ? a - b : b - a);
    const baseConversion = CONFIG.SENTIUS.currencies[denomination].conversion;
