...
export class ModuleArt {
  ...
  async #parseArtMapping(moduleId, mapping, credit) {
    let settings = game.settings.get("sentius", "moduleArtConfiguration")?.[moduleId];
  ...
  static getModuleArtPath(module) {
    ...
    const artPath = flags?.["sentius-art"];
  ...
  static getArtModules() {
    const settings = game.settings.get("sentius", "moduleArtConfiguration");
    const unsorted = [];
    const configs = [{
      id: game.system.id,
      label: game.system.title,
      mapping: "systems/sentius/json/fa-token-mapping.json",
      priority: settings.sentius?.priority ?? CONST.SORT_INTEGER_DENSITY,
      credit: `
        <em>
          Token artwork by
    ...
    for ( const module of game.modules ) {
      ...
      if ( !mapping ) continue;
      const config = { id: module.id, label: module.title, credit: flags?.["sentius-art-credit"], mapping };
      configs.push(config);
