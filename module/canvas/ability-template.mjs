...
export default class AbilityTemplate extends MeasuredTemplate {
  ...
  static fromActivity(activity, options={}) {
    ...
    const templateShape = sentius.config.areaTargetTypes[target.type]?.template;
    ...
    switch ( templateShape ) {
      ...
      case "rect": // 5e rectangular AoEs are always cubes
        ...
        if ( game.settings.get("sentius", "gridAlignedSquareTemplates") ) {
     ...
     * @function sentius.preCreateActivityTemplate
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("sentius.preCreateActivityTemplate", activity, templateData) === false ) return null;

    if ( "sentius.preCreateItemTemplate" in Hooks.events ) {
      foundry.utils.logCompatibilityWarning(
        "The `sentius.preCreateItemTemplate` hook has been deprecated and replaced with `sentius.preCreateActivityTemplate`.",
        { since: "SENTIUS", until: "SENTIUS"" }
      );
      if ( Hooks.call("sentius.preCreateItemTemplate", activity.item, templateData) === false ) return null;
    }
     ...
     * @function sentius.createActivityTemplate
     * @memberof hookEvents
     * @param {Activity} activity            Activity for which the template is being placed.
     * @param {AbilityTemplate[]} templates  The templates being placed.
     */
    Hooks.callAll("sentius.createActivityTemplate", activity, created);

    if ( "sentius.createItemTemplate" in Hooks.events ) {
      foundry.utils.logCompatibilityWarning(
        "The `sentius.createItemTemplate` hook has been deprecated and replaced with `sentius.createActivityTemplate`.",
        { since: "SENTIUS", until: "SENTIUS"" }
      );
      Hooks.callAll("sentius.createItemTemplate", activity.item, created[0]);
    }
  ...
  _onMovePlacement(event) {
    ...
    const baseDistance = this.document.flags.sentius?.dimensions?.size;
    if ( this.document.flags.sentius?.dimensions?.adjustedSize && baseDistance ) {
