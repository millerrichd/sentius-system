...
class MessageRegistry {
  ...
  static track(message) {
    const origin = message.getFlag("sentius", "originatingMessage");
    const type = message.getFlag("sentius", "roll.type");
...
class SpellListRegistry {
   ...
   */
  static get options() {
    return Object.entries(CONFIG.SENTIUS.spellListTypes).map(([type, group]) => {
   ...
   * @param {string} type        Type of list as defined in `CONFIG.SENTIUS.spellListTypes`.
...
export class SpellList {
   ...
   */
  get name() {
    return sentius.registry[SpellList.#REGISTRIES[this.metadata.type]]?.get(this.metadata.identifier)?.name
      ?? this.metadata.name;
  }
