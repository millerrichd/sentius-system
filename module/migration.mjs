...
export async function migrateWorld({ bypassVersionCheck=false }={}) {
  ...
  for ( const [actor, valid] of actors ) {
    try {
      ...
      err.message = `Failed sentius system migration for Actor ${actor.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( const [item, valid] of items ) {
    try {
      ...
      err.message = `Failed sentius system migration for Item ${item.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( const m of game.macros ) {
    try {
      ...
      err.message = `Failed sentius system migration for Macro ${m.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( const table of game.tables ) {
    try {
      ...
      err.message = `Failed sentius system migration for RollTable ${table.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( const s of game.scenes ) {
    try {
      ...
      err.message = `Failed sentius system migration for Scene ${s.name}: ${err.message}`;
      console.error(err);
    }
    ...
    for ( const token of s.tokens ) {
      ...
      try {
        ...
        err.message = `Failed sentius system migration for ActorDelta [${token.id}]: ${err.message}`;
        console.error(err);
      }
    }
  }
  ...
  game.settings.set("sentius", "systemMigrationVersion", game.system.version);
...
export async function migrateCompendium(pack, { bypassVersionCheck=false, strict=false }={}) {
  ...
  try {
    ...
    sentius.moduleArt.suppressArt = true;
    ...
    for ( let doc of documents ) {
      ...
      // Handle migration failures
        ...
        err.message = `Failed sentius system migration for document ${doc.name} in pack ${pack.collection}: ${err.message}`;
        console.error(err);
        if ( strict ) throw err;
      }
    }
    ...
    sentius.moduleArt.suppressArt = false;
  }
}
...
export async function refreshCompendium(pack, { bypassVersionCheck, migrate=true }={}) {
  ...
  if ( migrate ) {
    try {
      ...
      err.message = `Failed sentius system migration pack ${pack.collection}: ${err.message}`;
      console.error(err);
      return;
    }
  }
...
export async function migrateArmorClass(pack) {
  ...
  const updates = [];
  const armor = new Set(Object.keys(CONFIG.SENTIUS.armorTypes));
...
export async function migrateSettings() {
  ...
  const disableExperienceTracking = game.settings.storage.get("world")
    ?.find(s => s.key === "sentius.disableExperienceTracking")?.value;
  const levelingMode = game.settings.storage.get("world")?.find(s => s.key === "sentius.levelingMode")?.value;
  if ( (disableExperienceTracking !== undefined) && (levelingMode === undefined) ) {
    await game.settings.set("sentius", "levelingMode", "noxp");
  }
}
...
export function migrateItemData(item, itemData, migrationData, flags={}) {
  ...
  // Migrate embedded effects
  if ( itemData.effects ) {
    const riders = foundry.utils.getProperty(itemData, "flags.sentius.riders.effect");
    if ( riders?.length ) updateData["flags.sentius.riders.effect"] = riders;
    const effects = migrateEffects(itemData, migrationData, updateData);
    if ( riders?.length === updateData["flags.sentius.riders.effect"]?.length ) {
      delete updateData["flags.sentius.riders.effect"];
    }
    if ( effects.length > 0 ) updateData.effects = effects;
  }
  ...
  // Migrate properties
  const migratedProperties = foundry.utils.getProperty(itemData, "flags.sentius.migratedProperties");
  if ( migratedProperties?.length ) {
    ...
    updateData["flags.sentius.-=migratedProperties"] = null;
  }

  if ( foundry.utils.getProperty(itemData, "flags.sentius.persistSourceMigration") ) {
    flags.persistSourceMigration = true;
    updateData["flags.sentius.-=persistSourceMigration"] = null;
  }
...
export function migrateEffects(parent, migrationData, itemUpdateData) {
  ...
  return parent.effects.reduce((arr, e) => {
    ...
    if ( effectData.flags?.sentius?.rider ) {
      itemUpdateData["flags.sentius.riders.effect"] ??= [];
      itemUpdateData["flags.sentius.riders.effect"].push(effectData._id);
      effectUpdate["flags.sentius.-=rider"] = null;
    }
...
export const getMigrationData = async function() {
  const data = {};
  try {
    const icons = await fetch("systems/sentius/json/icon-migration.json");
    const spellIcons = await fetch("systems/sentius/json/spell-icon-migration.json");
...
function _migrateTokenImage(actorData, updateData) {
  const oldSystemPNG = /^systems\/sentius\/tokens\/([a-z]+)\/([A-z]+).png$/;
  for ( const path of ["texture.src", "prototypeToken.texture.src"] ) {
    ...
    if ( oldSystemPNG.test(v) ) {
      ...
      updateData[path] = `systems/sentius/tokens/${type}/${fileName}.webp`;
    }
  }
  return updateData;
}
...
function _migrateItemUses(item, itemData, updateData, flags) {
  const value = foundry.utils.getProperty(itemData, "flags.sentius.migratedUses");
  ...
  if ( value !== undefined ) updateData["flags.sentius.-=migratedUses"] = null;
}
...
/**
 * Migrate macros from the old 'sentius.rollItemMacro' and 'sentius.macros' commands to the new location.
 * @param {object} macro       Macro data to migrate.
 * @param {object} updateData  Existing update to expand upon.
 * @returns {object}           The updateData to apply.
 */
function _migrateMacroCommands(macro, updateData) {
  if ( macro.command.includes("game.sentius.rollItemMacro") ) {
    updateData.command = macro.command.replaceAll("game.sentius.rollItemMacro", "sentius.documents.macro.rollItem");
  } else if ( macro.command.includes("game.sentius.macros.") ) {
    updateData.command = macro.command.replaceAll("game.sentius.macros.", "sentius.documents.macro.");
  }
  return updateData;
}
...
export async function purgeFlags(pack) {
  const cleanFlags = flags => {
    const flags5e = flags.sentius || null;
    return flags5e ? {sentius: flags5e} : {};
  };
  await pack.configure({locked: false});
