...
export default class BasicRoll extends Roll {
  ...
  static async buildConfigure(config={}, dialog={}, message={}) {
     ...
     * method (e.g. `sentius.preRollSkillV2`, `sentius.preRollAbilityCheckV2`, `sentius.preRollV2`). Exact contents of the
     * configuration object will also change based on the roll type, but the same objects will always be present.
     * @function sentius.preRollV2
     * @memberof hookEvents
    ...
    for ( const hookName of config.hookNames ) {
      if ( Hooks.call(`sentius.preRoll${hookName.capitalize()}V2`, config, dialog, message) === false ) return [];
    }
     ...
     * Multiple hooks may be called depending on the rolling method (e.g. `sentius.postSkillCheckRollConfiguration`,
     * `sentius.postAbilityTestRollConfiguration`, and `sentius.postRollConfiguration` for skill checks). Exact contents of
     * the configuration object will also change based on the roll type, but the same objects will always be present.
     * @function sentius.postRollConfiguration
     * @memberof hookEvents
     ...
     */
    for ( const hookName of config.hookNames ) {
      const name = `sentius.post${hookName.capitalize()}RollConfiguration`;
  ...
  static async buildPost(rolls, config, message) {
    ...
    if ( messageId ) foundry.utils.setProperty(message.data, "flags.sentius.originatingMessage", messageId);
